...
.DESCRIPTION 
		Microsoft 365 Office application management. Remediation and repair operations.
...
try {
    ...
    [Parameter(Mandatory = $false)]
    [bool]$RestartApplications = $false,
    
    [Parameter(Mandatory = $false)]
    [bool]$ClearAllCredentials = $false
)
...
# Remediation result object
$RemediationResult = [PSCustomObject]@{
    ScriptName = "Remediate_M365_Office_ClearCachedCredentials"
...
function Stop-OfficeApplications {
    Write-LogEntry "Stopping Office applications before credential clearing"
    
    try {
        $OfficeProcesses = @("WINWORD", "EXCEL", "POWERPNT", "OUTLOOK", "TEAMS", "ONEDRIVE", "MSOSYNC")
        $StoppedProcesses = @()
        
        foreach ($ProcessName in $OfficeProcesses) {
            $Processes = Get-Process -Name $ProcessName -ErrorAction SilentlyContinue
            if ($Processes) {
                Write-LogEntry "Stopping $($Processes.Count) instance(s) of $ProcessName"
                foreach ($Process in $Processes) {
                    try {
                        ...
                        Write-LogEntry "Successfully stopped $ProcessName (PID: $($Process.Id))"
                    }
                    catch {
                        Write-LogEntry "Failed to stop $ProcessName (PID: $($Process.Id)): $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Warning"
        ...
        $RemediationResult.ApplicationsRestarted = $StoppedProcesses
        $RemediationResult.ActionsPerformed += "Stopped Office applications: $($StoppedProcesses -join ', ')"
        ...
        Write-LogEntry "Error stopping Office applications: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
        $RemediationResult.Errors += "Failed to stop Office applications: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
function Backup-CredentialData {
    param(
        [Parameter(Mandatory = $true)]
        [string]$BackupPath
    )
    
    Write-LogEntry "Creating backup of credential data to: $BackupPath"
    ...
    try {
        ...
        # Backup credential manager entries
        $CredBackupFile = Join-Path $BackupPath "credential_manager_backup.txt"
        $CredList = cmdkey /list 2>$null
        if ($CredList) {
            $CredList | Out-File -FilePath $CredBackupFile -Encoding UTF8
            Write-LogEntry "Credential manager entries backed up to: $CredBackupFile"
        }
        
        # Backup relevant registry entries
        $RegistryBackupPaths = @(
        ...
        foreach ($RegPath in $RegistryBackupPaths) {
            if (Test-Path $RegPath) {
                $RegFileName = ($RegPath -replace ':', '' -replace '\\', '_') + ".reg"
                $RegBackupFile = Join-Path $BackupPath $RegFileName
                
                try {
                    $RegKeyPath = $RegPath -replace 'HKCU:', 'HKEY_CURRENT_USER'
                    & reg export $RegKeyPath $RegBackupFile /y 2>$null
                    if ($LASTEXITCODE -eq 0) {
                        Write-LogEntry "Registry backup created: $RegBackupFile"
                    }
                }
                catch {
                    Write-LogEntry "Failed to backup registry path $RegPath" -Level "Warning"
                }
            }
        }
        
        $RemediationResult.BackupLocation = $BackupPath
        $RemediationResult.ActionsPerformed += "Created credential backup at: $BackupPath"
        return $true
    }
    catch {
        Write-LogEntry "Error creating credential backup: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
        $RemediationResult.Errors += "Backup creation failed: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
function Clear-CredentialManagerEntries {
    Write-LogEntry "Clearing Office-related entries from Windows Credential Manager"
    ...
    try {
        ...
        if ($CredList) {
            # Extract Office-related credential targets
            $OfficeTargets = $CredList | Where-Object { 
                <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "Target:.*microsoft|office|outlook|teams|onedrive" -or
                <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "msteams|office365|login\.microsoftonline|graph\.microsoft"
            }
            
            foreach ($TargetLine in $OfficeTargets) {
                if ($TargetLine -match "Target: (.+)") {
                    $TargetName = $matches[1].Trim()
                    
                    try {
                        Write-LogEntry "Removing credential target: $TargetName"
                        & cmdkey /delete:$TargetName 2>$null
                        
                        if ($LASTEXITCODE -eq 0) {
                            $ClearedCredentials += $TargetName
                            Write-LogEntry "Successfully removed credential: $TargetName"
                        }
                        else {
                            Write-LogEntry "Failed to remove credential: $TargetName" -Level "Warning"
                        }
                    }
                    catch {
                        Write-LogEntry "Error removing credential $TargetName : $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Warning"
        ...
        $RemediationResult.CredentialsCleared = $ClearedCredentials
        $RemediationResult.ActionsPerformed += "Cleared $($ClearedCredentials.Count) credential manager entries"
        
        Write-LogEntry "Credential Manager cleanup completed. Removed $($ClearedCredentials.Count) entries"
        return $true
    }
    catch {
        Write-LogEntry "Error clearing Credential Manager entries: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
        $RemediationResult.Errors += "Credential Manager clearing failed: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
function Clear-RegistryCredentialData {
    Write-LogEntry "Clearing Office credential data from registry"
    ...
    try {
        ...
        $RegistryPaths = @(
            ...
            "HKCU:\SOFTWARE\Microsoft\Office\16.0\Common\ExperimentConfigs\AuthFailures"
        )
        
        foreach ($Path in $RegistryPaths) {
            if (Test-Path $Path) {
                try {
                    Write-LogEntry "Clearing registry path: $Path"
                    
                    # Get child items before removal for logging
                    $ChildItems = Get-ChildItem $Path -ErrorAction SilentlyContinue
                    
                    if ($ChildItems) {
                        foreach ($Child in $ChildItems) {
                            Remove-Item $Child.PSPath -Recurse -Force -ErrorAction SilentlyContinue
                            $RemovedEntries += $Child.PSPath
                        }
                    }
                    
                    # Also clear properties that might contain credential data
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "credential|token|auth|identity" 
                        } | ForEach-Object {
                            try {
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -Force -ErrorAction SilentlyContinue
                                $RemovedEntries += "$Path\$(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Name)"
                                ...
                                Write-LogEntry "Could not remove property $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name) from $Path" -Level "Warning"
                            }
                        }
                    }
                    
                    Write-LogEntry "Successfully cleared registry path: $Path"
                }
                catch {
                    Write-LogEntry "Error clearing registry path $Path : $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Warning"
        ...
        # Clear specific Office authentication registry values
        $SpecificValues = @(
            @{ Path = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Common\Identity"; Name = "DisableADAL" },
            @{ Path = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Common\Identity"; Name = "DisableAADWAM" }
        )
        
        foreach ($Value in $SpecificValues) {
            if (Test-Path $Value.Path) {
                try {
                    Remove-ItemProperty -Path $Value.Path -Name $Value.Name -Force -ErrorAction SilentlyContinue
                    $RemovedEntries += "$($Value.Path)\$($Value.Name)"
                    Write-LogEntry "Removed registry value: $($Value.Path)\$($Value.Name)"
                }
                catch {
                    # This is expected if the value doesn't exist
                }
            }
        }
        
        $RemediationResult.RegistryEntriesRemoved = $RemovedEntries
        $RemediationResult.ActionsPerformed += "Cleared $($RemovedEntries.Count) registry credential entries"
        
        Write-LogEntry "Registry credential cleanup completed. Removed $($RemovedEntries.Count) entries"
        return $true
    }
    catch {
        Write-LogEntry "Error clearing registry credential data: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
        $RemediationResult.Errors += "Registry credential clearing failed: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
function Clear-TemporaryCredentialFiles {
    Write-LogEntry "Clearing temporary Office credential files"
    ...
    try {
        ...
        foreach ($TempPath in $TempPaths) {
            if (Test-Path $TempPath) {
                try {
                    Write-LogEntry "Clearing temporary files in: $TempPath"
                    
                    $Files = Get-ChildItem $TempPath -Recurse -Force -ErrorAction SilentlyContinue | Where-Object {
                        <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "credential|token|auth|identity|cache" -or
                        <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Extension -in @(".dat", ".tmp", ".cache")
                    }
                    ...
                    foreach ($File in $Files) {
                        try {
                            ...
                            Write-LogEntry "Removed file: $($File.FullName)"
                        }
                        catch {
                            Write-LogEntry "Could not remove file: $($File.FullName)" -Level "Warning"
                        }
                    }
                }
                catch {
                    Write-LogEntry "Error accessing temp path $TempPath : $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Warning"
        ...
        # Clear IE/Edge credential cache that Office might use
        $IECachePath = "$env:LOCALAPPDATA\Microsoft\Windows\INetCache"
        if (Test-Path $IECachePath) {
            try {
                $IECredFiles = Get-ChildItem $IECachePath -Recurse -Force -ErrorAction SilentlyContinue | Where-Object {
                    <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "microsoft|office|login" -and <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Extension -eq ".dat"
                }
                
                foreach ($File in $IECredFiles) {
                    try {
                        Remove-Item $File.FullName -Force -ErrorAction SilentlyContinue
                        $RemovedFiles += $File.FullName
                    }
                    catch {
                        # Expected for locked files
                    }
                }
            }
            catch {
                Write-LogEntry "Could not access IE cache for credential cleanup" -Level "Warning"
            }
        }
        
        $RemediationResult.FilesRemoved = $RemovedFiles
        $RemediationResult.ActionsPerformed += "Cleared $($RemovedFiles.Count) temporary credential files"
        
        Write-LogEntry "Temporary credential file cleanup completed. Removed $($RemovedFiles.Count) files"
        return $true
    }
    catch {
        Write-LogEntry "Error clearing temporary credential files: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
        $RemediationResult.Errors += "Temporary file clearing failed: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
function Start-OfficeApplications {
    if (-not $RestartApplications) {
        Write-LogEntry "Application restart not requested - skipping"
        return $true
    }
    
    Write-LogEntry "Restarting Office applications"
    
    try {
        $RestartedApps = @()
        
        # Only restart applications that were previously running
        foreach ($AppName in $RemediationResult.ApplicationsRestarted) {
            try {
                $AppPath = $null
                
                switch ($AppName.ToUpper()) {
                    "WINWORD" { 
                        $AppPath = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Winword.exe" -Name "(default)" -ErrorAction SilentlyContinue
                        if ($AppPath) { $AppPath = $AppPath.'(default)' }
                    }
                    "EXCEL" { 
                        $AppPath = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Excel.exe" -Name "(default)" -ErrorAction SilentlyContinue
                        if ($AppPath) { $AppPath = $AppPath.'(default)' }
                    }
                    "OUTLOOK" { 
                        $AppPath = Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\Outlook.exe" -Name "(default)" -ErrorAction SilentlyContinue
                        if ($AppPath) { $AppPath = $AppPath.'(default)' }
                    }
                }
                
                if ($AppPath -and (Test-Path $AppPath)) {
                    Write-LogEntry "Starting application: $AppPath"
                    Start-Process $AppPath -ErrorAction SilentlyContinue
                    $RestartedApps += $AppName
                    Start-Sleep -Seconds 2
                }
            }
            catch {
                Write-LogEntry "Failed to restart $AppName : $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Warning"
        ...
        $RemediationResult.ActionsPerformed += "Restarted applications: $($RestartedApps -join ', ')"
        Write-LogEntry "Application restart completed. Restarted: $($RestartedApps -join ', ')"
        return $true
    }
    catch {
        Write-LogEntry "Error restarting Office applications: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
        $RemediationResult.Errors += "Application restart failed: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
function Test-RemediationSuccess {
    Write-LogEntry "Verifying credential clearing was successful"
    
    try {
        $ValidationIssues = @()
        
        # Check if credential manager still has problematic entries
        $RemainingCreds = cmdkey /list 2>$null | Where-Object { 
            <#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "ERROR|expired|invalid"
        }
        
        if ($RemainingCreds) {
            $ValidationIssues += "Corrupted credentials still present in Credential Manager"
        }
        
        # Check if registry still has stale authentication data
        ...
        if (Test-Path $AuthFailurePath) {
            ...
            if ($FailureEntries.Count -gt 0) {
                $ValidationIssues += "Authentication failure entries still present in registry"
            }
        }
        
        if ($ValidationIssues.Count -eq 0) {
            Write-LogEntry "Remediation validation successful - no remaining credential issues detected"
            return $true
        }
        else {
            Write-LogEntry "Remediation validation found remaining issues: $($ValidationIssues -join '; ')" -Level "Warning"
            $RemediationResult.Errors += $ValidationIssues
            return $false
        }
    }
    catch {
        Write-LogEntry "Error during remediation validation: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
Write-LogEntry "Starting M365 Office cached credential remediation"

try {
    # Create backup if requested
    if ($BackupCredentials) {
        $BackupPath = Join-Path $env:TEMP "OfficeCredentialBackup_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
        if (-not (Backup-CredentialData -BackupPath $BackupPath)) {
            Write-LogEntry "Backup creation failed - continuing with remediation" -Level "Warning"
        }
    }
    
    # Stop Office applications
    $AppsStoppedSuccessfully = Stop-OfficeApplications
    
    # Perform credential clearing operations
    $SuccessCount = 0
    $TotalOperations = 3
    
    if (Clear-CredentialManagerEntries) { $SuccessCount++ }
    if (Clear-RegistryCredentialData) { $SuccessCount++ }
    if (Clear-TemporaryCredentialFiles) { $SuccessCount++ }
    
    # Restart applications if requested and if they were stopped successfully
    if ($AppsStoppedSuccessfully) {
        Start-OfficeApplications
    }
    ...
    if ($SuccessCount -eq $TotalOperations -and $ValidationSuccess) {
        $RemediationResult.Success = $true
        $RemediationResult.ExitCode = 0
        Write-LogEntry "Credential remediation completed successfully"
    }
    elseif ($SuccessCount -gt 0) {
        $RemediationResult.Success = $false
        $RemediationResult.ExitCode = 2
        Write-LogEntry "Credential remediation partially completed ($SuccessCount/$TotalOperations operations successful)" -Level "Warning"
    }
    else {
        $RemediationResult.Success = $false
        $RemediationResult.ExitCode = 1
        Write-LogEntry "Credential remediation failed - no operations completed successfully" -Level "Error"
    }
}
catch {
    Write-LogEntry "Critical error during remediation: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
    $RemediationResult.Success = $false
    $RemediationResult.ExitCode = 1
    $RemediationResult.Errors += "Critical remediation error: $(<#
.SCRIPT NAME
		Remediate_M365_Office_ClearCachedCredentials.ps1
...
# Summary logging
Write-LogEntry "Remediation completed. Success: $($RemediationResult.Success). Actions Performed: $($RemediationResult.ActionsPerformed.Count). Errors: $($RemediationResult.Errors.Count). Exit Code: $($RemediationResult.ExitCode)"
