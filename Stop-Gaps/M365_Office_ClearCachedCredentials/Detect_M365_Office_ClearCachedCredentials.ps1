...
.DESCRIPTION 
		Microsoft 365 Office application management. Detection and health checking.
...
try {
    ...
    [Parameter(Mandatory = $false)]
    [ValidateRange(1, 365)]
    [int]$MaxCredentialAge = 30,
    
    [Parameter(Mandatory = $false)]
    [ValidateNotNullOrEmpty()]
    [string[]]$CheckApplications = @("WINWORD.EXE", "EXCEL.EXE", "POWERPNT.EXE", "OUTLOOK.EXE", "TEAMS.EXE")
)
...
$DetectionResult = [PSCustomObject]@{
    ScriptName = "Detect_M365_Office_ClearCachedCredentials"
...
function Test-OfficeInstallation {
    Write-LogEntry "Checking Office installation status"
    ...
    foreach ($Path in $RegistryPaths) {
        if (Test-Path $Path) {
            $Versions = Get-ChildItem $Path -ErrorAction SilentlyContinue | Where-Object { <#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match '\d+\.\d+' }
            foreach ($Version in $Versions) {
                $VersionNumber = $Version.PSChildName
                $InstallRoot = Get-ItemProperty -Path "$($Version.PSPath)\Common\InstallRoot" -Name "Path" -ErrorAction SilentlyContinue
                if ($InstallRoot) {
    ...
    $DetectionResult.Details["OfficeVersions"] = $OfficeVersions
    return $OfficeVersions.Count -gt 0
}

function Test-CredentialManagerEntries {
    Write-LogEntry "Checking Windows Credential Manager for Office entries"
    
    try {
        $CredentialIssues = @()
        $OfficeCredentials = @()
        
        # Check for Office-related stored credentials
        $StoredCreds = cmdkey /list 2>$null
        if ($StoredCreds) {
            $OfficeTargets = $StoredCreds | Where-Object { 
                <#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "msteams|office365|login\.microsoftonline"
            }
            
            foreach ($Target in $OfficeTargets) {
                if ($Target -match "Target: (.+)") {
                    $TargetName = $matches[1]
                    $OfficeCredentials += $TargetName
                    
                    # Check for expired or corrupted credentials
                    $CredInfo = cmdkey /list:$TargetName 2>$null
                    if ($CredInfo -match "ERROR|expired|invalid") {
                        $CredentialIssues += "Corrupted credential found: $TargetName"
                    }
                }
            }
        }
        
        $DetectionResult.Details["OfficeCredentials"] = $OfficeCredentials
        $DetectionResult.Details["CredentialCount"] = $OfficeCredentials.Count
        
        if ($CredentialIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $CredentialIssues
            Write-LogEntry "Found $($CredentialIssues.Count) credential issues" -Level "Warning"
            return $true
        }
        
        if ($OfficeCredentials.Count -gt 10) {
            $DetectionResult.IssuesFound += "Excessive number of stored Office credentials detected ($($OfficeCredentials.Count))"
            Write-LogEntry "Excessive Office credentials detected: $($OfficeCredentials.Count)" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "Credential Manager check completed - $($OfficeCredentials.Count) Office credentials found"
        return $false
    }
    catch {
        Write-LogEntry "Error checking Credential Manager: $(<#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
function Test-RegistryCredentialData {
    Write-LogEntry "Checking registry for Office credential data"
    ...
    try {
        ...
        foreach ($Path in $CheckPaths) {
            if (Test-Path $Path) {
                ...
                $CredentialEntries = $Items | Where-Object { 
                    <#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.PSChildName -match "credential|token|auth|identity" -or
                    (<#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Property -and <#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Property -match "credential|token|auth")
                }
                
                foreach ($Entry in $CredentialEntries) {
                    try {
                        $LastWrite = $Entry.LastWriteTime
                        if ($LastWrite -lt (Get-Date).AddDays(-$MaxCredentialAge)) {
                            $RegistryIssues += "Stale credential data in registry: $($Entry.PSPath) (Last modified: $LastWrite)"
                        }
                    }
                    catch {
                        $RegistryIssues += "Unable to read credential data: $($Entry.PSPath)"
                    }
                }
            }
        }
        
        # Check for specific authentication failures in registry
        $AuthFailurePath = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Common\ExperimentConfigs\AuthFailures"
        if (Test-Path $AuthFailurePath) {
            $FailureCount = (Get-ItemProperty $AuthFailurePath -ErrorAction SilentlyContinue).PSObject.Properties.Count
            if ($FailureCount -gt 0) {
                $RegistryIssues += "Authentication failures detected in registry ($FailureCount entries)"
            }
        }
        
        $DetectionResult.Details["RegistryIssues"] = $RegistryIssues
        
        if ($RegistryIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $RegistryIssues
            Write-LogEntry "Found $($RegistryIssues.Count) registry credential issues" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "Registry credential check completed - no issues found"
        return $false
    }
    catch {
        Write-LogEntry "Error checking registry credential data: $(<#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
function Test-ApplicationAuthStatus {
    Write-LogEntry "Checking Office application authentication status"
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.ProcessName -in ($CheckApplications -replace '\.EXE', '') }
        
        foreach ($App in $RunningApps) {
            # Check if application has authentication-related threads or handles
            $AppName = $App.ProcessName
            Write-LogEntry "Checking authentication status for $AppName"
            
            # Check for authentication error events in recent logs
            $AuthErrors = Get-WinEvent -FilterHashtable @{
                LogName = 'Application'
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.LevelDisplayName -eq "Error" -and
                (<#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "authentication|credential|sign.*in|login" -or
                 <#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "0x80070005|0x8007000E|0x80040111")
            }
            
            if ($AuthErrors) {
                $AuthIssues += "Authentication errors detected for $AppName in event logs ($($AuthErrors.Count) events)"
            }
        }
        
        # Check for recent Office activation issues
        $ActivationErrors = Get-WinEvent -FilterHashtable @{
            LogName = 'Application'
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "activation|license|subscription"
        }
        
        if ($ActivationErrors) {
            $AuthIssues += "Office activation/licensing errors detected ($($ActivationErrors.Count) events)"
        }
        
        $DetectionResult.Details["AuthenticationIssues"] = $AuthIssues
        $DetectionResult.Details["RunningOfficeApps"] = $RunningApps.ProcessName
        
        if ($AuthIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $AuthIssues
            Write-LogEntry "Found $($AuthIssues.Count) application authentication issues" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "Application authentication check completed - no issues found"
        return $false
    }
    catch {
        Write-LogEntry "Error checking application authentication status: $(<#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
function Test-UserReportedIssues {
    Write-LogEntry "Checking for user-reported authentication issues"
    
    try {
        $UserIssues = @()
        
        # Check for recent Office crashes that might indicate auth issues
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "winword|excel|powerpnt|outlook|teams" -and
            <#
.SCRIPT NAME
		Detect_M365_Office_ClearCachedCredentials.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.LevelDisplayName -eq "Error"
        ...
        if ($CrashEvents.Count -gt 5) {
            $UserIssues += "High number of Office application crashes detected ($($CrashEvents.Count) in past 7 days)"
        }
        
        # Check for sign-in prompts persistence (registry indicators)
        $SignInPromptPath = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Common\Identity\Settings"
        if (Test-Path $SignInPromptPath) {
            $DisableADAL = Get-ItemProperty $SignInPromptPath -Name "DisableADAL" -ErrorAction SilentlyContinue
            if ($DisableADAL -and $DisableADAL.DisableADAL -eq 1) {
                $UserIssues += "ADAL authentication disabled - may indicate previous credential issues"
            }
        }
        
        # Check for multiple Office profiles (indication of auth troubleshooting)
        $ProfilePath = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Outlook\Profiles"
        if (Test-Path $ProfilePath) {
            $ProfileCount = (Get-ChildItem $ProfilePath -ErrorAction SilentlyContinue).Count
            if ($ProfileCount -gt 2) {
                $UserIssues += "Multiple Outlook profiles detected ($ProfileCount) - may indicate authentication troubleshooting"
            }
        }
        
        $DetectionResult.Details["UserReportedIssues"] = $UserIssues
...
Write-LogEntry "Starting M365 Office cached credential detection"

try {
    # Verify Office is installed
    if (-not (Test-OfficeInstallation)) {
        Write-LogEntry "Microsoft Office installation not detected - skipping credential detection" -Level "Warning"
        $DetectionResult.Details["SkipReason"] = "Office not installed"
        ...
        if (Test-CredentialManagerEntries) { $IssueDetected = $true }
        if (Test-RegistryCredentialData) { $IssueDetected = $true }
        if (Test-ApplicationAuthStatus) { $IssueDetected = $true }
        if (Test-UserReportedIssues) { $IssueDetected = $true }
        
        # Determine final result
        if ($IssueDetected) {
            $DetectionResult.ActionRequired = $true
            $DetectionResult.ExitCode = 1
            $DetectionResult.Recommendations += "Clear cached Office credentials using credential manager and registry cleanup"
            $DetectionResult.Recommendations += "Restart affected Office applications after credential clearing"
            $DetectionResult.Recommendations += "Monitor authentication success after remediation"
            
            Write-LogEntry "Credential issues detected - remediation required" -Level "Warning"
        }
        else {
            $DetectionResult.ActionRequired = $false
            $DetectionResult.ExitCode = 0
            Write-LogEntry "No credential issues detected - Office authentication appears healthy"
        }
    }
}
catch {
