...
.DESCRIPTION 
		Chrome browser cache and data management. Detection and health checking.
...
try {
    ...
    [string]$LogSource = "SysTrack-Browser-Chrome-Cache",
    ...
    [Parameter(Mandatory = $false)]
    [ValidateNotNullOrEmpty()]
    [string[]]$CheckUserProfiles = @($env:USERNAME),
    
    [Parameter(Mandatory = $false)]
    [ValidateRange(1, 30)]
    [int]$PerformanceThresholdDays = 7
)
...
$DetectionResult = [PSCustomObject]@{
    ScriptName = "Detect_Browser_Chrome_ClearCache"
...
function Test-ChromeInstallation {
    Write-LogEntry "Checking Google Chrome installation status"
    ...
    try {
        ...
        foreach ($Path in $ChromePaths) {
            if (Test-Path $Path) {
                $VersionInfo = Get-ItemProperty $Path -ErrorAction SilentlyContinue
        ...
        foreach ($RegPath in $RegPaths) {
            if (Test-Path $RegPath) {
                $ChromeReg = Get-ItemProperty $RegPath -ErrorAction SilentlyContinue
                if ($ChromeReg) {
                    $DetectionResult.BrowserInfo["RegistryVersion"] = $ChromeReg.DisplayVersion
                    $DetectionResult.BrowserInfo["InstallLocation"] = $ChromeReg.InstallLocation
                }
            }
        }
        
        $DetectionResult.BrowserInfo["Installations"] = $ChromeInstallations
        $DetectionResult.BrowserInfo["InstallationCount"] = $ChromeInstallations.Count
        
        if ($ChromeInstallations.Count -gt 0) {
            Write-LogEntry "Found $($ChromeInstallations.Count) Chrome installation(s)"
            return $true
        }
        else {
            Write-LogEntry "Google Chrome installation not detected" -Level "Warning"
            return $false
        }
    }
    catch {
        Write-LogEntry "Error checking Chrome installation: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
function Test-ChromeCacheSize {
    Write-LogEntry "Analyzing Chrome cache size and structure"
    ...
    try {
        ...
        foreach ($UserProfile in $CheckUserProfiles) {
            $UserCachePath = if ($UserProfile -eq $env:USERNAME) {
            ...
            if (Test-Path $UserCachePath) {
                Write-LogEntry "Analyzing cache for user: $UserProfile"
                
                # Calculate cache size
                $CacheFiles = Get-ChildItem $UserCachePath -Recurse -Force -ErrorAction SilentlyContinue
                $UserCacheSize = ($CacheFiles | Measure-Object -Property Length -Sum -ErrorAction SilentlyContinue).Sum
                $UserCacheSizeMB = [math]::Round($UserCacheSize / 1MB, 2)
                
                $TotalCacheSize += $UserCacheSizeMB
                
                $CacheDetails[$UserProfile] = @{
                    Path = $UserCachePath
                    SizeMB = $UserCacheSizeMB
                    FileCount = $CacheFiles.Count
                    LastModified = if ($CacheFiles) { ($CacheFiles | Sort-Object LastWriteTime -Descending | Select-Object -First 1).LastWriteTime } else { $null }
                }
                
                # Check for excessive cache size
                if ($UserCacheSizeMB -gt $MaxCacheSizeMB) {
                    $CacheIssues += "User $UserProfile cache size exceeds threshold: $UserCacheSizeMB MB (Limit: $MaxCacheSizeMB MB)"
                    Write-LogEntry "Cache size issue detected for $UserProfile : $UserCacheSizeMB MB" -Level "Warning"
                }
                
                # Check for very old cache files
                $OldCacheFiles = $CacheFiles | Where-Object { 
                    <#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -lt (Get-Date).AddDays(-30) 
                }
                if ($OldCacheFiles.Count -gt 0) {
                    $CacheIssues += "User $UserProfile has $($OldCacheFiles.Count) cache files older than 30 days"
                }
                
                # Check for corrupted cache indicators
                $CorruptedFiles = $CacheFiles | Where-Object { 
                    <#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Length -eq 0 -or (<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "tmp|temp" -and <#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -lt (Get-Date).AddDays(-7))
                }
                if ($CorruptedFiles.Count -gt 0) {
                    $CacheIssues += "User $UserProfile has $($CorruptedFiles.Count) potentially corrupted cache files"
                }
                
                Write-LogEntry "User $UserProfile cache analysis: $UserCacheSizeMB MB, $($CacheFiles.Count) files"
            }
            else {
                Write-LogEntry "Cache path not found for user: $UserProfile" -Level "Warning"
            }
        }
        
        $DetectionResult.CacheDetails = $CacheDetails
        $DetectionResult.CacheDetails["TotalSizeMB"] = $TotalCacheSize
        $DetectionResult.CacheDetails["MaxAllowedMB"] = $MaxCacheSizeMB
        
        if ($CacheIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $CacheIssues
            Write-LogEntry "Found $($CacheIssues.Count) cache size/structure issues" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "Cache size analysis completed - no issues found (Total: $TotalCacheSize MB)"
        return $false
    }
    catch {
        Write-LogEntry "Error analyzing Chrome cache size: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
function Test-ChromePerformanceIssues {
    Write-LogEntry "Checking for Chrome performance issues"
    ...
    try {
        ...
        # Check for Chrome crash events
        $CrashEvents = Get-WinEvent -FilterHashtable @{
            LogName = 'Application'
            ID = 1000, 1001
            StartTime = (Get-Date).AddDays(-$PerformanceThresholdDays)
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "chrome\.exe|Google Chrome" -and <#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.LevelDisplayName -eq "Error"
        ...
        $PerformanceMetrics.CrashEvents = $CrashEvents.Count
        if ($CrashEvents.Count -gt 5) {
            $PerformanceIssues += "High number of Chrome crashes detected: $($CrashEvents.Count) events in $PerformanceThresholdDays days"
        }
        
        # Check current Chrome memory usage
        $ChromeProcesses = Get-Process -Name "chrome" -ErrorAction SilentlyContinue
        if ($ChromeProcesses) {
            $TotalMemory = ($ChromeProcesses | Measure-Object -Property WorkingSet64 -Sum).Sum / 1MB
            $PerformanceMetrics.MemoryUsage = [math]::Round($TotalMemory, 2)
            $PerformanceMetrics.ProcessCount = $ChromeProcesses.Count
            
            Write-LogEntry "Current Chrome memory usage: $($PerformanceMetrics.MemoryUsage) MB ($($ChromeProcesses.Count) processes)"
            
            # Check for excessive memory usage
            if ($PerformanceMetrics.MemoryUsage -gt 2048) {
                $PerformanceIssues += "High Chrome memory usage detected: $($PerformanceMetrics.MemoryUsage) MB"
            }
            
            # Check for excessive process count
            if ($PerformanceMetrics.ProcessCount -gt 20) {
                $PerformanceIssues += "High Chrome process count detected: $($PerformanceMetrics.ProcessCount) processes"
            }
        }
        
        # Check for Chrome hanging events
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "chrome\.exe|Google Chrome"
        }
        
        if ($HangEvents.Count -gt 3) {
            $PerformanceIssues += "Chrome hanging events detected: $($HangEvents.Count) events in $PerformanceThresholdDays days"
        }
        
        # Check for slow startup indicators in registry
        $StartupRegPath = "HKCU:\SOFTWARE\Google\Chrome\PreferenceMACs\Default"
        if (Test-Path $StartupRegPath) {
            try {
                $StartupData = Get-ItemProperty $StartupRegPath -ErrorAction SilentlyContinue
                # This is a simplified check - in real scenarios you'd parse JSON preferences
                if ($StartupData) {
                    Write-LogEntry "Chrome startup configuration detected"
                }
            }
            catch {
                Write-LogEntry "Could not read Chrome startup configuration" -Level "Warning"
            }
        }
        
        $DetectionResult.PerformanceMetrics = $PerformanceMetrics
        
        if ($PerformanceIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $PerformanceIssues
            Write-LogEntry "Found $($PerformanceIssues.Count) performance issues" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "Performance analysis completed - no issues found"
        return $false
    }
    catch {
        Write-LogEntry "Error checking Chrome performance: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
function Test-ChromeUserReportedIssues {
    Write-LogEntry "Checking for user-reported Chrome issues"
    
    try {
        $UserIssues = @()
        
        # Check for Chrome error reporting files
        foreach ($UserProfile in $CheckUserProfiles) {
            $CrashReportsPath = if ($UserProfile -eq $env:USERNAME) {
                "$env:LOCALAPPDATA\Google\Chrome\User Data\Crashpad\reports"
            }
            else {
                "C:\Users\$UserProfile\AppData\Local\Google\Chrome\User Data\Crashpad\reports"
            }
            
            if (Test-Path $CrashReportsPath) {
                $CrashReports = Get-ChildItem $CrashReportsPath -Filter "*.dmp" -ErrorAction SilentlyContinue | Where-Object {
                    <#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -gt (Get-Date).AddDays(-$PerformanceThresholdDays)
                }
                
                if ($CrashReports.Count -gt 0) {
                    $UserIssues += "User $UserProfile has $($CrashReports.Count) recent crash reports"
                    Write-LogEntry "Found $($CrashReports.Count) crash reports for user $UserProfile" -Level "Warning"
                }
            }
            
            # Check for Chrome extension issues that might indicate cache problems
            $ExtensionsPath = if ($UserProfile -eq $env:USERNAME) {
                "$env:LOCALAPPDATA\Google\Chrome\User Data\Default\Extensions"
            }
            else {
                "C:\Users\$UserProfile\AppData\Local\Google\Chrome\User Data\Default\Extensions"
            }
            
            if (Test-Path $ExtensionsPath) {
                $Extensions = Get-ChildItem $ExtensionsPath -Directory -ErrorAction SilentlyContinue
                if ($Extensions.Count -gt 50) {
                    $UserIssues += "User $UserProfile has excessive Chrome extensions ($($Extensions.Count)) which may impact cache performance"
                }
            }
        }
        
        # Check for Chrome stability issues in system events
        $StabilityEvents = Get-WinEvent -FilterHashtable @{
            LogName = 'System'
            ID = 6008, 1074
            StartTime = (Get-Date).AddDays(-$PerformanceThresholdDays)
        } -ErrorAction SilentlyContinue | Where-Object {
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "chrome|browser"
        }
        
        if ($StabilityEvents.Count -gt 2) {
            $UserIssues += "System stability events involving Chrome detected: $($StabilityEvents.Count) events"
        }
        
        # Check for excessive Chrome history that might indicate cache correlation
        foreach ($UserProfile in $CheckUserProfiles) {
            $HistoryPath = if ($UserProfile -eq $env:USERNAME) {
            ...
            if (Test-Path $HistoryPath) {
                $HistorySize = (Get-Item $HistoryPath).Length / 1MB
                if ($HistorySize -gt 100) {
                    $UserIssues += "User $UserProfile has large Chrome history file ($([math]::Round($HistorySize, 2)) MB) which may correlate with cache issues"
                }
            }
        }
        
        if ($UserIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $UserIssues
            Write-LogEntry "Found $($UserIssues.Count) user-reported or system-indicated issues" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "User issue check completed - no issues found"
        return $false
    }
    catch {
        Write-LogEntry "Error checking user-reported issues: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
function Test-ChromeCacheCorruption {
    Write-LogEntry "Checking for Chrome cache corruption indicators"
    
    try {
        $CorruptionIssues = @()
        
        foreach ($UserProfile in $CheckUserProfiles) {
            $CachePath = if ($UserProfile -eq $env:USERNAME) {
                "$env:LOCALAPPDATA\Google\Chrome\User Data\Default\Cache"
            }
            else {
                "C:\Users\$UserProfile\AppData\Local\Google\Chrome\User Data\Default\Cache"
            ...
            if (Test-Path $CachePath) {
                # Check for cache index corruption
                $IndexFile = Join-Path $CachePath "index"
                if (Test-Path $IndexFile) {
                    $IndexSize = (Get-Item $IndexFile).Length
                    if ($IndexSize -eq 0) {
                        $CorruptionIssues += "User $UserProfile has corrupted cache index file (0 bytes)"
                    }
                }
                
                # Check for excessive lock files or temp files
                $LockFiles = Get-ChildItem $CachePath -Filter "*.lock" -ErrorAction SilentlyContinue
                $TempFiles = Get-ChildItem $CachePath -Filter "*.tmp" -ErrorAction SilentlyContinue | Where-Object {
                    <#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -lt (Get-Date).AddHours(-24)
                }
                
                if ($LockFiles.Count -gt 0) {
                    $CorruptionIssues += "User $UserProfile has $($LockFiles.Count) cache lock files indicating potential corruption"
                }
                
                if ($TempFiles.Count -gt 10) {
                    $CorruptionIssues += "User $UserProfile has $($TempFiles.Count) stale temporary cache files"
                }
                
                # Check for cache data structure issues
                $DataFolders = Get-ChildItem $CachePath -Directory -Name "data_*" -ErrorAction SilentlyContinue
                if ($DataFolders.Count -eq 0) {
                    $CorruptionIssues += "User $UserProfile cache is missing expected data folders structure"
                }
            }
        }
        
        if ($CorruptionIssues.Count -gt 0) {
            $DetectionResult.IssuesFound += $CorruptionIssues
            Write-LogEntry "Found $($CorruptionIssues.Count) cache corruption indicators" -Level "Warning"
            return $true
        }
        
        Write-LogEntry "Cache corruption check completed - no issues found"
        return $false
    }
    catch {
        Write-LogEntry "Error checking cache corruption: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
Write-LogEntry "Starting Chrome cache detection"

try {
    # Verify Chrome is installed
    if (-not (Test-ChromeInstallation)) {
        Write-LogEntry "Google Chrome not detected - skipping cache detection" -Level "Warning"
        $DetectionResult.CacheDetails["SkipReason"] = "Chrome not installed"
        ...
        if (Test-ChromeCacheSize) { $IssueDetected = $true }
        if (Test-ChromePerformanceIssues) { $IssueDetected = $true }
        if (Test-ChromeUserReportedIssues) { $IssueDetected = $true }
        if (Test-ChromeCacheCorruption) { $IssueDetected = $true }
        
        # Determine final result
        if ($IssueDetected) {
            $DetectionResult.ActionRequired = $true
            $DetectionResult.ExitCode = 1
            $DetectionResult.Recommendations += "Clear Chrome cache to resolve performance and corruption issues"
            $DetectionResult.Recommendations += "Close Chrome before cache clearing operation"
            $DetectionResult.Recommendations += "Monitor Chrome performance after cache clearing"
            
            Write-LogEntry "Chrome cache issues detected - clearing recommended" -Level "Warning"
        }
        else {
            $DetectionResult.ActionRequired = $false
            $DetectionResult.ExitCode = 0
            Write-LogEntry "No Chrome cache issues detected - cache appears healthy"
        }
    }
}
catch {
    Write-LogEntry "Critical error during detection: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Level "Error"
    ...
    $DetectionResult.IssuesFound += "Detection script encountered critical error: $(<#
.SCRIPT NAME
		Detect_Browser_Chrome_ClearCache.ps1
...
# Summary logging
Write-LogEntry "Detection completed. Action Required: $($DetectionResult.ActionRequired). Issues Found: $($DetectionResult.IssuesFound.Count). Exit Code: $($DetectionResult.ExitCode)"
