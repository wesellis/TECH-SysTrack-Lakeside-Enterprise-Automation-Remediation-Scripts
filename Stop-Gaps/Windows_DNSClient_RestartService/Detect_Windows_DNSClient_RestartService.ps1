...
try {
    ...
    [Parameter(Mandatory=$false)]
    [string]$LogSource = "SysTrack_DNSClient_Detection"
)
...
function Test-DNSClientServiceStatus {
    try {
        Write-DetectionLog "Testing DNS Client service status and health"
        ...
        if ($serviceHealth.Status -ne "Running") {
            $script:IssuesFound += "DNS Client service is not running (Status: $($serviceHealth.Status))"
            $script:HealthScore -= 30
            Write-DetectionLog "ISSUE: DNS Client service is not running" "Warning"
        }
        ...
        if ($serviceHealth.Status -eq "Running" -and -not $serviceHealth.CanStop) {
            $script:IssuesFound += "DNS Client service cannot be stopped, indicating potential stability issues"
            $script:HealthScore -= 20
            Write-DetectionLog "ISSUE: DNS Client service cannot be stopped" "Warning"
        }
        
        # Check if start type is not automatic
        if ($serviceHealth.StartType -ne "Auto") {
            $script:IssuesFound += "DNS Client service start type is not set to Automatic (Current: $($serviceHealth.StartType))"
            $script:HealthScore -= 15
            Write-DetectionLog "ISSUE: DNS Client service start type not Automatic" "Warning"
        }
        
        return $serviceHealth
    }
    catch {
        $errorMsg = "Failed to retrieve DNS Client service information: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
function Test-DNSClientServiceDependencies {
    try {
        Write-DetectionLog "Testing DNS Client service dependencies"
        ...
        $errorMsg = "Failed to check DNS Client service dependencies: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
function Test-DNSClientServicePerformance {
    try {
        Write-DetectionLog "Testing DNS Client service performance metrics"
        ...
        if ($service.Status -eq "Running" -and $service.ProcessId -gt 0) {
            ...
            if ($process) {
                ...
                # Check for excessive memory usage (over 100MB for DNS Client is unusual)
                if ($memoryUsage -gt 100) {
                    $script:IssuesFound += "DNS Client service using excessive memory: $memoryUsage MB"
                    $script:HealthScore -= 15
                    Write-DetectionLog "ISSUE: Excessive memory usage: $memoryUsage MB" "Warning"
                }
                
                # Check for excessive handle count (over 1000 could indicate issues)
                if ($handleCount -gt 1000) {
                    $script:IssuesFound += "DNS Client service has excessive handle count: $handleCount"
                    $script:HealthScore -= 10
        ...
        $errorMsg = "Failed to check DNS Client service performance: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
function Test-DNSClientEventLogErrors {
    try {
        Write-DetectionLog "Checking DNS Client service event log errors"
        
        $errorEvents = @()
        $warningEvents = @()
        
        # Check System event log for DNS Client service errors
        ...
        $totalErrors = $errorEvents.Count + $dnsEvents.Count
        $totalWarnings = $warningEvents.Count
        
        Write-DetectionLog "Found $totalErrors DNS Client errors in last 24 hours"
        Write-DetectionLog "Found $totalWarnings DNS Client warnings in last 24 hours"
        
        # Evaluate error severity
        if ($totalErrors -gt 10) {
            $script:IssuesFound += "High number of DNS Client errors in event log: $totalErrors errors"
            $script:HealthScore -= 25
            Write-DetectionLog "ISSUE: High number of DNS Client errors: $totalErrors" "Warning"
        }
        elseif ($totalErrors -gt 5) {
            $script:IssuesFound += "Moderate number of DNS Client errors in event log: $totalErrors errors"
            $script:HealthScore -= 15
            Write-DetectionLog "ISSUE: Moderate DNS Client errors: $totalErrors" "Warning"
        }
        elseif ($totalErrors -gt 0) {
            $script:IssuesFound += "Some DNS Client errors found in event log: $totalErrors errors"
            $script:HealthScore -= 5
            Write-DetectionLog "ISSUE: Some DNS Client errors found: $totalErrors" "Warning"
        }
        
        if ($totalWarnings -gt 20) {
            $script:IssuesFound += "High number of DNS Client warnings: $totalWarnings warnings"
            $script:HealthScore -= 10
            Write-DetectionLog "ISSUE: High number of DNS Client warnings: $totalWarnings" "Warning"
        }
        ...
        $errorMsg = "Failed to check DNS Client event log errors: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
function Test-DNSClientServiceUptime {
    try {
        Write-DetectionLog "Checking DNS Client service uptime and restart history"
        ...
        if ($service.Status -eq "Running" -and $service.ProcessId -gt 0) {
            ...
            if ($process) {
                ...
                Write-DetectionLog "DNS Client service uptime: $uptimeHours hours"
                
                # Check if service has been restarted recently (less than 1 hour could indicate issues)
                if ($uptimeHours -lt 1) {
                    $script:IssuesFound += "DNS Client service was recently restarted (uptime: $uptimeHours hours)"
                    $script:HealthScore -= 10
                    Write-DetectionLog "ISSUE: Recent service restart detected" "Warning"
                }
                
                # Check for very short uptime indicating frequent restarts
                if ($uptimeHours -lt 0.25) {  # Less than 15 minutes
                    $script:IssuesFound += "DNS Client service has very short uptime, indicating potential instability"
                    $script:HealthScore -= 20
        ...
        $errorMsg = "Failed to check DNS Client service uptime: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
function Test-DNSResolutionFunctionality {
    try {
        Write-DetectionLog "Testing DNS resolution functionality"
        ...
        foreach ($domain in $testDomains) {
            try {
                ...
                Write-DetectionLog "Resolved $domain in $resolutionTime ms"
                
                # Check for slow resolution (over 2 seconds)
                if ($resolutionTime -gt 2000) {
                    $resolutionIssues++
                    Write-DetectionLog "Slow DNS resolution for $domain : $resolutionTime ms" "Warning"
                }
            }
            catch {
                $resolutionIssues++
                Write-DetectionLog "Failed to resolve $domain : $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        Write-DetectionLog "Average DNS resolution time: $averageResolutionTime ms"
        Write-DetectionLog "DNS resolution issues: $resolutionIssues out of $($testDomains.Count)"
        
        # Evaluate DNS resolution performance
        if ($resolutionIssues -eq $testDomains.Count) {
            $script:IssuesFound += "Complete DNS resolution failure - all test domains failed"
            $script:HealthScore -= 30
            Write-DetectionLog "ISSUE: Complete DNS resolution failure" "Error"
        }
        elseif ($resolutionIssues -gt 0) {
            $script:IssuesFound += "Partial DNS resolution issues: $resolutionIssues out of $($testDomains.Count) domains failed"
            $script:HealthScore -= 15
            Write-DetectionLog "ISSUE: Partial DNS resolution failure" "Warning"
        }
        
        if ($averageResolutionTime -gt 1000) {
            $script:IssuesFound += "Slow DNS resolution performance: average $averageResolutionTime ms"
            $script:HealthScore -= 10
            Write-DetectionLog "ISSUE: Slow DNS resolution performance" "Warning"
        }
        
        return @{
            ResolutionIssues = $resolutionIssues
            AverageResolutionTime = $averageResolutionTime
            TestedDomains = $testDomains.Count
        }
    }
    catch {
        $errorMsg = "Failed to test DNS resolution functionality: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
try {
    Write-DetectionLog "Starting DNS Client service detection"
    Initialize-EventLogSource
    
    # Perform all detection tests
    $serviceStatus = Test-DNSClientServiceStatus
    $serviceDependencies = Test-DNSClientServiceDependencies
    $servicePerformance = Test-DNSClientServicePerformance
    $eventLogErrors = Test-DNSClientEventLogErrors
    $serviceUptime = Test-DNSClientServiceUptime
    $dnsResolution = Test-DNSResolutionFunctionality
    
    # Calculate final health score
    $script:HealthScore = [math]::Max(0, $script:HealthScore)
    
    # Determine if remediation is needed
    $remediationNeeded = $script:IssuesFound.Count -gt 0 -or $script:HealthScore -lt 80
    ...
    if ($remediationNeeded) {
        $script:ExitCode = 1
        Write-DetectionLog "DNS Client service restart recommended - Health Score: $($script:HealthScore)%" "Warning"
        Write-DetectionLog "Issues found: $($script:IssuesFound.Count)" "Warning"
    }
    else {
        $script:ExitCode = 0
        Write-DetectionLog "DNS Client service is healthy - Health Score: $($script:HealthScore)%" "Information"
    }
    ...
    Write-DetectionLog "DNS Client service detection completed with exit code: $($script:ExitCode)"
}
catch {
    $script:ExitCode = 1
    $errorMsg = "Critical error during DNS Client service detection: $(<#
.SCRIPT NAME
		Detect_Windows_DNSClient_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
    ...
    Write-DetectionLog "DNS Client service detection script completed"
