...
.DESCRIPTION 
		Microsoft Teams cache and configuration management. Detection and health checking.
...
function Test-TeamsCacheHealth {
    $issues = @()
    $severity = "Low"
    $cacheMetrics = @{}
    ...
    try {
        ...
        $cachePaths = @{
            ...
            "Logs" = "$env:LOCALAPPDATA\Microsoft\Teams\logs.txt"
            "GPUCache" = "$env:LOCALAPPDATA\Microsoft\Teams\GPUCache"
            "IndexedDB" = "$env:LOCALAPPDATA\Microsoft\Teams\IndexedDB"
            "Backgrounds" = "$env:LOCALAPPDATA\Microsoft\Teams\Backgrounds\Uploads"
        }
        ...
        foreach ($cacheType in $cachePaths.GetEnumerator()) {
            if ($cacheType.Key -eq "TempFiles") {
                $tempPaths = Get-ChildItem "$env:TEMP" -Directory -Filter "teams-for-*" -ErrorAction SilentlyContinue
                foreach ($tempPath in $tempPaths) {
                    $size = (Get-ChildItem $tempPath.FullName -Recurse -File -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum
                    if ($size) {
                        $totalCacheSize += $size
                        $cacheMetrics["TempFiles"] = $size
                    }
                }
            } elseif (Test-Path $cacheType.Value) {
                if ($cacheType.Key -eq "Logs" -and (Test-Path $cacheType.Value)) {
                    $size = (Get-Item $cacheType.Value -ErrorAction SilentlyContinue).Length
                } else {
                ...
                if ($size) {
                    $totalCacheSize += $size
                    $cacheMetrics[$cacheType.Key] = $size
                }
            }
        }
        ...
        $cacheMetrics["TotalSizeMB"] = $totalCacheMB
        
        # Check for excessive cache size (over 500MB indicates cleanup needed)
        if ($totalCacheMB -gt 500) {
            $issues += "Teams cache size excessive: $totalCacheMB MB"
            $severity = "High"
        } elseif ($totalCacheMB -gt 200) {
            $issues += "Teams cache size large: $totalCacheMB MB"
            if ($severity -eq "Low") { $severity = "Medium" }
        }
        
        # Check for old cache files (over 30 days)
        $oldCacheThreshold = (Get-Date).AddDays(-30)
        $oldCacheFiles = 0
        
        foreach ($cacheType in $cachePaths.GetEnumerator()) {
            if ($cacheType.Key -ne "Logs" -and (Test-Path $cacheType.Value)) {
                $oldFiles = Get-ChildItem $cacheType.Value -Recurse -File -ErrorAction SilentlyContinue | 
                           Where-Object {<#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -lt $oldCacheThreshold}
                $oldCacheFiles += $oldFiles.Count
            }
        }
        
        if ($oldCacheFiles -gt 100) {
            $issues += "Many old cache files detected: $oldCacheFiles files older than 30 days"
            if ($severity -ne "High") { $severity = "Medium" }
        }
        
        # Check Teams process performance indicators
        $teamsProcess = Get-Process "Teams" -ErrorAction SilentlyContinue
        if ($teamsProcess) {
            # Check memory usage
            $maxMemoryMB = ($teamsProcess | Measure-Object WorkingSet -Maximum).Maximum / 1MB
            $cacheMetrics["MaxMemoryUsageMB"] = [math]::Round($maxMemoryMB, 2)
            
            if ($maxMemoryMB -gt 500) {
                $issues += "Teams high memory usage: $([math]::Round($maxMemoryMB, 2)) MB"
                if ($severity -eq "Low") { $severity = "Medium" }
            }
            
            # Check for multiple Teams processes (can indicate cache issues)
            if ($teamsProcess.Count -gt 5) {
                $issues += "Multiple Teams processes running: $($teamsProcess.Count)"
                if ($severity -eq "Low") { $severity = "Medium" }
            }
            
            # Check if any Teams processes are not responding
            $unresponsiveProcesses = $teamsProcess | Where-Object {-not <#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Responding}
            if ($unresponsiveProcesses) {
                $issues += "Teams processes not responding: $($unresponsiveProcesses.Count)"
                $severity = "High"
            }
        }
        
        # Check for corrupt cache indicators
        $corruptCacheIndicators = @()
        
        # Check for Teams error logs
        $logsPath = "$env:LOCALAPPDATA\Microsoft\Teams\logs.txt"
        if (Test-Path $logsPath) {
            $recentLogs = Get-Content $logsPath -Tail 500 -ErrorAction SilentlyContinue | 
                         Where-Object {<#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "$(Get-Date -Format 'yyyy-MM-dd')|$((Get-Date).AddDays(-1).ToString('yyyy-MM-dd'))"}
            ...
            foreach ($pattern in $errorPatterns) {
                $matches = $recentLogs | Where-Object {<#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match $pattern}
                if ($matches -and $matches.Count -gt 0) {
                    $corruptCacheIndicators += "Cache errors in logs: $($matches.Count) entries"
                    break
                }
            }
        }
        ...
        if (Test-Path $indexedDBPath) {
            ...
            if ($lockFiles -and $lockFiles.Count -gt 0) {
                $corruptCacheIndicators += "IndexedDB lock files found: $($lockFiles.Count)"
            }
        }
        
        if ($corruptCacheIndicators.Count -gt 0) {
            $issues += $corruptCacheIndicators
            $severity = "High"
        }
        
        # Check Windows Event Log for Teams crashes
        $yesterday = (Get-Date).AddDays(-1)
        try {
            $teamsCrashes = Get-WinEvent -FilterHashtable @{LogName='Application'; StartTime=$yesterday; Level=2} -ErrorAction SilentlyContinue | 
                           Where-Object {<#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.ProviderName -match "Teams|Microsoft Teams"}
            if ($teamsCrashes -and $teamsCrashes.Count -gt 2) {
                $issues += "Teams application crashes detected: $($teamsCrashes.Count) in last 24 hours"
                if ($severity -eq "Low") { $severity = "Medium" }
            }
        } catch {
        ...
        # Check for Teams startup performance issues
        try {
            $teamsStartupReg = "HKCU:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run"
            if (Test-Path $teamsStartupReg) {
                $teamsStartup = Get-ItemProperty $teamsStartupReg -Name "com.squirrel.Teams.Teams" -ErrorAction SilentlyContinue
                if ($teamsStartup -and $totalCacheMB -gt 300) {
                    $issues += "Large cache may impact Teams startup performance"
                    if ($severity -eq "Low") { $severity = "Medium" }
                }
            }
        } catch {
        ...
        $issues += "Error during Teams cache health check: $(<#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
    ...
    $requiresCleanup = ($totalCacheMB -gt 200) -or 
                      ($oldCacheFiles -gt 50) -or 
                      ($corruptCacheIndicators.Count -gt 0) -or
                      ($issues | Where-Object {<#
.SCRIPT NAME
		Detect_M365_Teams_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "not responding|crash|corrupt|error"})
...
# Execute detection
$detection = Test-TeamsCacheHealth

# Output for SysTrack
Write-Output "M365_TEAMS_CACHE_CLEANUP_NEEDED: $($detection.RequiresCleanup)"
Write-Output "ISSUE_COUNT: $($detection.IssueCount)"
Write-Output "SEVERITY: $($detection.Severity)"
Write-Output "TOTAL_CACHE_SIZE_MB: $($detection.CacheMetrics.TotalSizeMB)"

if ($detection.CacheMetrics.Count -gt 0) {
    Write-Output "CACHE_BREAKDOWN:"
    foreach ($metric in $detection.CacheMetrics.GetEnumerator()) {
        if ($metric.Key -ne "TotalSizeMB") {
            $sizeMB = [math]::Round($metric.Value / 1MB, 2)
            Write-Output "  $($metric.Key): $sizeMB MB"
        }
    }
}
...
if ($detection.IssuesFound) {
    ...
    if ($detection.RequiresCleanup) {
        Write-Output "Teams cache cleanup recommended"
        exit 1  # Indicates remediation needed
    } else {
        Write-Output "Teams issues detected but cleanup not required"
        exit 0
    }
} else {
    Write-Output "No Teams cache issues detected"
