...
.DESCRIPTION 
		Windows DNS client and cache management. Detection and health checking.
...
try {
    ...
    [string]$LogSource = "SysTrack_WindowsDNS_Detection",
...
$ScriptName = "Detect_Windows_DNS_FixCache"
...
# Function to check DNS client service status
function Test-DNSClientService {
    try {
        $DNSClientInfo = @{
        ...
        $DNSClientService = Get-Service -Name "Dnscache" -ErrorAction Stop
        $DNSClientInfo.ServiceStatus = $DNSClientService.Status
        $DNSClientInfo.ServiceStartType = $DNSClientService.StartType
        $DNSClientInfo.ServiceRunning = ($DNSClientService.Status -eq 'Running')
        
        if ($DNSClientService.Status -eq 'Running') {
            try {
                $ServiceWMI = Get-WmiObject -Class Win32_Service -Filter "Name='Dnscache'" -ErrorAction Stop
                $DNSClientInfo.ProcessId = $ServiceWMI.ProcessId
            }
            catch {
                Write-LogEntry "Could not get DNS Client process information: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
            ...
            $DNSClientInfo.Issues += "DNS Client service is $($DNSClientService.Status) instead of Running"
        }
        
        if ($DNSClientService.StartType -eq 'Disabled') {
            $DNSClientInfo.Issues += "DNS Client service is disabled"
        }
        
        return $DNSClientInfo
    }
    catch {
        Write-LogEntry "Failed to check DNS Client service: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
    ...
    try {
        ...
        foreach ($Domain in $Domains) {
            ...
            try {
                ...
                $ResolutionTest.Success = $true
                $ResolutionTest.ResponseTime = ($EndTime - $StartTime).TotalMilliseconds
                $ResolutionTest.IPAddresses = $DNSResult | Where-Object { <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.IPAddress }
                ...
                $ResolutionResults.SuccessfulTests++
                
                # Check for slow resolution (over 2 seconds)
                if ($ResolutionTest.ResponseTime -gt 2000) {
                    $ResolutionResults.SlowResolutions += $Domain
                }
            }
            catch {
                $ResolutionTest.ErrorMessage = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message
                $ResolutionResults.FailedResolutions += $Domain
            }
            
            $ResolutionResults.TestedDomains += $ResolutionTest
            $ResolutionResults.TotalTests++
        }
        
        # Calculate average response time for successful resolutions
        $SuccessfulResolutions = $ResolutionResults.TestedDomains | Where-Object { <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Success }
        if ($SuccessfulResolutions.Count -gt 0) {
            $ResolutionResults.AverageResponseTime = ($SuccessfulResolutions | Measure-Object -Property ResponseTime -Average).Average
        }
        
        # Analyze results for issues
        $FailureRate = ($ResolutionResults.FailedResolutions.Count / $ResolutionResults.TotalTests) * 100
        
        if ($FailureRate -gt 20) {
            $ResolutionResults.Issues += "High DNS resolution failure rate: $([math]::Round($FailureRate, 1))%"
        }
        
        if ($ResolutionResults.SlowResolutions.Count -gt 0) {
            $ResolutionResults.Issues += "Slow DNS resolution detected for $($ResolutionResults.SlowResolutions.Count) domains"
        }
        
        if ($ResolutionResults.AverageResponseTime -gt 1000) {
            $ResolutionResults.Issues += "Average DNS response time is slow: $([math]::Round($ResolutionResults.AverageResponseTime, 0))ms"
        }
        
        return $ResolutionResults
    }
    catch {
        Write-LogEntry "Failed to test DNS resolution: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
# Function to check DNS cache size and content
function Test-DNSCacheStatus {
    try {
        $CacheStatus = @{
            CacheEntries = 0
            CacheSize = 0
        ...
        try {
            ...
            $CacheStatus.CacheEntries = $CacheEntries.Count
...
ExitScript -msg "Script Completed" -code $returnCode.Data -eq $null }
                $CacheStatus.NegativeCacheEntries = $NegativeEntries.Count
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.TimeToLive -eq 0 -or (<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.TimeToLive -lt 60 -and <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.TimeToLive -gt 0) }
                $CacheStatus.ExpiredEntries = $ExpiredEntries.Count
                
                # Get sample entries for analysis
                $CacheStatus.SampleEntries = $CacheEntries | Select-Object -First 5 | ForEach-Object {
                    @{
                        Entry = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Entry
                        Type = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Type
                        Status = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Status
                        TimeToLive = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.TimeToLive
                        Data = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Data
                    }
                }
                
                # Calculate approximate cache size
                $CacheStatus.CacheSize = [math]::Round(($CacheEntries.Count * 100) / 1024, 2)  # Rough estimate in KB
            }
            
            # Analyze cache health
            if ($CacheStatus.CacheEntries -eq 0) {
                $CacheStatus.Issues += "DNS cache is empty - possible service issue"
            }
            elseif ($CacheStatus.CacheEntries -gt 5000) {
                $CacheStatus.Issues += "DNS cache is very large ($($CacheStatus.CacheEntries) entries) - may need flushing"
            }
            
            $NegativeRate = if ($CacheStatus.CacheEntries -gt 0) { ($CacheStatus.NegativeCacheEntries / $CacheStatus.CacheEntries) * 100 } else { 0 }
            if ($NegativeRate -gt 30) {
                $CacheStatus.Issues += "High negative cache entry rate: $([math]::Round($NegativeRate, 1))%"
            }
            
            if ($CacheStatus.ExpiredEntries -gt ($CacheStatus.CacheEntries * 0.5)) {
                $CacheStatus.Issues += "High number of expired cache entries: $($CacheStatus.ExpiredEntries)"
            }
        }
        catch {
            $CacheStatus.Issues += "Could not retrieve DNS cache information: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
            Write-LogEntry "Error retrieving DNS cache: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        Write-LogEntry "Failed to check DNS cache status: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
function Test-DNSConfiguration {
    try {
        ...
        # Get network adapter DNS configuration
        try {
...
ExitScript -msg "Script Completed" -code $returnCode.Virtual -eq $false }
            ...
            foreach ($Adapter in $NetworkAdapters) {
                ...
                try {
                    $DNSClientServerAddress = Get-DnsClientServerAddress -InterfaceIndex $Adapter.InterfaceIndex -AddressFamily IPv4 -ErrorAction Stop
                    $AdapterDNS.DNSServers = $DNSClientServerAddress.ServerAddresses
                    
                    # Check DHCP status
                    $NetIPInterface = Get-NetIPInterface -InterfaceIndex $Adapter.InterfaceIndex -AddressFamily IPv4 -ErrorAction SilentlyContinue
                    if ($NetIPInterface) {
                        $AdapterDNS.DHCPEnabled = $NetIPInterface.Dhcp -eq 'Enabled'
                    }
                    
                    # Validate DNS servers
                    if ($AdapterDNS.DNSServers.Count -eq 0) {
                        $AdapterDNS.Issues += "No DNS servers configured"
                    }
                    else {
                        $DNSConfig.ConfiguredAdapters++
                        $DNSConfig.DNSServers += $AdapterDNS.DNSServers
                        
                        # Test DNS server reachability
                        foreach ($DNSServer in $AdapterDNS.DNSServers) {
                            try {
                                $PingResult = Test-NetConnection -ComputerName $DNSServer -Port 53 -WarningAction SilentlyContinue -ErrorAction Stop
                                if (-not $PingResult.TcpTestSucceeded) {
                                    $AdapterDNS.Issues += "DNS server $DNSServer is not reachable on port 53"
                                }
                            }
                            catch {
                                $AdapterDNS.Issues += "Cannot test connectivity to DNS server $DNSServer"
                            }
                        }
                    }
                }
                catch {
                    $AdapterDNS.Issues += "Could not retrieve DNS configuration: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
                ...
                $DNSConfig.NetworkAdapters += $AdapterDNS
            }
            
            # Overall configuration analysis
            if ($DNSConfig.ConfiguredAdapters -eq 0) {
                $DNSConfig.Issues += "No network adapters have DNS servers configured"
            }
            
            $UniqueDNSServers = $DNSConfig.DNSServers | Sort-Object -Unique
            if ($UniqueDNSServers.Count -eq 1) {
                $DNSConfig.Issues += "Only one unique DNS server configured - consider adding backup"
            }
        }
        catch {
            $DNSConfig.Issues += "Error analyzing DNS configuration: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
            Write-LogEntry "Error checking DNS configuration: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        Write-LogEntry "Failed to check DNS configuration: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
# Function to check DNS-related event log errors
function Test-DNSEventLogErrors {
    try {
        $EventLogErrors = @{
            RecentErrors = @()
            CriticalErrors = @()
        ...
        $CheckStartTime = (Get-Date).AddHours(-24)
        
        # Check DNS Client events
        try {
            $DNSClientErrors = Get-WinEvent -FilterHashtable @{
            ...
            if ($DNSClientErrors) {
                $EventLogErrors.RecentErrors += $DNSClientErrors | ForEach-Object {
                    @{
                        TimeCreated = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Message.Length))
                ...
                # Identify critical DNS errors
                $CriticalDNSErrors = $DNSClientErrors | Where-Object { <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Id -in @(1014, 4004, 4013, 4015) }
                $EventLogErrors.CriticalErrors += $CriticalDNSErrors
            }
        }
        catch {
            Write-LogEntry "Could not check DNS Client event log: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        # Check System log for DNS-related errors
        try {
            $SystemDNSErrors = Get-WinEvent -FilterHashtable @{
                LogName = 'System'
                Level = 2  # Error level
                StartTime = $CheckStartTime
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -like "*DNS*" -or <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -like "*Dnscache*" -or <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.ProviderName -like "*DNS*"
            }
            
            if ($SystemDNSErrors) {
                $EventLogErrors.RecentErrors += $SystemDNSErrors | ForEach-Object {
                    @{
                        TimeCreated = <#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Message.Length))
            ...
            Write-LogEntry "Could not check System log for DNS errors: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        $EventLogErrors.ErrorCount = $EventLogErrors.RecentErrors.Count
        $EventLogErrors.CriticalErrorCount = $EventLogErrors.CriticalErrors.Count
        
        return $EventLogErrors
    }
    catch {
        Write-LogEntry "Failed to check DNS event log errors: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
...
Write-LogEntry "Starting DNS cache fix detection"

try {
    # Check DNS Client service status
    Write-LogEntry "Checking DNS Client service status"
    $DNSClientStatus = Test-DNSClientService
    if ($DNSClientStatus) {
        $DNSStatus.ClientService = $DNSClientStatus
        
        if ($DNSClientStatus.Issues.Count -gt 0) {
            $IssuesFound += $DNSClientStatus.Issues
            $DetectionRequired = $true
        }
    }
    else {
        $IssuesFound += "Cannot retrieve DNS Client service status"
        $DetectionRequired = $true
    }
    
    # Test DNS resolution performance
    Write-LogEntry "Testing DNS resolution performance"
    $DNSResolution = Test-DNSResolution -Domains $TestDomains
    if ($DNSResolution) {
        $DNSStatus.Resolution = $DNSResolution
        
        if ($DNSResolution.Issues.Count -gt 0) {
            $IssuesFound += $DNSResolution.Issues
            $DetectionRequired = $true
        }
        
        # Additional resolution checks
        if ($DNSResolution.FailedResolutions.Count -gt 0) {
            $IssuesFound += "Failed to resolve $($DNSResolution.FailedResolutions.Count) test domains"
            $DetectionRequired = $true
        }
    }
    else {
        $IssuesFound += "Cannot test DNS resolution performance"
        $DetectionRequired = $true
    }
    
    # Check DNS cache status
    Write-LogEntry "Checking DNS cache status"
    $DNSCache = Test-DNSCacheStatus
    if ($DNSCache) {
        $DNSStatus.Cache = $DNSCache
        
        if ($DNSCache.Issues.Count -gt 0) {
            $IssuesFound += $DNSCache.Issues
            $DetectionRequired = $true
        }
    }
    else {
        $IssuesFound += "Cannot retrieve DNS cache status"
        $DetectionRequired = $true
    }
    
    # Check DNS configuration
    Write-LogEntry "Checking DNS configuration"
    $DNSConfiguration = Test-DNSConfiguration
    if ($DNSConfiguration) {
        $DNSStatus.Configuration = $DNSConfiguration
        
        if ($DNSConfiguration.Issues.Count -gt 0) {
            $IssuesFound += $DNSConfiguration.Issues
            $DetectionRequired = $true
        }
    }
    else {
        $IssuesFound += "Cannot retrieve DNS configuration"
        $DetectionRequired = $true
    }
    
    # Check for DNS-related errors in event logs
    Write-LogEntry "Checking for DNS-related errors in event logs"
    $EventLogErrors = Test-DNSEventLogErrors
    if ($EventLogErrors) {
        $DNSStatus.EventLogs = $EventLogErrors
        
        if ($EventLogErrors.CriticalErrorCount -gt 0) {
            $IssuesFound += "Found $($EventLogErrors.CriticalErrorCount) critical DNS errors in the last 24 hours"
            $DetectionRequired = $true
        }
        elseif ($EventLogErrors.ErrorCount -gt 20) {
            $IssuesFound += "Found $($EventLogErrors.ErrorCount) DNS-related errors in the last 24 hours (threshold: 20)"
            $DetectionRequired = $true
        }
    }
    ...
    if ($DetectionRequired) {
        Write-LogEntry "DNS cache fix required. Issues found: $($IssuesFound.Count)" "Warning"
        foreach ($Issue in $IssuesFound) {
            Write-LogEntry "Issue: $Issue" "Warning"
        }
    }
    else {
        Write-LogEntry "DNS system is operating normally. No cache fix required."
    }
    ...
    if ($Detailed) {
        ...
        @{
            ...
            Summary = if ($DetectionRequired) { "DNS cache fix needed: $($IssuesFound -join '; ')" } else { "DNS system operating normally" }
            Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        } | ConvertTo-Json
    }
    ...
    $ErrorMessage = "Critical error during DNS detection: $(<#
.SCRIPT NAME
		Detect_Windows_DNS_FixCache.ps1
