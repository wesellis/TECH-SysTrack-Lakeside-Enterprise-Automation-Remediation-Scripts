...
try {
    ...
    [string]$ValidationType = "Standard",
    
    [ValidateRange(60, 300)]
    [int]$TimeoutSeconds = 180,
    
    [string]$LogPath = "$env:ProgramData\SysTrack\Logs\SCCM_FollowUp.log",
    [switch]$Verbose
)

# Initialize logging and global results
function Write-LogEntry {
    ...
    try {
        ...
        if ($Verbose -or $Level -eq "ERROR") { Write-Host $logEntry }
    } catch { Write-Warning "Failed to write to log: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
} catch { Write-Warning "Could not create log directory: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
Write-LogEntry "=== SCCM Client Agent Restart Follow-up Validation Started ==="
Write-LogEntry "Follow-up Script Version: 1.0.0"
Write-LogEntry "Parameters: ValidationType=$ValidationType, TimeoutSeconds=$TimeoutSeconds"
...
function Test-ServiceHealth {
    Write-LogEntry "Validating SCCM service health"
    try {
        ...
        foreach ($serviceName in $services) {
            ...
            if ($service -and $service.Status -eq 'Running') {
                $healthyServices++
                Write-LogEntry "Service '$serviceName' is running"
            } elseif ($serviceName -eq 'CcmExec') {
                $global:ValidationResults.Issues += "Critical service CcmExec not running"
                Write-LogEntry "Critical service CcmExec not running" "ERROR"
                return $false
            }
        }
        
        # Test WMI responsiveness for CcmExec
        try {
            $wmiTest = Get-WmiObject -Namespace "ROOT\ccm" -Class "SMS_Client" -ErrorAction Stop
            Write-LogEntry "WMI responsiveness confirmed"
            return $true
        } catch {
            $global:ValidationResults.Issues += "WMI not responsive after restart"
            Write-LogEntry "WMI not responsive: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "ERROR"
        ...
        $global:ValidationResults.Issues += "Service health check failed: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        Write-LogEntry "Service health check failed: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
function Test-ClientFunctionality {
    Write-LogEntry "Testing SCCM client functionality"
    try {
        # Test client configuration access
        $clientConfig = Get-WmiObject -Namespace "ROOT\ccm" -Class "SMS_Client" -ErrorAction Stop
        if ($clientConfig) {
            Write-LogEntry "Client configuration accessible"
            if ($clientConfig.ClientSite) {
                Write-LogEntry "Client assigned to site: $($clientConfig.ClientSite)"
            }
            return $true
        } else {
            $global:ValidationResults.Issues += "Client configuration not accessible"
            return $false
        }
    } catch {
        $global:ValidationResults.Issues += "Client functionality test failed: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        Write-LogEntry "Client functionality test failed: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
function Test-Communication {
    Write-LogEntry "Testing SCCM communication"
    try {
        $managementPoints = Get-WmiObject -Namespace "ROOT\ccm" -Class "SMS_Authority" -ErrorAction SilentlyContinue
        if ($managementPoints) {
            Write-LogEntry "Management points configured"
            return $true
        } else {
            $global:ValidationResults.Warnings += "No management points configured"
            Write-LogEntry "No management points configured" "WARN"
            return $false
        }
    } catch {
        $global:ValidationResults.Warnings += "Communication test failed: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        Write-LogEntry "Communication test failed: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "WARN"
        return $false
    }
}
...
# Main execution
try {
    Write-LogEntry "Starting SCCM Client Agent restart validation"
    $startTime = Get-Date
    
    # Load previous remediation results
    $remediationPath = "$env:ProgramData\SysTrack\Remediation\SCCM_RestartAgent_Results.json"
    if (Test-Path $remediationPath) {
        $remediationResults = Get-Content -Path $remediationPath -Raw | ConvertFrom-Json
        Write-LogEntry "Previous restart successful: $($remediationResults.RestartSuccessful)"
    }
    
    # Perform validation tests
    $serviceHealthy = Test-ServiceHealth
    $functionalityOk = Test-ClientFunctionality
    $communicationOk = Test-Communication
    
    # Calculate health score
    $score = 0
    if ($serviceHealthy) { $score += 50 }
    if ($functionalityOk) { $score += 30 }
    if ($communicationOk) { $score += 20 }
    
    $global:ValidationResults.OverallHealthScore = $score
    $global:ValidationResults.ValidationPassed = $score -ge 80
    
    $duration = (Get-Date) - $startTime
    
    # Generate summary
    Write-LogEntry "=== SCCM Client Agent Restart Validation Summary ==="
    Write-LogEntry "Validation Type: $ValidationType"
    Write-LogEntry "Service Health: $($serviceHealthy ? 'PASS' : 'FAIL')"
    Write-LogEntry "Client Functionality: $($functionalityOk ? 'PASS' : 'FAIL')"
    Write-LogEntry "Communication: $($communicationOk ? 'PASS' : 'FAIL')"
    Write-LogEntry "Overall Health Score: $score%"
    Write-LogEntry "Validation Result: $($global:ValidationResults.ValidationPassed ? 'PASS' : 'FAIL')"
    Write-LogEntry "Validation Duration: $([math]::Round($duration.TotalSeconds, 2)) seconds"
    Write-LogEntry "Issues Found: $($global:ValidationResults.Issues.Count)"
    Write-LogEntry "Warnings: $($global:ValidationResults.Warnings.Count)"
    
    # Save results
    $outputPath = "$env:ProgramData\SysTrack\FollowUp\SCCM_RestartAgent_Validation.json"
    try {
        $outputDir = Split-Path $outputPath -Parent
        if (-not (Test-Path $outputDir)) { New-Item -Path $outputDir -ItemType Directory -Force | Out-Null }
        $global:ValidationResults | ConvertTo-Json -Depth 3 | Out-File -FilePath $outputPath -Encoding UTF8
        Write-LogEntry "Validation results saved to: $outputPath"
    } catch {
        Write-LogEntry "Could not save validation results: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "WARN"
    ...
    # Event log entry
    try {
        $eventMessage = "SCCM Client Agent restart validation completed. Score: $score%. Result: $($global:ValidationResults.ValidationPassed ? 'PASS' : 'FAIL'). Duration: $([math]::Round($duration.TotalSeconds, 2))s"
        Write-EventLog -LogName "Application" -Source "SysTrack" -EventId 3001 -EntryType Information -Message $eventMessage -ErrorAction SilentlyContinue
    } catch { }
    
    Write-LogEntry "=== SCCM Client Agent Restart Validation Completed ==="
    
    # Exit with appropriate code
    if ($global:ValidationResults.ValidationPassed) {
        Write-LogEntry "Exiting with code 0 - Validation successful"
        exit 0
    } elseif ($score -ge 50) {
        Write-LogEntry "Exiting with code 2 - Partial validation success"
        exit 2
    } else {
        Write-LogEntry "Exiting with code 1 - Validation failed"
        exit 1
    }
}
catch {
    $errorMsg = "Critical error during SCCM Client Agent restart validation: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_RestartAgent.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
    ...
    try {
        Write-EventLog -LogName "Application" -Source "SysTrack" -EventId 3002 -EntryType Error -Message $errorMsg -ErrorAction SilentlyContinue
    } catch { }
    
    Write-LogEntry "Exiting with code 3 - Validation error"
    exit 3
}
