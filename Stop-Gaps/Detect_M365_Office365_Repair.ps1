...
.DESCRIPTION 
		Microsoft Office 365 repair and maintenance operations. Detection and health checking.
...
try {
    if ($Simulate) {
        ...
        LogWrite "SIMULATION MODE: Script would perform: microsoft office 365 repair and maintenance operations"
        $returnCode = 0
    } else {
        # Original script logic preserved below
        LogWrite "Starting microsoft office 365 repair and maintenance operations"
        
function Test-M365OfficeRepairNeeded {
    $issues = @()
    $severity = "Low"
    ...
    try {
        ...
        if (Test-Path $crashPath) {
            $recentCrashes = Get-ChildItem $crashPath | Where-Object {<#
.SCRIPT NAME
		Detect_M365_Office365_Repair.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -gt (Get-Date).AddDays(-7)}
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.ProcessName -match "WINWORD|EXCEL|POWERPNT|OUTLOOK"} -ErrorAction SilentlyContinue
        $hangingProcesses = $officeProcesses | Where-Object {<#
.SCRIPT NAME
		Detect_M365_Office365_Repair.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Responding -eq $false}
        if ($hangingProcesses) {
        ...
        $issues += "Error during Office health check: $(<#
.SCRIPT NAME
		Detect_M365_Office365_Repair.ps1
...
# Execute detection
$detection = Test-M365OfficeRepairNeeded

# Output for SysTrack
Write-Output "M365_OFFICE365_REPAIR_NEEDED: $($detection.RequiresRepair)"
Write-Output "ISSUE_COUNT: $($detection.IssueCount)"
Write-Output "SEVERITY: $($detection.Severity)"

if ($detection.IssuesFound) {
    Write-Output "ISSUES_DETECTED:"
    foreach ($issue in $detection.Issues) {
        Write-Output "  - $issue"
    }
    exit 1  # Indicates remediation needed
} else {
    Write-Output "No Office repair needed"
    exit 0  # No issues found
}
        
        LogWrite "Completed microsoft office 365 repair and maintenance operations"
        $returnCode = 0
    }
} catch {
