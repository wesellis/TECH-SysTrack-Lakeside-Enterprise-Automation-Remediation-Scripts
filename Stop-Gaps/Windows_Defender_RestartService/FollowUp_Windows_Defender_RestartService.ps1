...
.DESCRIPTION 
		Windows Defender antivirus service management. Post-remediation validation and verification.
...
try {
    ...
    [string]$LogSource = "SysTrack_WindowsDefender_FollowUp",
    ...
    [Parameter(Mandatory=$false)]
    [int]$RecheckIntervalHours = 4
)

# Initialize variables
$ScriptName = "FollowUp_Windows_Defender_RestartService"
...
# Function to validate service health post-restart
function Test-ServiceHealthPost {
    ...
    try {
        ...
        if ($Service.Status -eq 'Running') {
            ...
            $ServiceHealth.Issues += "Service is $($Service.Status) instead of Running"
        }
        
        # Check start type
        if ($Service.StartType -in @('Automatic', 'Manual')) {
            $ServiceHealth.HealthScore += 20
        }
        else {
            $ServiceHealth.Issues += "Service start type is $($Service.StartType)"
        }
        ...
        if ($Service.Status -eq 'Running') {
            try {
                ...
                if ($ServiceWMI.ProcessId -gt 0) {
                    ...
                    $UptimeMinutes = if ($Process.StartTime) { ((Get-Date) - $Process.StartTime).TotalMinutes } else { 0 }
                    
                    if ($UptimeMinutes -gt 1) {
                        $ServiceHealth.HealthScore += 20  # Process has been stable for over 1 minute
                    }
                    
                    # Memory usage check (adjust thresholds per service)
                    $MemoryThreshold = if ($ServiceName -eq $DefenderServiceName) { 500 } else { 200 }
                    if ($ServiceHealth.ProcessInfo.MemoryMB -lt $MemoryThreshold) {
                        $ServiceHealth.HealthScore += 10
                    }
                    else {
                        $ServiceHealth.Issues += "High memory usage: $($ServiceHealth.ProcessInfo.MemoryMB) MB"
                    }
                    
                    # Handle count check (excessive handles can indicate leaks)
                    if ($ServiceHealth.ProcessInfo.HandleCount -lt 10000) {
                        $ServiceHealth.HealthScore += 10
                    }
                    else {
                        $ServiceHealth.Issues += "High handle count: $($ServiceHealth.ProcessInfo.HandleCount)"
                    }
                }
            }
            catch {
                $ServiceHealth.Issues += "Could not retrieve process information: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        $ServiceHealth.Healthy = ($ServiceHealth.HealthScore -ge 80 -and $ServiceHealth.Issues.Count -eq 0)
        
        return $ServiceHealth
    }
    catch {
        Write-LogEntry "Failed to validate service health for $ServiceName : $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
# Function to validate Windows Defender protection status
function Test-DefenderProtectionStatus {
    try {
        $ProtectionStatus = @{
        ...
        try {
            ...
            $ProtectionStatus.AntivirusEnabled = $ComputerStatus.AntivirusEnabled
            $ProtectionStatus.RealTimeProtectionEnabled = $ComputerStatus.RealTimeProtectionEnabled
            $ProtectionStatus.BehaviorMonitorEnabled = $ComputerStatus.BehaviorMonitorEnabled
            $ProtectionStatus.IoavProtectionEnabled = $ComputerStatus.IoavProtectionEnabled
            $ProtectionStatus.NetworkInspectionEnabled = $ComputerStatus.NISEnabled
            $ProtectionStatus.TamperProtectionEnabled = $ComputerStatus.TamperProtectionSource -ne "NotSet"
            $ProtectionStatus.CloudProtectionLevel = $Preferences.MAPSReporting
            $ProtectionStatus.EngineVersion = $ComputerStatus.AMEngineVersion
            
            # Calculate definition age
            if ($ComputerStatus.AntivirusSignatureLastUpdated) {
                $ProtectionStatus.DefinitionAge = ((Get-Date) - $ComputerStatus.AntivirusSignatureLastUpdated).TotalDays
            }
            
            # Score protection features
            if ($ProtectionStatus.AntivirusEnabled) { $ProtectionStatus.ProtectionScore += 25 }
            else { $ProtectionStatus.Issues += "Antivirus protection is disabled" }
            
            if ($ProtectionStatus.RealTimeProtectionEnabled) { $ProtectionStatus.ProtectionScore += 25 }
            else { $ProtectionStatus.Issues += "Real-time protection is disabled" }
            
            if ($ProtectionStatus.BehaviorMonitorEnabled) { $ProtectionStatus.ProtectionScore += 15 }
            else { $ProtectionStatus.Recommendations += "Enable behavior monitoring for enhanced protection" }
            
            if ($ProtectionStatus.IoavProtectionEnabled) { $ProtectionStatus.ProtectionScore += 10 }
            else { $ProtectionStatus.Recommendations += "Enable IOAV protection" }
            
            if ($ProtectionStatus.NetworkInspectionEnabled) { $ProtectionStatus.ProtectionScore += 15 }
            else { $ProtectionStatus.Recommendations += "Enable Network Inspection System" }
            
            if ($ProtectionStatus.CloudProtectionLevel -ne "Disabled") { $ProtectionStatus.ProtectionScore += 10 }
            else { $ProtectionStatus.Recommendations += "Enable cloud-based protection for better detection" }
            
            # Check definition freshness
            if ($ProtectionStatus.DefinitionAge -ge 0) {
                if ($ProtectionStatus.DefinitionAge -le 1) {
                    $ProtectionStatus.ProtectionScore += 10  # Definitions very fresh
                }
                elseif ($ProtectionStatus.DefinitionAge -le 7) {
                    $ProtectionStatus.ProtectionScore += 5   # Definitions reasonably fresh
                }
                else {
                    $ProtectionStatus.Issues += "Antivirus definitions are $([math]::Round($ProtectionStatus.DefinitionAge, 1)) days old"
                }
            }
            
        }
        catch {
            $ProtectionStatus.Issues += "Could not retrieve Windows Defender status: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
            Write-LogEntry "Error retrieving Windows Defender status: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        ...
        Write-LogEntry "Failed to validate Windows Defender protection status: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
# Function to check for post-restart errors
function Test-PostRestartErrors {
    try {
        $ErrorCheck = @{
            NewErrors = @()
            CriticalErrors = @()
        ...
        $CheckStartTime = (Get-Date).AddHours(-1)  # Check last hour only
        ...
        try {
            ...
            if ($DefenderErrors) {
                $ErrorCheck.NewErrors += $DefenderErrors | ForEach-Object {
                    @{
                        TimeCreated = <#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message.Substring(0, [Math]::Min(150, <#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Message.Length))
                        Source = "Defender Operational"
                    }
                }
                
                # Identify critical errors
                $ErrorCheck.CriticalErrors += $DefenderErrors | Where-Object { <#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Id -in @(1116, 1117, 2001, 2004, 3002) }
                $ErrorCheck.CriticalErrorCount = $ErrorCheck.CriticalErrors.Count
            }
        }
        catch {
            Write-LogEntry "Could not check Defender Operational log: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Message -like "*Windows Defender*"
            ...
            if ($SystemErrors) {
                $ErrorCheck.NewErrors += $SystemErrors | ForEach-Object {
                    @{
                        TimeCreated = <#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Message.Length))
            ...
            Write-LogEntry "Could not check System log: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        $ErrorCheck.ErrorCount = $ErrorCheck.NewErrors.Count
        
        return $ErrorCheck
    }
    catch {
        Write-LogEntry "Failed to check post-restart errors: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
function Invoke-ComprehensiveValidation {
    try {
        ...
        Write-LogEntry "Performing comprehensive post-restart validation"
        
        # Test service health
        Write-LogEntry "Validating Windows Defender service health"
        $MainServiceHealth = Test-ServiceHealthPost -ServiceName $DefenderServiceName
        $NISServiceHealth = Test-ServiceHealthPost -ServiceName $DefenderNISServiceName
        
        $ValidationSummary.ValidationDetails.MainService = $MainServiceHealth
        $ValidationSummary.ValidationDetails.NISService = $NISServiceHealth
        
        if ($MainServiceHealth -and $MainServiceHealth.Healthy) {
            $ValidationSummary.HealthScore += 30
            $ValidationResults += "Windows Defender Antivirus Service is healthy"
        }
        else {
            $CriticalIssues += "Windows Defender Antivirus Service health check failed"
            if ($MainServiceHealth) {
                $CriticalIssues += $MainServiceHealth.Issues
            }
        }
        
        if ($NISServiceHealth -and $NISServiceHealth.Healthy) {
            $ValidationSummary.HealthScore += 15
            $ValidationResults += "Windows Defender Network Inspection Service is healthy"
        }
        else {
            $Recommendations += "Consider checking Network Inspection Service configuration"
            if ($NISServiceHealth) {
                $Recommendations += $NISServiceHealth.Issues
            }
        }
        
        $ValidationSummary.ServicesHealthy = ($MainServiceHealth -and $MainServiceHealth.Healthy)
        
        # Test protection status
        Write-LogEntry "Validating Windows Defender protection status"
        $ProtectionStatus = Test-DefenderProtectionStatus
        $ValidationSummary.ValidationDetails.ProtectionStatus = $ProtectionStatus
        
        if ($ProtectionStatus) {
            $ValidationSummary.HealthScore += $ProtectionStatus.ProtectionScore * 0.4  # Scale to 40 points max
            
            if ($ProtectionStatus.ProtectionScore -ge 80) {
                $ValidationSummary.ProtectionActive = $true
                $ValidationResults += "Windows Defender protection is active and properly configured"
            }
            else {
                $CriticalIssues += $ProtectionStatus.Issues
                $Recommendations += $ProtectionStatus.Recommendations
            }
        }
        
        # Check for new errors
        Write-LogEntry "Checking for post-restart errors"
        $ErrorCheck = Test-PostRestartErrors
        $ValidationSummary.ValidationDetails.ErrorCheck = $ErrorCheck
        
        if ($ErrorCheck) {
            if ($ErrorCheck.CriticalErrorCount -eq 0) {
                $ValidationSummary.HealthScore += 15
                $ValidationSummary.NoNewErrors = $true
                $ValidationResults += "No critical errors detected post-restart"
            }
            else {
                $CriticalIssues += "Found $($ErrorCheck.CriticalErrorCount) critical errors after restart"
            }
            
            if ($ErrorCheck.ErrorCount -gt 5) {
                $Recommendations += "High error rate detected - consider deeper investigation"
            }
        }
        
        # Calculate overall health
        $ValidationSummary.OverallHealthy = (
            $ValidationSummary.ServicesHealthy -and 
            $ValidationSummary.ProtectionActive -and 
            $ValidationSummary.NoNewErrors -and
            $ValidationSummary.HealthScore -ge 75
        )
        
        return $ValidationSummary
    }
    catch {
        Write-LogEntry "Error during comprehensive validation: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
# Function to generate health improvement recommendations
function Get-HealthRecommendations {
    ...
    try {
        if (-not $ValidationSummary.OverallHealthy) {
            if (-not $ValidationSummary.ServicesHealthy) {
                $HealthRecommendations += "CRITICAL: Restart Windows Defender services again"
                $HealthRecommendations += "Consider manual service management or system restart"
            }
            
            if (-not $ValidationSummary.ProtectionActive) {
                $HealthRecommendations += "Enable all Windows Defender protection features"
                $HealthRecommendations += "Update Windows Defender definitions manually"
                $HealthRecommendations += "Check for Group Policy restrictions on Windows Defender"
            }
            
            if (-not $ValidationSummary.NoNewErrors) {
                $HealthRecommendations += "Investigate recent Windows Defender errors in event logs"
                $HealthRecommendations += "Consider running Windows Defender offline scan"
            }
            
            if ($ValidationSummary.HealthScore -lt 50) {
                $HealthRecommendations += "ESCALATE: Multiple Windows Defender issues require manual intervention"
                $HealthRecommendations += "Consider reinstalling Windows Defender or running system file check"
            }
        }
        else {
            $HealthRecommendations += "Windows Defender restart was successful - all systems healthy"
            $HealthRecommendations += "Consider scheduling regular definition updates"
            $HealthRecommendations += "Monitor Windows Defender performance over next 24 hours"
        }
        ...
        Write-LogEntry "Error generating health recommendations: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
# Function to schedule recheck if needed
function Set-RecheckSchedule {
    param(
        [bool]$HealthStatus,
        [int]$IntervalHours
    )
    ...
    try {
        if (-not $HealthStatus) {
            Write-LogEntry "Scheduling health recheck in $IntervalHours hours due to validation issues"
            
            # Create scheduled task for recheck (simplified version)
            $RecheckScript = $MyInvocation.MyCommand.Path
            $TaskName = "SysTrack_WindowsDefender_HealthRecheck"
            
            # Note: In production, this would integrate with the health clinic scheduling system
            Write-LogEntry "Recheck scheduled for $(Get-Date (Get-Date).AddHours($IntervalHours) -Format 'yyyy-MM-dd HH:mm:ss')"
            
            return $true
        }
        else {
            Write-LogEntry "No recheck needed - Windows Defender is healthy"
            return $false
        }
    }
    catch {
        Write-LogEntry "Error scheduling recheck: $(<#
.SCRIPT NAME
		FollowUp_Windows_Defender_RestartService.ps1
...
Write-LogEntry "Starting Windows Defender restart follow-up validation"

try {
    # Perform comprehensive validation
    $ValidationSummary = Invoke-ComprehensiveValidation
    
    if ($ValidationSummary) {
        $ValidationSuccessful = $ValidationSummary.OverallHealthy
        $HealthScore = $ValidationSummary.HealthScore
        
        # Generate recommendations
        $AllRecommendations = Get-HealthRecommendations -ValidationSummary $ValidationSummary
        
        # Schedule recheck if needed
        if ($ScheduleRecheck) {
            Set-RecheckSchedule -HealthStatus $ValidationSuccessful -IntervalHours $RecheckIntervalHours
        }
        
        # Log validation results
        if ($ValidationSuccessful) {
            Write-LogEntry "Follow-up validation successful. Health score: $HealthScore/100"
        }
        else {
            Write-LogEntry "Follow-up validation failed. Health score: $HealthScore/100. Critical issues: $($CriticalIssues.Count)" "Warning"
        }
        ...
        $FollowUpResults = @{
            ...
            Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            ScriptVersion = "1.0"
            NextAction = if ($ValidationSuccessful) { "Monitor for 24 hours" } else { "Investigate critical issues" }
        }
        
        # Output for SysTrack integration
        $FollowUpResults | ConvertTo-Json -Depth 10
        
        # Health clinic integration
        Write-LogEntry "Health clinic integration: Status=$ValidationSuccessful, Score=$HealthScore, Issues=$($CriticalIssues.Count)"
        ...
        $ErrorMessage = "Failed to perform validation - critical system error"
