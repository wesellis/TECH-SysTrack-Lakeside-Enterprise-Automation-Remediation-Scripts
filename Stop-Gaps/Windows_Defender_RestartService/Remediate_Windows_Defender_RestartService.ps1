...
.DESCRIPTION 
		Windows Defender antivirus service management. Remediation and repair operations.
...
try {
    ...
    [string]$LogSource = "SysTrack_WindowsDefender_Remediation",
...
$ScriptName = "Remediate_Windows_Defender_RestartService"
...
# Function to safely stop a service with dependencies
function Stop-ServiceSafely {
    ...
    try {
        ...
        if ($Service.Status -eq 'Stopped') {
            Write-LogEntry "Service $ServiceName is already stopped"
            return $true
        }
        
        Write-LogEntry "Attempting to stop service: $ServiceName"
        
        # Get dependent services that need to be stopped first
        $DependentServices = Get-Service -Name $ServiceName | ForEach-Object { <#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.DependentServices }
        
        if ($DependentServices) {
            Write-LogEntry "Found $($DependentServices.Count) dependent services for $ServiceName"
            foreach ($DepService in $DependentServices) {
                if ($DepService.Status -eq 'Running') {
                    ...
                    try {
                        Stop-Service -Name $DepService.Name -Force -NoWait -ErrorAction Stop
                    }
                    catch {
                        Write-LogEntry "Warning: Could not stop dependent service $($DepService.Name): $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
                        $Warnings += "Could not stop dependent service $($DepService.Name)"
                    }
                }
            }
        ...
        $StopTimeout = (Get-Date).AddSeconds($TimeoutSeconds)
        do {
        ...
        # If service hasn't stopped, try to kill the process
        if ($Force) {
            Write-LogEntry "Service $ServiceName did not stop gracefully, attempting to terminate process" "Warning"
            try {
                $ServiceProcess = Get-WmiObject -Class Win32_Service -Filter "Name='$ServiceName'" | ForEach-Object {
                    if (<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.ProcessId -ErrorAction SilentlyContinue
                ...
                if ($ServiceProcess) {
                    $ServiceProcess | Stop-Process -Force -ErrorAction Stop
                    Start-Sleep -Seconds 3
                    Write-LogEntry "Service process terminated forcefully"
                    return $true
                }
            }
            catch {
                Write-LogEntry "Could not terminate service process: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        $Errors += "Error stopping service $ServiceName : $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
# Function to safely start a service with dependencies
function Start-ServiceSafely {
    ...
    try {
        ...
        if ($Service.Status -eq 'Running') {
            Write-LogEntry "Service $ServiceName is already running"
            return $true
        }
        
        # Check if service is disabled
        if ($Service.StartType -eq 'Disabled') {
            Write-LogEntry "Service $ServiceName is disabled, setting to Automatic"
            try {
                Set-Service -Name $ServiceName -StartupType Automatic -ErrorAction Stop
            }
            catch {
                Write-LogEntry "Could not change service startup type: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
                $Errors += "Could not change service startup type for $ServiceName"
                return $false
            }
        }
        ...
        $ServiceDependencies = $Service.ServicesDependedOn
        if ($ServiceDependencies) {
            foreach ($DepService in $ServiceDependencies) {
                if ($DepService.Status -ne 'Running') {
                    Write-LogEntry "Starting dependency service: $($DepService.Name)"
                    try {
                        Start-Service -Name $DepService.Name -ErrorAction Stop
                    }
                    catch {
                        Write-LogEntry "Warning: Could not start dependency service $($DepService.Name): $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
                        $Warnings += "Could not start dependency service $($DepService.Name)"
                    }
                }
            }
        }
        ...
        $StartTimeout = (Get-Date).AddSeconds($TimeoutSeconds)
        do {
            Start-Sleep -Seconds 2
            $Service = Get-Service -Name $ServiceName -ErrorAction SilentlyContinue
        ...
        } while ((Get-Date) -lt $StartTimeout)
        
        Write-LogEntry "Service $ServiceName failed to start within timeout period" "Error"
        $Errors += "Service $ServiceName failed to start within timeout period"
        return $false
    }
    catch {
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        $Errors += "Error starting service $ServiceName : $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
# Function to restart Windows Defender services
function Restart-DefenderServices {
    try {
        $RestartResults = @{
            MainServiceRestarted = $false
            NISServiceRestarted = $false
            ServicesRunning = $false
            Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        }
        
        Write-LogEntry "Beginning Windows Defender service restart sequence"
        
        # Step 1: Stop services in correct order (NIS first, then main service)
        Write-LogEntry "Phase 1: Stopping Windows Defender services"
        
        $NISServiceStopped = Stop-ServiceSafely -ServiceName $DefenderNISServiceName -TimeoutSeconds 60
        if ($NISServiceStopped) {
            $ActionsCompleted += "Stopped Windows Defender Network Inspection Service"
        }
        
        $MainServiceStopped = Stop-ServiceSafely -ServiceName $DefenderServiceName -TimeoutSeconds 60
        if ($MainServiceStopped) {
            $ActionsCompleted += "Stopped Windows Defender Antivirus Service"
        }
        
        # Wait a moment for cleanup
        Write-LogEntry "Waiting for service cleanup"
        Start-Sleep -Seconds 5
        
        # Step 2: Start services in correct order (main service first, then NIS)
        Write-LogEntry "Phase 2: Starting Windows Defender services"
        
        $MainServiceStarted = Start-ServiceSafely -ServiceName $DefenderServiceName -TimeoutSeconds 120
        if ($MainServiceStarted) {
            $ActionsCompleted += "Started Windows Defender Antivirus Service"
            $RestartResults.MainServiceRestarted = $true
        }
        else {
            $Errors += "Failed to start Windows Defender Antivirus Service"
        }
        
        # Wait for main service to initialize before starting NIS
        Start-Sleep -Seconds 10
        
        $NISServiceStarted = Start-ServiceSafely -ServiceName $DefenderNISServiceName -TimeoutSeconds 90
        if ($NISServiceStarted) {
            $ActionsCompleted += "Started Windows Defender Network Inspection Service"
            $RestartResults.NISServiceRestarted = $true
        }
        else {
            # NIS service failure is not critical if main service is running
            $Warnings += "Windows Defender Network Inspection Service could not be started"
        }
        
        # Step 3: Verify both services are running
        Write-LogEntry "Phase 3: Verifying service status"
        Start-Sleep -Seconds 5
        
        try {
            $MainService = Get-Service -Name $DefenderServiceName -ErrorAction Stop
            $NISService = Get-Service -Name $DefenderNISServiceName -ErrorAction Stop
            
            $MainServiceOK = ($MainService.Status -eq 'Running')
            $NISServiceOK = ($NISService.Status -eq 'Running' -or $NISService.StartType -eq 'Disabled')
            
            $RestartResults.ServicesRunning = $MainServiceOK -and $NISServiceOK
            
            Write-LogEntry "Service verification: Main=$($MainService.Status), NIS=$($NISService.Status)"
            
            if ($RestartResults.ServicesRunning) {
                $ActionsCompleted += "Verified all Windows Defender services are running properly"
            }
        }
        catch {
            Write-LogEntry "Error verifying service status: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
            $Errors += "Error verifying service status"
        }
        
        return $RestartResults
    }
    catch {
        Write-LogEntry "Critical error during service restart: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        $Errors += "Critical error during service restart: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
# Function to refresh Windows Defender status
function Update-DefenderStatus {
    try {
        Write-LogEntry "Refreshing Windows Defender status and definitions"
        
        # Update malware definitions
        try {
            Update-MpSignature -ErrorAction Stop
            $ActionsCompleted += "Updated Windows Defender malware definitions"
            Write-LogEntry "Successfully updated malware definitions"
        }
        catch {
            Write-LogEntry "Could not update malware definitions: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
            $Warnings += "Could not update malware definitions"
        }
        
        # Refresh computer status
        try {
            $ComputerStatus = Get-MpComputerStatus -ErrorAction Stop
            Write-LogEntry "Windows Defender status refreshed. Real-time protection: $($ComputerStatus.RealTimeProtectionEnabled)"
        }
        catch {
            Write-LogEntry "Could not refresh computer status: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
            $Warnings += "Could not refresh computer status"
        }
        
        return $true
    }
    catch {
        Write-LogEntry "Error updating Windows Defender status: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        $Errors += "Error updating Windows Defender status"
        return $false
    }
}

# Main remediation logic
Write-LogEntry "Starting Windows Defender service restart remediation"

try {
    # Pre-flight checks
    Write-LogEntry "Performing pre-remediation checks"
    ...
    if (-not $IsAdmin) {
        $ErrorMessage = "Script must be run with administrative privileges"
    ...
    try {
        $DefenderFeature = Get-WindowsOptionalFeature -Online -FeatureName "Windows-Defender" -ErrorAction SilentlyContinue
        if ($DefenderFeature -and $DefenderFeature.State -eq 'Disabled') {
            $ErrorMessage = "Windows Defender feature is disabled on this system"
            Write-LogEntry $ErrorMessage "Error"
            $Errors += $ErrorMessage
        ...
        Write-LogEntry "Could not check Windows Defender feature status: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
    ...
    # Execute the service restart
    Write-LogEntry "Executing Windows Defender service restart"
    $RestartResults = Restart-DefenderServices
    
    if ($RestartResults -and $RestartResults.ServicesRunning) {
        Write-LogEntry "Windows Defender services restarted successfully"
        ...
        Write-LogEntry "Windows Defender service restart failed or incomplete" "Error"
        $RemediationSuccessful = $false
    }
    ...
    $RemediationResults = @{
        ...
        Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
        ScriptVersion = "1.0"
        NextAction = if ($RemediationSuccessful) { "Run follow-up validation" } else { "Manual intervention required" }
    }
    
    # Log final results
    if ($RemediationSuccessful) {
        Write-LogEntry "Remediation completed successfully. Actions: $($ActionsCompleted.Count), Warnings: $($Warnings.Count)"
    }
    else {
        Write-LogEntry "Remediation failed. Errors: $($Errors.Count), Warnings: $($Warnings.Count)" "Error"
    }
    ...
    elseif ($ActionsCompleted.Count -gt 0) {
        Write-LogEntry "Exiting with code 2 - Partial remediation completed"
        exit 2
    }
    else {
    ...
    $ErrorMessage = "Critical error during Windows Defender remediation: $(<#
.SCRIPT NAME
		Remediate_Windows_Defender_RestartService.ps1
