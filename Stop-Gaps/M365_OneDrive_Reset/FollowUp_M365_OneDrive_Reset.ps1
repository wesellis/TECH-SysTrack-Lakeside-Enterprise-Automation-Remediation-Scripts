...
.DESCRIPTION 
		Microsoft OneDrive synchronization and reset operations. Post-remediation validation and verification.
...
try {
    if ($Simulate) {
        ...
        LogWrite "SIMULATION MODE: Script would perform: microsoft onedrive synchronization and reset operations"
        $returnCode = 0
    } else {
        # Original script logic preserved below
        LogWrite "Starting microsoft onedrive synchronization and reset operations"
...
function Test-OneDriveServiceHealth {
    try {
        $healthStatus = @{
            ProcessRunning = $false
            ProcessResponding = $false
            SyncFunctional = $false
        ...
        if ($oneDriveProcess) {
            ...
            $healthStatus.HealthScore += 25
            
            # Test 2: Process responding
            if ($oneDriveProcess.Responding) {
                $healthStatus.ProcessResponding = $true
                $healthStatus.HealthScore += 25
            }
        }
        ...
        try {
            $connectivity = Test-NetConnection "onedrive.live.com" -Port 443 -WarningAction SilentlyContinue
            if ($connectivity.TcpTestSucceeded) {
                $healthStatus.NetworkConnectivity = $true
                $healthStatus.HealthScore += 25
            }
        } catch {
            Write-EventLogEntry "Network connectivity test failed during follow-up" -Type "Warning"
        }
        
        # Test 4: Sync functionality check
        try {
            $oneDriveFolder = "$env:USERPROFILE\OneDrive"
            if (Test-Path $oneDriveFolder) {
                # Check for new sync errors
                $recentTmpFiles = Get-ChildItem $oneDriveFolder -Recurse -Filter "*.tmp" -ErrorAction SilentlyContinue | 
                                  Where-Object {<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.CreationTime -gt (Get-Date).AddMinutes(-30)}
                ...
                # Check for new conflicted copies
                $newConflicts = Get-ChildItem $oneDriveFolder -Recurse -ErrorAction SilentlyContinue | 
                               Where-Object {<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "-.*-PC|conflicted copy" -and <#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.CreationTime -gt (Get-Date).AddMinutes(-30)}
                ...
                if ((-not $recentTmpFiles -or $recentTmpFiles.Count -lt 3) -and 
                    (-not $newConflicts -or $newConflicts.Count -eq 0)) {
                    $healthStatus.SyncFunctional = $true
                    $healthStatus.HealthScore += 25
                }
            }
        } catch {
            Write-EventLogEntry "Sync functionality test failed during follow-up: $(<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Type "Warning"
        ...
        Write-EventLogEntry "Error during OneDrive health assessment: $(<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
function Test-RemediationEffectiveness {
    try {
        ...
        # Re-run the original detection logic to compare
        $oneDrivePath = "$env:LOCALAPPDATA\Microsoft\OneDrive\OneDrive.exe"
        if (-not (Test-Path $oneDrivePath)) {
            $effectiveness.NewIssues += "OneDrive executable still missing after reset"
            $effectiveness.RecommendedActions += "Reinstall OneDrive application"
        ...
        if (-not $oneDriveProcess) {
            $issues += "OneDrive process not running"
        } elseif (-not $oneDriveProcess.Responding) {
            $issues += "OneDrive process not responding"
        } else {
            $effectiveness.IssuesResolved += "OneDrive process running and responding"
        }
        
        # Registry error check
        $oneDriveReg = "HKCU:\SOFTWARE\Microsoft\OneDrive"
        if (Test-Path $oneDriveReg) {
            $errorKeys = Get-ChildItem "$oneDriveReg\Accounts" -Recurse -ErrorAction SilentlyContinue | 
                        Where-Object {<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "SyncError"}
            if ($errorKeys) {
                $issues += "OneDrive sync errors still present in registry"
            } else {
                $effectiveness.IssuesResolved += "Registry sync errors cleared"
            }
        }
        ...
        if (Test-Path $oneDriveFolder) {
            ...
            $tmpFiles = Get-ChildItem $oneDriveFolder -Recurse -Filter "*.tmp" -ErrorAction SilentlyContinue
            if ($tmpFiles -and $tmpFiles.Count -gt 5) {
                $issues += "Multiple temporary files still present ($($tmpFiles.Count))"
            } else {
                $effectiveness.IssuesResolved += "Temporary file issues resolved"
            }
            
            # Check for conflicted copies
            $conflictedFiles = Get-ChildItem $oneDriveFolder -Recurse -ErrorAction SilentlyContinue | 
                              Where-Object {<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Name -match "-.*-PC|conflicted copy"}
            if ($conflictedFiles -and $conflictedFiles.Count -gt 3) {
                $issues += "Multiple conflicted copies still present ($($conflictedFiles.Count))"
                $effectiveness.RecommendedActions += "Manual review of conflicted files needed"
            } else {
                $effectiveness.IssuesResolved += "Conflicted file issues resolved"
            }
        }
        
        # Recent log error check
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.LastWriteTime -gt (Get-Date).AddMinutes(-30)}
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "ERROR|FATAL|Exception"}
                if ($errorLines -and $errorLines.Count -gt 2) {
                    $issues += "New errors appearing in OneDrive logs"
                    break
                }
            }
            if (-not ($issues | Where-Object {<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "New errors"})) {
                $effectiveness.IssuesResolved += "Log error issues resolved"
            }
        }
        ...
        try {
            ...
            if (-not $connectivity.TcpTestSucceeded) {
                $issues += "Network connectivity to OneDrive still failing"
                $effectiveness.RecommendedActions += "Investigate network connectivity issues"
            } else {
                $effectiveness.IssuesResolved += "Network connectivity confirmed working"
            }
        } catch {
            $issues += "Network connectivity test failed"
            $effectiveness.RecommendedActions += "Check network configuration"
        }
        
        $effectiveness.RemainingIssues = $issues
        $effectiveness.OverallSuccess = $issues.Count -eq 0
        
        # Add additional recommendations based on findings
        if ($issues.Count -gt 0) {
            if ($issues.Count -gt 2) {
                $effectiveness.RecommendedActions += "Consider full OneDrive reinstallation"
            }
        ...
        Write-EventLogEntry "Error testing remediation effectiveness: $(<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
function Send-HealthClinicUpdate {
    ...
    try {
        $healthUpdate = @{
            Automation = "M365_OneDrive_Reset"
        ...
        Write-EventLogEntry "HEALTH_CLINIC_UPDATE: $healthJson" -EventId 2010
        ...
        if (Test-Path $healthTrackingPath) {
            $trackingFile = Join-Path $healthTrackingPath "OneDrive_Health_$(Get-Date -Format 'yyyyMMdd').json"
...
function Schedule-RecheckTask {
    ...
    try {
        $taskName = "SysTrack_OneDrive_Recheck_$(Get-Date -Format 'yyyyMMddHHmm')"
        ...
        Register-ScheduledTask -TaskName $taskName -Action $action -Trigger $trigger -Settings $settings -Description "SysTrack OneDrive health recheck" -Force
        
        Write-EventLogEntry "Scheduled OneDrive recheck task for: $($triggerTime.ToString('yyyy-MM-dd HH:mm:ss'))" -EventId 2020
        return $taskName
    } catch {
...
function Start-OneDriveFollowUp {
    $startTime = Get-Date
    
    try {
        Write-EventLogEntry "Starting OneDrive remediation follow-up..." -EventId 2001
        
        # Wait for initial stabilization if delay specified
        if ($ValidationDelayMinutes -gt 0) {
            Write-Output "Waiting $ValidationDelayMinutes minutes for OneDrive to stabilize..."
            Start-Sleep -Seconds ($ValidationDelayMinutes * 60)
        }
        
        # Step 1: Health status assessment
        Write-Output "Step 1: Assessing OneDrive service health..."
        $healthStatus = Test-OneDriveServiceHealth
        
        # Step 2: Remediation effectiveness check
        Write-Output "Step 2: Testing remediation effectiveness..."
        $effectiveness = Test-RemediationEffectiveness
        
        # Step 3: Health clinic integration
        Write-Output "Step 3: Updating health clinic tracking..."
        $healthClinicUpdate = Send-HealthClinicUpdate -HealthStatus $healthStatus -Effectiveness $effectiveness -RemediationTime $startTime.AddMinutes(-$ValidationDelayMinutes)
        ...
        # Log final results
        $logMessage = "OneDrive follow-up completed. Health Score: $($healthStatus.HealthScore)%, Remediation Success: $($effectiveness.OverallSuccess), Issues Resolved: $($effectiveness.IssuesResolved.Count), Remaining Issues: $($effectiveness.RemainingIssues.Count)"
        Write-EventLogEntry $logMessage -EventId 2002
        ...
        Write-EventLogEntry "OneDrive follow-up failed: $(<#
.SCRIPT NAME
		FollowUp_M365_OneDrive_Reset.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Type "Error" -EventId 2003
...
$followUp = Start-OneDriveFollowUp

# Output results for SysTrack
Write-Output "ONEDRIVE_FOLLOWUP_STATUS: $($followUp.Success)"
...
if ($followUp.Success) {
    ...
    Write-Output "OneDrive remediation follow-up completed successfully"
    exit 0
} else {
    Write-Output "FOLLOWUP_ERROR: $($followUp.Error)"
    Write-Output "OneDrive remediation follow-up failed"
    exit 2
}
        
        LogWrite "Completed microsoft onedrive synchronization and reset operations"
        $returnCode = 0
    }
} catch {
