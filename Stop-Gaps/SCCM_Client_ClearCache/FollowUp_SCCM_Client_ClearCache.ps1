...
try {
    ...
    [ValidateSet('Standard', 'Detailed', 'Json', 'HealthClinic')]
    [string]$OutputMode = 'Standard'
)

# Initialize script variables
$ScriptName = "FollowUp_SCCM_Client_ClearCache"
$ScriptVersion = "1.0.0"
$EventSource = "SysTrack_SCCM_FollowUp"
$EventLog = "Application"

# SCCM namespace definitions
$CacheNamespace = "ROOT\CCM\SoftMgmtAgent"
$SCCMClientNamespace = "ROOT\CCM"
...
try {
    Write-LogMessage "Starting SCCM Client Cache Clear follow-up validation" -Level Info
    
    # Test cache operational status
    ...
    if ($SCCMService -and $SCCMService.Status -eq 'Running') {
        ...
        Write-LogMessage "SCCM Client service is running" -Level Success
    } else {
        $FollowUpResult.Issues += "SCCM Client service is not running"
        Write-LogMessage "SCCM Client service is not running" -Level Error
    }
    
    # Test cache WMI access
    try {
        $CacheConfig = Get-WmiObject -Namespace $CacheNamespace -Class "CacheConfig" -ErrorAction Stop
        if ($CacheConfig) {
            Write-LogMessage "Cache WMI access verified" -Level Success
            
            # Get current cache metrics
            $CacheItems = Get-WmiObject -Namespace $CacheNamespace -Class "CacheInfoEx" -ErrorAction SilentlyContinue
            $FollowUpResult.CacheMetrics = @{
                TotalItems = if ($CacheItems) { $CacheItems.Count } else { 0 }
                TotalSizeMB = if ($CacheItems) { [math]::Round(($CacheItems | Measure-Object -Property ContentSize -Sum).Sum / 1MB, 2) } else { 0 }
                CachePath = $CacheConfig.Location
                MaxSizeMB = $CacheConfig.Size
            }
            
            # Determine health status
            if ($FollowUpResult.CacheMetrics.TotalSizeMB -lt 3072) {
                $FollowUpResult.HealthStatus = "Healthy"
            } elseif ($FollowUpResult.CacheMetrics.TotalSizeMB -lt 5120) {
                $FollowUpResult.HealthStatus = "Moderate"
            } else {
                $FollowUpResult.HealthStatus = "Needs Attention"
            }
            
            Write-LogMessage "Cache health status: $($FollowUpResult.HealthStatus)" -Level Info
        }
    } catch {
        $FollowUpResult.Issues += "Cannot access SCCM cache: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
        Write-LogMessage "Cannot access SCCM cache: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode" -Level Error
    ...
    if ($FollowUpResult.CacheMetrics.TotalSizeMB -gt 4096) {
        $FollowUpResult.Recommendations += "Schedule regular cache maintenance"
    }
    if ($FollowUpResult.CacheMetrics.TotalItems -gt 50) {
        $FollowUpResult.Recommendations += "Monitor for orphaned cache items"
    }
    
    # Determine exit code
    if ($FollowUpResult.Issues.Count -eq 0) {
        $FollowUpResult.ExitCode = 0
        Write-LogMessage "SCCM Client Cache Clear follow-up completed successfully" -Level Success
    } elseif ($FollowUpResult.ValidationSuccess) {
        $FollowUpResult.ExitCode = 2
        Write-LogMessage "Follow-up completed with recommendations" -Level Warning
    } else {
        $FollowUpResult.ExitCode = 1
        Write-LogMessage "Follow-up validation failed" -Level Error
    }
    
} catch {
    $ErrorMessage = "Critical error during follow-up validation: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    $FollowUpResult.Issues += $ErrorMessage
}

# Generate output
$OutputData = @"
SCCM Client Cache Clear Follow-up Results
=========================================
Timestamp: $($FollowUpResult.Timestamp)
Computer: $($FollowUpResult.ComputerName)
Validation Success: $($FollowUpResult.ValidationSuccess)
Health Status: $($FollowUpResult.HealthStatus)
Exit Code: $($FollowUpResult.ExitCode)

CACHE METRICS
Total Items: $($FollowUpResult.CacheMetrics.TotalItems)
Total Size: $($FollowUpResult.CacheMetrics.TotalSizeMB) MB
Max Size: $($FollowUpResult.CacheMetrics.MaxSizeMB) MB
Cache Path: $($FollowUpResult.CacheMetrics.CachePath)

ISSUES
$(if ($FollowUpResult.Issues.Count -gt 0) {
    $FollowUpResult.Issues | ForEach-Object { "- <#
.SCRIPT NAME
		FollowUp_SCCM_Client_ClearCache.ps1
...
RECOMMENDATIONS
$(if ($FollowUpResult.Recommendations.Count -gt 0) {
    $FollowUpResult.Recommendations | ForEach-Object { "- <#
.SCRIPT NAME
		FollowUp_SCCM_Client_ClearCache.ps1
