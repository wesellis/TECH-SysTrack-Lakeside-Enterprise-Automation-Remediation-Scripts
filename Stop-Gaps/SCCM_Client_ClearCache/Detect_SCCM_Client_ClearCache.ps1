...
try {
    ...
    [Parameter(Mandatory = $false)]
    [bool]$OrphanedContentCheck = $true,
...
$ScriptName = "Detect_SCCM_Client_ClearCache"
$ScriptVersion = "1.0.0"
...
function Test-SCCMClientInstallation {
    <#
    .SYNOPSIS
        Verifies SCCM client installation and cache service availability.
    #>
    
    Write-LogMessage "Testing SCCM client installation and cache service status" -Level Info
    
    $ClientStatus = @{
        IsInstalled = $false
        Version = $null
        ServiceStatus = $null
        CacheServiceStatus = $null
        WMIAccessible = $false
        CacheWMIAccessible = $false
        Issues = @()
    }
...
ExitScript -msg "Script Completed" -code $returnCode"
        ...
        # Test WMI access to cache namespace
        try {
            $CacheWMITest = Get-WmiObject -Namespace $CacheNamespace -Class "CacheConfig" -ErrorAction Stop
            if ($CacheWMITest) {
                $ClientStatus.CacheWMIAccessible = $true
            }
        }
        catch {
            $ClientStatus.Issues += "Cannot access SCCM cache WMI namespace: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
        ...
        # Get client version
        try {
            $VersionInfo = Get-WmiObject -Namespace $SCCMClientNamespace -Class "SMS_Client" -ErrorAction SilentlyContinue
            if ($VersionInfo) {
                $ClientStatus.Version = $VersionInfo.ClientVersion
            }
        }
        catch {
            Write-LogMessage "Could not retrieve SCCM client version" -Level Warning
        }
    }
    catch {
...
function Get-SCCMCacheConfiguration {
    <#
    .SYNOPSIS
        Retrieves SCCM cache configuration and settings.
    #>
    
    Write-LogMessage "Retrieving SCCM cache configuration" -Level Info
    ...
    try {
        ...
        $Config = Get-WmiObject -Namespace $CacheNamespace -Class "CacheConfig" -ErrorAction SilentlyContinue
        if ($Config) {
            $CacheConfig.CachePath = $Config.Location
            $CacheConfig.MaxCacheSizeMB = $Config.Size
            $CacheConfig.IsEnabled = $true
            
            # Calculate current cache size
            if ($CacheConfig.CachePath -and (Test-Path $CacheConfig.CachePath)) {
                try {
                    $CacheFiles = Get-ChildItem -Path $CacheConfig.CachePath -Recurse -File -ErrorAction SilentlyContinue
                    $CacheConfig.CurrentSizeMB = [math]::Round(($CacheFiles | Measure-Object -Property Length -Sum).Sum / 1MB, 2)
                }
                catch {
                    $CacheConfig.Issues += "Could not calculate current cache size: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
                ...
                try {
                    $Drive = Split-Path -Path $CacheConfig.CachePath -Qualifier
                    $DriveInfo = Get-WmiObject -Class Win32_LogicalDisk -Filter "DeviceID='$Drive'" -ErrorAction SilentlyContinue
                    if ($DriveInfo) {
                        $CacheConfig.FreeSpaceMB = [math]::Round($DriveInfo.FreeSpace / 1MB, 2)
                    }
                }
                catch {
                    $CacheConfig.Issues += "Could not determine free space on cache drive: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
                ...
                $CacheConfig.Issues += "Cache path does not exist or is inaccessible: $($CacheConfig.CachePath)"
            }
        }
        else {
            $CacheConfig.Issues += "Could not retrieve SCCM cache configuration"
        }
    }
    catch {
        $CacheConfig.Issues += "Error retrieving cache configuration: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode"
        Write-LogMessage "Error retrieving cache configuration: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
function Get-SCCMCacheItems {
    <#
    .SYNOPSIS
        Analyzes SCCM cache items for age, orphaned content, and integrity.
    #>
    
    Write-LogMessage "Analyzing SCCM cache items" -Level Info
    ...
    try {
        ...
        if ($CacheItems) {
            $CacheAnalysis.TotalItems = $CacheItems.Count
            $CurrentTime = Get-Date
            $AgeThreshold = $CurrentTime.AddDays(-$MaxItemAge)
            ...
            foreach ($Item in $CacheItems) {
                ...
                try {
                    if ($Item.LastReferenceTime) {
                        $ItemInfo.LastReferenced = [Management.ManagementDateTimeConverter]::ToDateTime($Item.LastReferenceTime)
                        $ItemInfo.Age = ($CurrentTime - $ItemInfo.LastReferenced).Days
                        
                        if ($ItemInfo.LastReferenced -lt $AgeThreshold) {
                            $ItemInfo.IsStale = $true
                            $CacheAnalysis.StaleItems += $ItemInfo
                        }
                    }
                    else {
                        $ItemInfo.IsOrphaned = $true
                        $CacheAnalysis.OrphanedItems += $ItemInfo
                    }
                }
                catch {
                    Write-LogMessage "Could not parse last reference time for item $($Item.ContentId)" -Level Warning
                    $ItemInfo.IsOrphaned = $true
                    $CacheAnalysis.OrphanedItems += $ItemInfo
                }
                
                # Check for orphaned content
                if ($OrphanedContentCheck -and -not $ItemInfo.IsStale -and -not $ItemInfo.IsOrphaned) {
                    try {
                        # Check if content directory exists and has valid content
                        if ($Item.Location -and (Test-Path $Item.Location)) {
                            ...
                            if (-not $ContentFiles) {
                                ...
                                $CacheAnalysis.ValidItems += $ItemInfo
                            }
                        }
                        else {
                            $ItemInfo.IsOrphaned = $true
                        ...
                        Write-LogMessage "Error checking content for item $($Item.ContentId): <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode" -Level Warning
                ...
                elseif (-not $ItemInfo.IsStale -and -not $ItemInfo.IsOrphaned) {
                    $CacheAnalysis.ValidItems += $ItemInfo
                }
                
                $CacheAnalysis.TotalSizeMB += $ItemInfo.SizeMB
            }
            
            # Calculate average age
            $ValidAgedItems = $CacheAnalysis.ValidItems + $CacheAnalysis.StaleItems | Where-Object { <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Age -gt 0 }
            if ($ValidAgedItems.Count -gt 0) {
                $CacheAnalysis.AverageAge = [math]::Round(($ValidAgedItems | Measure-Object -Property Age -Average).Average, 1)
                $CacheAnalysis.OldestItem = $ValidAgedItems | Sort-Object Age -Descending | Select-Object -First 1
            }
        }
        else {
        ...
        $CacheAnalysis.Issues += "Error analyzing cache items: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
function Test-CacheClearNeed {
    <#
    .SYNOPSIS
        Determines if SCCM cache clearing is needed based on analysis results.
    #>
    ...
    Write-LogMessage "Evaluating need for SCCM cache clearing" -Level Info
    ...
    $ClearNeed = @{
        ...
        DiskSpaceLow = $false
        OrphanedContentFound = $false
        StaleContentFound = $false
        EstimatedRecoveryMB = 0
        RecommendedAction = "None"
    }
    
    try {
        # Check cache size threshold
        if ($CacheConfig.CurrentSizeMB -gt $MaxSizeMB) {
            $ClearNeed.ShouldClear = $true
            $ClearNeed.SizeThresholdExceeded = $true
            $ClearNeed.Reasons += "Cache size ($($CacheConfig.CurrentSizeMB) MB) exceeds threshold ($MaxSizeMB MB)"
        }
        
        # Check disk space
        if ($CacheConfig.FreeSpaceMB -gt 0) {
            $TotalDiskMB = $CacheConfig.FreeSpaceMB + $CacheConfig.CurrentSizeMB
            $FreeSpacePercent = [math]::Round(($CacheConfig.FreeSpaceMB / $TotalDiskMB) * 100, 1)
            
            if ($FreeSpacePercent -lt $MinFreePercent) {
                $ClearNeed.ShouldClear = $true
                $ClearNeed.DiskSpaceLow = $true
                $ClearNeed.Priority = "High"
                $ClearNeed.Reasons += "Disk free space ($FreeSpacePercent%) below threshold ($MinFreePercent%)"
            }
        }
        
        # Check for orphaned content
        if ($CacheItems.OrphanedItems.Count -gt 0) {
            $ClearNeed.ShouldClear = $true
            $ClearNeed.OrphanedContentFound = $true
            $OrphanedSizeMB = ($CacheItems.OrphanedItems | Measure-Object -Property SizeMB -Sum).Sum
            $ClearNeed.EstimatedRecoveryMB += $OrphanedSizeMB
            $ClearNeed.Reasons += "$($CacheItems.OrphanedItems.Count) orphaned cache items found ($($OrphanedSizeMB) MB)"
        }
        
        # Check for stale content
        if ($CacheItems.StaleItems.Count -gt 0) {
            $ClearNeed.ShouldClear = $true
            $ClearNeed.StaleContentFound = $true
            $StaleSizeMB = ($CacheItems.StaleItems | Measure-Object -Property SizeMB -Sum).Sum
            $ClearNeed.EstimatedRecoveryMB += $StaleSizeMB
            $ClearNeed.Reasons += "$($CacheItems.StaleItems.Count) stale cache items found ($($StaleSizeMB) MB, older than $MaxItemAge days)"
        }
        
        # Check cache efficiency
        $InefficientCacheThreshold = 70 # percent
        if ($CacheItems.TotalItems -gt 0) {
            $InefficientItems = $CacheItems.OrphanedItems.Count + $CacheItems.StaleItems.Count
            $InefficientPercent = [math]::Round(($InefficientItems / $CacheItems.TotalItems) * 100, 1)
            
            if ($InefficientPercent -gt $InefficientCacheThreshold) {
                $ClearNeed.ShouldClear = $true
                $ClearNeed.Priority = "High"
                $ClearNeed.Reasons += "Cache efficiency poor: $InefficientPercent% of items are orphaned or stale"
            }
        }
        
        # Determine recommended action
        if ($ClearNeed.ShouldClear) {
            $ClearNeed.RecommendedAction = "Clear SCCM client cache"
            if ($ClearNeed.Priority -eq "High") {
                $ClearNeed.RecommendedAction += " with high priority"
            }
            
            if ($ClearNeed.EstimatedRecoveryMB -gt 0) {
                $ClearNeed.RecommendedAction += " (estimated recovery: $($ClearNeed.EstimatedRecoveryMB) MB)"
            }
        }
        else {
            $ClearNeed.RecommendedAction = "No cache clearing required - cache is healthy"
        }
    }
    catch {
        Write-LogMessage "Error evaluating cache clear need: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
ExitScript -msg "Script Completed" -code $returnCode" -Level Error
        $ClearNeed.ShouldClear = $true
        $ClearNeed.Reasons += "Error occurred during evaluation - clearing as precaution"
        $ClearNeed.Priority = "High"
    }
...
try {
    Write-LogMessage "Starting SCCM Client Cache Clear detection" -Level Info
    Write-LogMessage "Script version: $ScriptVersion" -Level Info
    Write-LogMessage "Maximum cache size threshold: $MaxCacheSizeMB MB" -Level Info
    Write-LogMessage "Maximum item age: $MaxItemAge days" -Level Info
    Write-LogMessage "Minimum free space: $MinFreeSpacePercent%" -Level Info
    Write-LogMessage "Orphaned content check: $OrphanedContentCheck" -Level Info
    ...
    if (-not $ClientHealth.IsInstalled -or $ClientHealth.Issues.Count -gt 0) {
        ...
        if (-not $ClientHealth.CacheWMIAccessible) {
            Write-LogMessage "Cannot access SCCM cache WMI namespace - critical issue" -Level Error
            $DetectionResult.ActionRequired = $true
            $DetectionResult.RecommendedActions += "Repair SCCM client cache WMI namespace access"
            throw "SCCM client cache WMI access unavailable"
        }
    }
    
    # Get cache configuration
    $CacheConfig = Get-SCCMCacheConfiguration
    $DetectionResult.CacheAnalysis.Configuration = $CacheConfig
    
    if ($CacheConfig.Issues.Count -gt 0) {
        Write-LogMessage "Cache configuration issues detected" -Level Warning
        $DetectionResult.Issues += $CacheConfig.Issues
        $DetectionResult.RecommendedActions += "Resolve SCCM cache configuration issues"
    }
    
    if (-not $CacheConfig.IsEnabled) {
        Write-LogMessage "SCCM cache is not enabled or configured" -Level Warning
        $DetectionResult.ExitCode = 2
        $DetectionResult.Issues += "SCCM cache is not properly configured"
        $DetectionResult.RecommendedActions += "Configure SCCM client cache properly"
        throw "SCCM cache not properly configured"
    }
    
    # Analyze cache items
    $CacheItems = Get-SCCMCacheItems
    $DetectionResult.CacheAnalysis.Items = $CacheItems
    
    if ($CacheItems.Issues.Count -gt 0) {
        Write-LogMessage "Cache item analysis issues detected" -Level Warning
        $DetectionResult.Issues += $CacheItems.Issues
    }
    
    # Determine if cache clearing is needed
    $ClearNeed = Test-CacheClearNeed -CacheConfig $CacheConfig -CacheItems $CacheItems -MaxSizeMB $MaxCacheSizeMB -MinFreePercent $MinFreeSpacePercent
    $DetectionResult.Details.ClearNeed = $ClearNeed
    
    if ($ClearNeed.ShouldClear) {
        Write-LogMessage "SCCM client cache clearing is required" -Level Info
        $DetectionResult.ActionRequired = $true
        $DetectionResult.ExitCode = 1
        $DetectionResult.Issues += $ClearNeed.Reasons
        $DetectionResult.RecommendedActions += $ClearNeed.RecommendedAction
        
        if ($ClearNeed.Priority -eq "High") {
            Write-LogMessage "High priority cache clearing needed" -Level Warning
        }
    }
    else {
        Write-LogMessage "SCCM client cache is healthy and does not need clearing" -Level Info
        $DetectionResult.ExitCode = 0
        $DetectionResult.RecommendedActions += "SCCM client cache is healthy"
    }
    ...
    $DetectionResult.Details.Summary = @{
        ...
        CacheEnabled = $CacheConfig.IsEnabled
        CachePath = $CacheConfig.CachePath
        MaxCacheSizeMB = $CacheConfig.MaxCacheSizeMB
        CurrentSizeMB = $CacheConfig.CurrentSizeMB
        FreeSpaceMB = $CacheConfig.FreeSpaceMB
        TotalItems = $CacheItems.TotalItems
        OrphanedItems = $CacheItems.OrphanedItems.Count
        StaleItems = $CacheItems.StaleItems.Count
        ValidItems = $CacheItems.ValidItems.Count
        AverageAge = $CacheItems.AverageAge
        ClearRequired = $ClearNeed.ShouldClear
        Priority = $ClearNeed.Priority
        EstimatedRecoveryMB = $ClearNeed.EstimatedRecoveryMB
    }
    
    Write-LogMessage "SCCM Client Cache Clear detection completed successfully" -Level Info
}
catch {
    $ErrorMessage = "Critical error during SCCM cache detection: <#
.SCRIPT NAME
		Detect_SCCM_Client_ClearCache.ps1
...
try {
    $OutputData = switch ($OutputMode) {
        ...
        'Detailed' {
            @"
SCCM Client Cache Clear Detection Report
========================================
...
Cache Enabled: $($DetectionResult.Details.Summary.CacheEnabled)

CACHE CONFIGURATION
Cache Path: $($DetectionResult.Details.Summary.CachePath)
Max Cache Size: $($DetectionResult.Details.Summary.MaxCacheSizeMB) MB
Current Size: $($DetectionResult.Details.Summary.CurrentSizeMB) MB
Free Space: $($DetectionResult.Details.Summary.FreeSpaceMB) MB

CACHE ANALYSIS
Total Items: $($DetectionResult.Details.Summary.TotalItems)
Valid Items: $($DetectionResult.Details.Summary.ValidItems)
Orphaned Items: $($DetectionResult.Details.Summary.OrphanedItems)
Stale Items: $($DetectionResult.Details.Summary.StaleItems)
Average Age: $($DetectionResult.Details.Summary.AverageAge) days
Clear Required: $($DetectionResult.Details.Summary.ClearRequired)
Priority: $($DetectionResult.Details.Summary.Priority)
Estimated Recovery: $($DetectionResult.Details.Summary.EstimatedRecoveryMB) MB
...
"@
        ...
        Default {
            @"
SCCM Client Cache Clear Detection
=================================
Status: $(if ($DetectionResult.ActionRequired) { "ACTION REQUIRED" } else { "OK" })
Exit Code: $($DetectionResult.ExitCode)
Cache Health: $(if ($DetectionResult.Details.Summary.CacheEnabled -and $DetectionResult.Details.Summary.CurrentSizeMB -lt $MaxCacheSizeMB) { "Healthy" } else { "Issues Detected" })
Cache Size: $($DetectionResult.Details.Summary.CurrentSizeMB) MB / $($DetectionResult.Details.Summary.MaxCacheSizeMB) MB
Items: $($DetectionResult.Details.Summary.TotalItems) total ($($DetectionResult.Details.Summary.OrphanedItems) orphaned, $($DetectionResult.Details.Summary.StaleItems) stale)
Clear Required: $($DetectionResult.Details.Summary.ClearRequired)
Priority: $($DetectionResult.Details.Summary.Priority)
"@
        }
    ...
    # Log final result
    $ResultMessage = "SCCM Client Cache Clear detection completed. Action Required: $($DetectionResult.ActionRequired), Exit Code: $($DetectionResult.ExitCode)"
