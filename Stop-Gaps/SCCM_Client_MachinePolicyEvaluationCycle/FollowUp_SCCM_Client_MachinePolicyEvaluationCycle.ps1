...
$ScriptName = "FollowUp_SCCM_Client_MachinePolicyEvaluationCycle"
$ScriptVersion = "1.0.0"
...
try {
    Write-LogMessage "Starting SCCM Machine Policy Evaluation Cycle follow-up validation" -Level Info
    Write-LogMessage "Validation mode: $ValidationMode" -Level Info
    
    $ValidationSuccess = $true
    $EffectivenessScore = 0
    
    # Validate SCCM client health
    Write-LogMessage "Validating SCCM client health" -Level Info
    try {
        $SCCMService = Get-Service -Name "CcmExec" -ErrorAction Stop
        if ($SCCMService.Status -eq 'Running') {
            $FollowUpResult.Improvements += "SCCM Client service is running and healthy"
            $EffectivenessScore += 25
            Write-LogMessage "SCCM Client service validation: PASS" -Level Success
        }
        else {
        ...
        $FollowUpResult.Issues += "SCCM Client service validation failed: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_MachinePolicyEvaluationCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    try {
        ...
        if ($WMITest) {
            $FollowUpResult.Improvements += "SCCM WMI namespace is accessible"
            $EffectivenessScore += 25
            Write-LogMessage "SCCM WMI validation: PASS" -Level Success
        }
    }
    catch {
        $FollowUpResult.Issues += "SCCM WMI namespace not accessible: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_MachinePolicyEvaluationCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    # Validate machine policy evaluation status
    Write-LogMessage "Validating machine policy evaluation effectiveness" -Level Info
    try {
        ...
        if ($Scheduler -and $Scheduler.FirstEvalTime) {
            $LastEvalTime = [Management.ManagementDateTimeConverter]::ToDateTime($Scheduler.FirstEvalTime)
            $TimeSinceEval = (Get-Date) - $LastEvalTime
            
            if ($TimeSinceEval.TotalHours -le 2) {
                $FollowUpResult.Improvements += "Machine policy evaluation cycle is current (Last: $($LastEvalTime.ToString('yyyy-MM-dd HH:mm:ss')))"
                $EffectivenessScore += 30
                Write-LogMessage "Policy evaluation timing validation: PASS" -Level Success
            }
            else {
                $FollowUpResult.Issues += "Machine policy evaluation is not current (Last: $($LastEvalTime.ToString('yyyy-MM-dd HH:mm:ss')))"
                $ValidationSuccess = $false
            }
        }
        else {
            $FollowUpResult.Issues += "Cannot determine machine policy evaluation status"
            $ValidationSuccess = $false
        }
    }
    catch {
        $FollowUpResult.Issues += "Error validating policy evaluation status: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_MachinePolicyEvaluationCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    # Validate policy health
    try {
        $Policies = Get-WmiObject -Namespace $MachinePolicyNamespace -Class "CCM_Policy" -ErrorAction SilentlyContinue
        if ($Policies) {
...
ExitScript -msg "Script Completed" -code $returnCode.PolicyRuleEnabled -eq $true }).Count
            $HealthPercentage = if ($Policies.Count -gt 0) { [math]::Round(($EnabledPolicies / $Policies.Count) * 100, 1) } else { 0 }
            
            if ($HealthPercentage -ge 80) {
                $FollowUpResult.Improvements += "Machine policy health is good ($HealthPercentage% enabled, $($Policies.Count) total)"
                $EffectivenessScore += 20
                Write-LogMessage "Policy health validation: PASS ($HealthPercentage%)" -Level Success
            }
            else {
                $FollowUpResult.Issues += "Machine policy health is poor ($HealthPercentage% enabled)"
                $ValidationSuccess = $false
            }
        }
        else {
            $FollowUpResult.Issues += "No machine policies found"
            $ValidationSuccess = $false
        }
    }
    catch {
        $FollowUpResult.Issues += "Error validating policy health: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_MachinePolicyEvaluationCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    # Check for recent policy errors
    try {
...
ExitScript -msg "Script Completed" -code $returnCode.Timestamp)
                    $ErrorTime -gt (Get-Date).AddHours(-2)
                }
                catch { $false }
            }
            
            if ($RecentErrors.Count -eq 0) {
                $FollowUpResult.Improvements += "No recent policy evaluation errors detected"
                Write-LogMessage "Policy error validation: PASS (No recent errors)" -Level Success
            }
            else {
                $FollowUpResult.Issues += "$($RecentErrors.Count) recent policy evaluation errors detected"
                $ValidationSuccess = $false
            }
        }
        else {
            $FollowUpResult.Improvements += "No policy evaluation errors found"
            Write-LogMessage "Policy error validation: PASS (No errors)" -Level Success
        }
    }
    catch {
        Write-LogMessage "Could not check for policy evaluation errors" -Level Warning
    }
    
    # Determine overall effectiveness
    $FollowUpResult.AutomationEffective = ($ValidationSuccess -and $EffectivenessScore -ge 70)
    $FollowUpResult.Success = $ValidationSuccess
    
    if ($FollowUpResult.AutomationEffective) {
        $FollowUpResult.ExitCode = 0
        $FollowUpResult.Recommendations += "Machine policy evaluation cycle automation was effective"
        $FollowUpResult.Recommendations += "Continue regular monitoring of SCCM client health"
        Write-LogMessage "Machine policy evaluation automation was EFFECTIVE (Score: $EffectivenessScore/100)" -Level Success
    }
    elseif ($ValidationSuccess) {
        $FollowUpResult.ExitCode = 2
        $FollowUpResult.Recommendations += "Machine policy evaluation partially effective - monitor for improvements"
        Write-LogMessage "Machine policy evaluation automation was PARTIALLY EFFECTIVE" -Level Warning
    }
    else {
        $FollowUpResult.ExitCode = 1
        $FollowUpResult.Recommendations += "Machine policy evaluation automation was not effective - investigate issues"
        $FollowUpResult.Recommendations += "Consider manual SCCM client repair or reinstallation"
        Write-LogMessage "Machine policy evaluation automation was NOT EFFECTIVE" -Level Error
    }
    
    Write-LogMessage "SCCM Machine Policy Evaluation Cycle follow-up completed" -Level Info
}
catch {
    $ErrorMessage = "Critical error during SCCM machine policy evaluation follow-up: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_MachinePolicyEvaluationCycle.ps1
...
try {
    $OutputData = @"
SCCM Machine Policy Evaluation Cycle Follow-up Results
======================================================
Timestamp: $($FollowUpResult.Timestamp)
Computer: $($FollowUpResult.ComputerName)
Validation Mode: $($FollowUpResult.ValidationMode)
Success: $($FollowUpResult.Success)
Automation Effective: $($FollowUpResult.AutomationEffective)
Exit Code: $($FollowUpResult.ExitCode)

VALIDATION SUMMARY
Effectiveness Score: $EffectivenessScore/100
Overall Status: $(if ($FollowUpResult.AutomationEffective) { "EFFECTIVE" } elseif ($FollowUpResult.Success) { "PARTIALLY EFFECTIVE" } else { "NOT EFFECTIVE" })

$(if ($FollowUpResult.Improvements.Count -gt 0) {
"IMPROVEMENTS DETECTED
$($FollowUpResult.Improvements | ForEach-Object { "- <#
.SCRIPT NAME
		FollowUp_SCCM_Client_MachinePolicyEvaluationCycle.ps1
...
NEXT ACTIONS
- Monitor SCCM client health regularly
- Schedule periodic machine policy evaluation cycles
- Review SCCM logs for any recurring issues
- Consider automation scheduling for proactive maintenance
"@
    
    Write-Output $OutputData
    
    # Log final result
    $ResultMessage = "SCCM Machine Policy Evaluation Cycle follow-up completed. Effective: $($FollowUpResult.AutomationEffective), Exit Code: $($FollowUpResult.ExitCode)"
