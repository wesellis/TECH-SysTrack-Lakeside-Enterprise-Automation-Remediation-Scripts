...
try {
    ...
    [Parameter(Mandatory = $false)]
    [bool]$ValidateDataCompleteness = $true,
    
    [Parameter(Mandatory = $false)]
    [bool]$CheckRecentActivity = $true,
    
    [Parameter(Mandatory = $false)]
    [ValidateRange(1, 24)]
    [int]$ActivityTimeframeHours = 2
)

# Initialize script variables
$ScriptName = "FollowUp_SCCM_Client_DiscoveryDataCollectionCycle"
$ScriptVersion = "1.0.0"
...
try {
    Write-LogMessage "Starting SCCM Client Discovery Data Collection Cycle follow-up validation" -Level Info
    Write-LogMessage "Script version: $ScriptVersion" -Level Info
    Write-LogMessage "Activity timeframe: $ActivityTimeframeHours hours" -Level Info
    Write-LogMessage "Data completeness validation: $ValidateDataCompleteness" -Level Info
    Write-LogMessage "Recent activity check: $CheckRecentActivity" -Level Info
    
    # Test SCCM client health
    Write-LogMessage "Phase 1: Validating SCCM client health" -Level Info
    $ClientHealth = @{
    ...
    try {
        ...
        if ($SCCMService -and $SCCMService.Status -eq 'Running') {
            $ClientHealth.ServiceRunning = $true
        }
        else {
            $ClientHealth.Issues += "SCCM Client service not running"
        }
        
        $SMSClient = Get-WmiObject -Namespace $SCCMNamespace -Class "SMS_Client" -ErrorAction SilentlyContinue
        if ($SMSClient) {
            $ClientHealth.WMIAccessible = $true
        }
        else {
            $ClientHealth.Issues += "SCCM WMI namespace not accessible"
        }
        
        $ClientHealth.OverallHealth = if ($ClientHealth.ServiceRunning -and $ClientHealth.WMIAccessible) { "Healthy" } else { "Critical" }
    }
    catch {
        $ClientHealth.Issues += "Error validating client health: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode"
        $ClientHealth.OverallHealth = "Error"
    }
    
    $FollowUpResult.Details.ClientHealth = $ClientHealth
    
    # Validate discovery cycle execution
    Write-LogMessage "Phase 2: Validating discovery cycle execution" -Level Info
    $DiscoveryStatus = @{
        RecentExecutionDetected = $false
    ...
    if ($CheckRecentActivity) {
        $CheckTimeframe = (Get-Date).AddHours(-$ActivityTimeframeHours)
...
ExitScript -msg "Script Completed" -code $returnCode.InventoryActionID -eq $DiscoveryScheduleID }
                if ($DiscoveryAction) {
                    try {
                        $LastExecutionTime = [Management.ManagementDateTimeConverter]::ToDateTime($DiscoveryAction.LastUpdateTime)
                        if ($LastExecutionTime -gt $CheckTimeframe) {
                            $DiscoveryStatus.RecentExecutionDetected = $true
                            $DiscoveryStatus.ActivityEvidence += "Discovery action updated within timeframe"
                        }
                        
                        if ($DiscoveryAction.LastReportDate) {
                            $LastReportTime = [Management.ManagementDateTimeConverter]::ToDateTime($DiscoveryAction.LastReportDate)
                            if ($LastReportTime -gt $CheckTimeframe) {
                                $DiscoveryStatus.ExecutionSuccessful = $true
                                $DiscoveryStatus.ActivityEvidence += "Discovery data reported successfully"
                            }
                        }
                    }
                    catch {
                        $DiscoveryStatus.Issues += "Could not parse discovery timestamps"
                    }
                }
            }
            
            # Check event log for discovery activity
            $DiscoveryEvents = Get-EventLog -LogName "Application" -After $CheckTimeframe -ErrorAction SilentlyContinue |
                Where-Object { <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Source -like "*SCCM*" -and <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -like "*Discovery*" }
            
            if ($DiscoveryEvents) {
                $DiscoveryStatus.ActivityEvidence += "Discovery events found in event log"
                if ($DiscoveryEvents | Where-Object { <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.EntryType -eq 'Information' }) {
                    $DiscoveryStatus.ExecutionSuccessful = $true
                }
            }
        }
        catch {
            $DiscoveryStatus.Issues += "Error checking discovery activity: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
        ...
        if ($DiscoveryStatus.RecentExecutionDetected -and $DiscoveryStatus.ExecutionSuccessful) {
            $DiscoveryStatus.CycleHealth = "Healthy"
        }
        elseif ($DiscoveryStatus.RecentExecutionDetected) {
            $DiscoveryStatus.CycleHealth = "Warning"
        }
        else {
            $DiscoveryStatus.CycleHealth = "Critical"
            $DiscoveryStatus.Issues += "No recent discovery activity detected"
        }
    }
    
    $FollowUpResult.DiscoveryValidation = $DiscoveryStatus
    
    # Validate data completeness if enabled
    Write-LogMessage "Phase 3: Validating discovery data completeness" -Level Info
    $DataCompleteness = @{
    ...
    if ($ValidateDataCompleteness) {
        try {
            $ComponentChecks = @(
                ...
                @{ Name = "Processor"; Class = "Win32_Processor" },
                @{ Name = "Memory"; Class = "Win32_PhysicalMemory" },
                @{ Name = "Network"; Class = "Win32_NetworkAdapter" },
                @{ Name = "Storage"; Class = "Win32_LogicalDisk" }
            )
            
            foreach ($Check in $ComponentChecks) {
                try {
                    $Data = Get-WmiObject -Class $Check.Class -ErrorAction SilentlyContinue
                    if ($Data) {
                        $DataCompleteness.ComponentsHealthy++
                        Write-LogMessage "$($Check.Name) data present" -Level Debug
                    }
                    else {
                        $DataCompleteness.Issues += "$($Check.Name) data missing"
                    }
                }
                catch {
                    $DataCompleteness.Issues += "Error checking $($Check.Name): <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
            ...
            $DataCompleteness.CompletenessPercentage = [math]::Round(($DataCompleteness.ComponentsHealthy / $DataCompleteness.TotalComponents) * 100, 1)
            $DataCompleteness.DataQualityScore = if ($DataCompleteness.CompletenessPercentage -ge 85) { "Excellent" } 
                                               elseif ($DataCompleteness.CompletenessPercentage -ge 70) { "Good" } 
                                               elseif ($DataCompleteness.CompletenessPercentage -ge 50) { "Fair" } 
                                               else { "Poor" }
        }
        catch {
            $DataCompleteness.Issues += "Error validating data completeness: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    $FollowUpResult.Details.DataCompleteness = $DataCompleteness
    
    # Generate health metrics
    $FollowUpResult.HealthMetrics = @{
        OverallHealth = "Unknown"
        ClientHealth = $ClientHealth.OverallHealth
        DiscoveryHealth = $DiscoveryStatus.CycleHealth
        DataCompleteness = $DataCompleteness.CompletenessPercentage
        DataQuality = $DataCompleteness.DataQualityScore
        RecentActivity = $DiscoveryStatus.RecentExecutionDetected
        ValidationTimestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
    }
    
    # Determine overall validation success
    $ValidationSuccessful = $ClientHealth.OverallHealth -eq "Healthy" -and 
                           $DiscoveryStatus.CycleHealth -in @("Healthy", "Warning") -and
                           $DataCompleteness.CompletenessPercentage -ge 70
    
    if ($ValidationSuccessful) {
        $FollowUpResult.ValidationSuccess = $true
        $FollowUpResult.ExitCode = 0
        $FollowUpResult.HealthMetrics.OverallHealth = "Healthy"
        Write-LogMessage "SCCM discovery cycle follow-up validation successful" -Level Info
    }
    else {
        $FollowUpResult.ValidationSuccess = $false
        $FollowUpResult.ExitCode = if ($ClientHealth.OverallHealth -eq "Critical") { 2 } else { 1 }
        $FollowUpResult.HealthMetrics.OverallHealth = "Warning"
        $FollowUpResult.Issues += $ClientHealth.Issues + $DiscoveryStatus.Issues + $DataCompleteness.Issues
        Write-LogMessage "SCCM discovery cycle follow-up validation failed" -Level Warning
    }
    
    # Generate recommendations
    if ($ClientHealth.OverallHealth -ne "Healthy") {
        $FollowUpResult.Recommendations += "Investigate and resolve SCCM client health issues"
    }
    if ($DiscoveryStatus.CycleHealth -eq "Critical") {
        $FollowUpResult.Recommendations += "Re-trigger discovery data collection cycle"
    }
    if ($DataCompleteness.CompletenessPercentage -lt 70) {
        $FollowUpResult.Recommendations += "Investigate discovery data collection issues"
    }
    
    # Populate summary details
    $FollowUpResult.Details.Summary = @{
        ValidationSuccess = $FollowUpResult.ValidationSuccess
        OverallHealth = $FollowUpResult.HealthMetrics.OverallHealth
        ClientOperational = $ClientHealth.ServiceRunning -and $ClientHealth.WMIAccessible
        RecentDiscoveryActivity = $DiscoveryStatus.RecentExecutionDetected
        DiscoveryExecutionSuccess = $DiscoveryStatus.ExecutionSuccessful
        DataCompletenessPercentage = $DataCompleteness.CompletenessPercentage
        DataQualityScore = $DataCompleteness.DataQualityScore
        IssuesIdentified = $FollowUpResult.Issues.Count
        RecommendationsProvided = $FollowUpResult.Recommendations.Count
        ActivityEvidenceCount = $DiscoveryStatus.ActivityEvidence.Count
    }
}
catch {
    $ErrorMessage = "Critical error during SCCM discovery cycle follow-up: <#
.SCRIPT NAME
		FollowUp_SCCM_Client_DiscoveryDataCollectionCycle.ps1
...
ExitScript -msg "Script Completed" -code $returnCode"
    ...
    $FollowUpResult.HealthMetrics.OverallHealth = "Error"
    ...
    $OutputData = switch ($OutputMode) {
        ...
        'HealthClinic' {
            @{
                ...
                ComputerName = $FollowUpResult.ComputerName
                AutomationType = "SCCM_Discovery_Data_Collection"
                HealthStatus = $FollowUpResult.HealthMetrics.OverallHealth
                ValidationSuccess = $FollowUpResult.ValidationSuccess
                Metrics = $FollowUpResult.HealthMetrics
                Issues = $FollowUpResult.Issues
                Recommendations = $FollowUpResult.Recommendations
            } | ConvertTo-Json -Depth 5
        }
        'Detailed' {
            @"
SCCM Client Discovery Data Collection Cycle Follow-up Report
===========================================================
...
Overall Health: $($FollowUpResult.HealthMetrics.OverallHealth)
Exit Code: $($FollowUpResult.ExitCode)

CLIENT HEALTH
Service Running: $($FollowUpResult.Details.Summary.ClientOperational)
Client Health: $($FollowUpResult.HealthMetrics.ClientHealth)

DISCOVERY VALIDATION
Recent Activity: $($FollowUpResult.Details.Summary.RecentDiscoveryActivity)
Execution Success: $($FollowUpResult.Details.Summary.DiscoveryExecutionSuccess)
Discovery Health: $($FollowUpResult.HealthMetrics.DiscoveryHealth)

DATA COMPLETENESS
Completeness: $($FollowUpResult.Details.Summary.DataCompletenessPercentage)%
Quality Score: $($FollowUpResult.Details.Summary.DataQualityScore)
...
"@
        ...
        Default {
            @"
SCCM Client Discovery Data Collection Cycle Follow-up
=====================================================
Status: $(if ($FollowUpResult.ValidationSuccess) { "SUCCESS" } else { "FAILED" })
Overall Health: $($FollowUpResult.HealthMetrics.OverallHealth)
Exit Code: $($FollowUpResult.ExitCode)
Client Health: $($FollowUpResult.HealthMetrics.ClientHealth)
Discovery Health: $($FollowUpResult.HealthMetrics.DiscoveryHealth)
Data Completeness: $($FollowUpResult.Details.Summary.DataCompletenessPercentage)%
Recent Activity: $($FollowUpResult.Details.Summary.RecentDiscoveryActivity)
Issues: $($FollowUpResult.Issues.Count)
...
"@
    ...
    # Log final result
    $ResultMessage = "SCCM Client Discovery Data Collection Cycle follow-up completed. Success: $($FollowUpResult.ValidationSuccess), Health: $($FollowUpResult.HealthMetrics.OverallHealth)"
