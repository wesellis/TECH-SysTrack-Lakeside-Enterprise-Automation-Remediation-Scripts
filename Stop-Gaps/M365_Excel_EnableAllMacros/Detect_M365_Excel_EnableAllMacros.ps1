...
.DESCRIPTION 
		Microsoft 365 Excel macro and configuration management. Detection and health checking.
...
function Test-ExcelMacroSettings {
    $issues = @()
    $severity = "Low"
    $macroSettings = @{}
    
    try {
        # Check if Office/Excel is installed
        $officeInstallPaths = @(
        ...
        foreach ($path in $officeInstallPaths) {
            if (Test-Path $path) {
                $excelInstalled = $true
                $excelPath = $path
                if ($path -match "Office16") {
                    $officeVersion = "Office 2016/2019/2021/365"
        ...
        if (-not $excelInstalled) {
            return @{
                IssuesFound = $false
                IssueCount = 0
                Issues = @("Excel not installed")
                Severity = "Info"
                RequiresMacroEnablement = $false
                MacroSettings = @{}
                OfficeVersion = "Not installed"
            }
        }
        
        $macroSettings["ExcelPath"] = $excelPath
        $macroSettings["OfficeVersion"] = $officeVersion
        
        # Check Excel macro security settings in registry
        $excelRegistryPaths = @(
            "HKCU:\SOFTWARE\Microsoft\Office\16.0\Excel\Security",
            "HKCU:\SOFTWARE\Microsoft\Office\15.0\Excel\Security",
            "HKLM:\SOFTWARE\Microsoft\Office\16.0\Excel\Security",
            "HKLM:\SOFTWARE\Microsoft\Office\15.0\Excel\Security",
            "HKLM:\SOFTWARE\Policies\Microsoft\Office\16.0\Excel\Security",
            "HKLM:\SOFTWARE\Policies\Microsoft\Office\15.0\Excel\Security"
        )
        
        $macroSecurityLevel = $null
        $vbaWarningsPolicy = $null
        $accessVBOM = $null
        $blockMacros = $null
        
        foreach ($regPath in $excelRegistryPaths) {
            if (Test-Path $regPath) {
                try {
                    # VBASecurityLevel: 1=Enable all, 2=Prompt, 3=Digitally signed only, 4=Disable all
                    $secLevel = Get-ItemProperty $regPath -Name "VBASecurityLevel" -ErrorAction SilentlyContinue
                    if ($secLevel -and $null -eq $macroSecurityLevel) {
                        $macroSecurityLevel = $secLevel.VBASecurityLevel
                        $macroSettings["VBASecurityLevel"] = $macroSecurityLevel
                        $macroSettings["VBASecuritySource"] = $regPath
                    }
                    
                    # VBAWarnings: 1=Enable all, 2=Disable with notification, 3=Disable all, 4=Disable all except digitally signed
                    $warnings = Get-ItemProperty $regPath -Name "VBAWarnings" -ErrorAction SilentlyContinue
                    if ($warnings -and $null -eq $vbaWarningsPolicy) {
                        $vbaWarningsPolicy = $warnings.VBAWarnings
                        $macroSettings["VBAWarnings"] = $vbaWarningsPolicy
                    }
                    
                    # AccessVBOM: 1=Allow access to VBA object model, 0=Deny access
                    $vbom = Get-ItemProperty $regPath -Name "AccessVBOM" -ErrorAction SilentlyContinue
                    if ($vbom -and $null -eq $accessVBOM) {
                        $accessVBOM = $vbom.AccessVBOM
                        $macroSettings["AccessVBOM"] = $accessVBOM
                    }
                    
                    # Check for macro blocking policies
                    $blockMacroValue = Get-ItemProperty $regPath -Name "BlockMacros" -ErrorAction SilentlyContinue
                    if ($blockMacroValue -and $null -eq $blockMacros) {
                        $blockMacros = $blockMacroValue.BlockMacros
                        $macroSettings["BlockMacros"] = $blockMacros
                    }
                } catch {
        ...
        # Analyze macro security settings
        if ($null -ne $macroSecurityLevel) {
            switch ($macroSecurityLevel) {
                1 { 
                    $macroSettings["SecurityDescription"] = "Enable all macros (not recommended)"
                    # This is what we want, but check if there are conflicting policies
                }
                2 { 
                    $macroSettings["SecurityDescription"] = "Disable all macros with notification"
                    $issues += "Excel macro security set to prompt for macros (Level 2)"
                    $severity = "Medium"
                }
                3 { 
                    $macroSettings["SecurityDescription"] = "Disable all macros except digitally signed macros"
                    $issues += "Excel macro security set to digitally signed only (Level 3)"
                    $severity = "High"
                }
                4 { 
                    $macroSettings["SecurityDescription"] = "Disable all macros without notification"
                    $issues += "Excel macro security set to disable all macros (Level 4)"
                    $severity = "High"
                }
                default {
                    $issues += "Unknown Excel macro security level: $macroSecurityLevel"
                    $severity = "Medium"
                }
            }
        } else {
            $issues += "Excel macro security level not found in registry"
            $severity = "Medium"
        }
        
        # Check VBA Warnings policy
        if ($null -ne $vbaWarningsPolicy) {
            switch ($vbaWarningsPolicy) {
                1 { 
                    $macroSettings["WarningsDescription"] = "Enable all macros (VBA Warnings = 1)"
                    # This is what we want
                }
                2 { 
                    $macroSettings["WarningsDescription"] = "Disable with notification (VBA Warnings = 2)"
                    $issues += "VBA warnings policy set to disable with notification"
                    if ($severity -eq "Low") { $severity = "Medium" }
                }
                3 { 
                    $macroSettings["WarningsDescription"] = "Disable all macros (VBA Warnings = 3)"
                    $issues += "VBA warnings policy set to disable all macros"
                    $severity = "High"
                }
                4 { 
                    $macroSettings["WarningsDescription"] = "Disable except digitally signed (VBA Warnings = 4)"
                    $issues += "VBA warnings policy set to digitally signed only"
                    $severity = "High"
                }
                default {
                    $issues += "Unknown VBA warnings policy: $vbaWarningsPolicy"
                    if ($severity -eq "Low") { $severity = "Medium" }
                }
            }
        }
        
        # Check if macros are completely blocked
        if ($blockMacros -eq 1) {
            $issues += "Macros are explicitly blocked by policy"
            $severity = "High"
            $macroSettings["BlockMacrosDescription"] = "Macros blocked by policy"
        }
        
        # Check Trust Center settings
        $trustCenterPath = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Excel\Security\Trusted Locations"
        if (-not (Test-Path $trustCenterPath)) {
            $trustCenterPath = "HKCU:\SOFTWARE\Microsoft\Office\15.0\Excel\Security\Trusted Locations"
        }
        
        if (Test-Path $trustCenterPath) {
            $trustedLocations = Get-ChildItem $trustCenterPath -ErrorAction SilentlyContinue
            $macroSettings["TrustedLocationsCount"] = $trustedLocations.Count
            
            if ($trustedLocations.Count -eq 0) {
                $issues += "No trusted locations configured for Excel"
                if ($severity -eq "Low") { $severity = "Medium" }
            }
        }
        ...
        foreach ($gpoPath in $gpoPaths) {
            if (Test-Path $gpoPath) {
                ...
                if ($gpoSettings) {
                    $gpoRestrictions += "Group Policy settings found at: $gpoPath"
                    
                    # Check specific GPO macro restrictions
                    if ($gpoSettings.VBASecurityLevel -and $gpoSettings.VBASecurityLevel -gt 1) {
                        $gpoRestrictions += "GPO enforces macro security level: $($gpoSettings.VBASecurityLevel)"
                    }
                    if ($gpoSettings.VBAWarnings -and $gpoSettings.VBAWarnings -gt 1) {
                        $gpoRestrictions += "GPO enforces VBA warnings: $($gpoSettings.VBAWarnings)"
                    }
                }
            }
        }
        ...
        if ($gpoRestrictions.Count -gt 0) {
            $issues += $gpoRestrictions
            $severity = "High"
            $macroSettings["GPORestrictions"] = $gpoRestrictions
        }
        
        # Check current Excel process for active workbooks with macros
        $excelProcess = Get-Process "EXCEL" -ErrorAction SilentlyContinue
        if ($excelProcess) {
            $macroSettings["ExcelRunning"] = $true
            $macroSettings["ExcelProcessCount"] = $excelProcess.Count
        } else {
            $macroSettings["ExcelRunning"] = $false
        }
        
        # Check Windows Event Log for macro-related errors
        try {
            $yesterday = (Get-Date).AddDays(-1)
            $macroErrors = Get-WinEvent -FilterHashtable @{LogName='Application'; StartTime=$yesterday; Level=2} -ErrorAction SilentlyContinue | 
                          Where-Object {<#
.SCRIPT NAME
		Detect_M365_Excel_EnableAllMacros.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.ProviderName -match "Excel|Office" -and <#
.SCRIPT NAME
		Detect_M365_Excel_EnableAllMacros.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Message -match "macro|VBA"}
            if ($macroErrors -and $macroErrors.Count -gt 0) {
                $issues += "Excel macro errors found in Event Log: $($macroErrors.Count) errors in last 24 hours"
                if ($severity -eq "Low") { $severity = "Medium" }
                $macroSettings["RecentMacroErrors"] = $macroErrors.Count
            }
        } catch {
            # Event log access might fail, continue without this check
        }
        
        # Check for common macro-enabled file types in recent files
        try {
            $recentFilesPath = "HKCU:\SOFTWARE\Microsoft\Office\16.0\Excel\File MRU"
            if (-not (Test-Path $recentFilesPath)) {
                $recentFilesPath = "HKCU:\SOFTWARE\Microsoft\Office\15.0\Excel\File MRU"
            }
            
            if (Test-Path $recentFilesPath) {
                $recentFiles = Get-ItemProperty $recentFilesPath -ErrorAction SilentlyContinue
                $macroEnabledFiles = 0
                
                foreach ($property in $recentFiles.PSObject.Properties) {
                    if ($property.Name -match "Item \d+" -and $property.Value -match "\.(xlsm|xltm|xlam)") {
                        $macroEnabledFiles++
                    }
                }
                
                $macroSettings["RecentMacroEnabledFiles"] = $macroEnabledFiles
                if ($macroEnabledFiles -gt 0 -and ($macroSecurityLevel -gt 1 -or $vbaWarningsPolicy -gt 1)) {
                    $issues += "Recent macro-enabled files detected but macros are restricted: $macroEnabledFiles files"
                    if ($severity -eq "Low") { $severity = "Medium" }
                }
            }
        } catch {
            # Registry access issues, continue
        }
        
        # Check Office Click-to-Run vs MSI installation
        try {
            $clickToRunPath = "HKLM:\SOFTWARE\Microsoft\Office\ClickToRun\Configuration"
            if (Test-Path $clickToRunPath) {
                $macroSettings["InstallationType"] = "Click-to-Run"
                # Click-to-Run installations may have different policy enforcement
            } else {
                $macroSettings["InstallationType"] = "MSI"
            }
        } catch {
            $macroSettings["InstallationType"] = "Unknown"
        }
        
    } catch {
        $issues += "Error during Excel macro settings check: $(<#
.SCRIPT NAME
		Detect_M365_Excel_EnableAllMacros.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
    ...
    # Determine if macro enablement is needed
    $requiresMacroEnablement = ($macroSecurityLevel -gt 1) -or 
                              ($vbaWarningsPolicy -gt 1) -or 
                              ($blockMacros -eq 1) -or
                              ($gpoRestrictions.Count -gt 0) -or
                              ($issues | Where-Object {<#
.SCRIPT NAME
		Detect_M365_Excel_EnableAllMacros.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -match "security|disable|block|restrict"})
    ...
    $result = @{
        ...
        Issues = $issues
        Severity = $severity
        RequiresMacroEnablement = $requiresMacroEnablement
...
# Execute detection
$detection = Test-ExcelMacroSettings

# Output for SysTrack
Write-Output "M365_EXCEL_MACRO_ENABLEMENT_NEEDED: $($detection.RequiresMacroEnablement)"
Write-Output "ISSUE_COUNT: $($detection.IssueCount)"
Write-Output "SEVERITY: $($detection.Severity)"
Write-Output "OFFICE_VERSION: $($detection.OfficeVersion)"

if ($detection.MacroSettings.Count -gt 0) {
    Write-Output "MACRO_SETTINGS_SUMMARY:"
    foreach ($setting in $detection.MacroSettings.GetEnumerator()) {
        Write-Output "  $($setting.Key): $($setting.Value)"
    }
}

if ($detection.IssuesFound) {
    Write-Output "MACRO_RESTRICTIONS_DETECTED:"
    foreach ($issue in $detection.Issues) {
        Write-Output "  - $issue"
    }
    
    if ($detection.RequiresMacroEnablement) {
        Write-Output "Excel macro enablement recommended"
        exit 1  # Indicates remediation needed
    } else {
        Write-Output "Excel macro issues detected but enablement not required"
        exit 0
    }
} else {
    Write-Output "No Excel macro restrictions detected"
