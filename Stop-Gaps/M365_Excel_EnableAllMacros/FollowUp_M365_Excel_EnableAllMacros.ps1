...
.DESCRIPTION 
		Microsoft 365 Excel macro and configuration management. Post-remediation validation and verification.
...
param(
    ...
    [int]$ValidationDelayMinutes = 2,
    [switch]$ScheduleRecheck,
    [int]$RecheckHours = 24,
    [switch]$TestMacroExecution,
    [switch]$Simulate,
...
function Test-MacroSettingsValidation {
    try {
        $validationStatus = @{
            SettingsCorrect = $false
            OfficeVersionsChecked = @()
            SecurityLevelValid = $false
            WarningsDisabled = $false
            VBOMAccessEnabled = $false
            TrustedLocationsCount = 0
            HealthScore = 0
            ConfigurationErrors = @()
        }
        
        # Check Excel macro settings for all Office versions
        ...
        foreach ($version in $officeVersions) {
            ...
            if (Test-Path $regPath) {
                $validationStatus.OfficeVersionsChecked += $version
                
                try {
                    $settings = Get-ItemProperty $regPath -ErrorAction SilentlyContinue
                    
                    # Check VBA Security Level
                    if ($settings.VBASecurityLevel -eq 1) {
                        $validationStatus.SecurityLevelValid = $true
                        $validationStatus.HealthScore += 25
                        Write-EventLogEntry "VBA Security Level correctly set to 1 for Office $version"
                    } else {
                        $validationStatus.ConfigurationErrors += "VBA Security Level not set to 1 for Office $version (Current: $($settings.VBASecurityLevel))"
                    }
                    
                    # Check VBA Warnings
                    if ($settings.VBAWarnings -eq 1) {
                        $validationStatus.WarningsDisabled = $true
                        $validationStatus.HealthScore += 25
                        Write-EventLogEntry "VBA Warnings correctly set to 1 for Office $version"
                    } else {
                        $validationStatus.ConfigurationErrors += "VBA Warnings not set to 1 for Office $version (Current: $($settings.VBAWarnings))"
                    }
                    
                    # Check VBA Object Model Access
                    if ($settings.AccessVBOM -eq 1) {
                        $validationStatus.VBOMAccessEnabled = $true
                        $validationStatus.HealthScore += 25
                        Write-EventLogEntry "VBA Object Model access correctly enabled for Office $version"
                    } else {
                        $validationStatus.ConfigurationErrors += "VBA Object Model access not enabled for Office $version"
                    }
                    
                    # Check for conflicting BlockMacros setting
                    if ($settings.BlockMacros -eq 1) {
                        $validationStatus.ConfigurationErrors += "BlockMacros is set to 1, which conflicts with macro enablement for Office $version"
                    }
                    
                } catch {
                    $validationStatus.ConfigurationErrors += "Error reading settings for Office $version`: $(<#
.SCRIPT NAME
		FollowUp_M365_Excel_EnableAllMacros.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
                ...
                if (Test-Path $trustedLocationsPath) {
                    $trustedLocations = Get-ChildItem $trustedLocationsPath -ErrorAction SilentlyContinue
                    $validationStatus.TrustedLocationsCount += $trustedLocations.Count
        ...
        $validationStatus.SettingsCorrect = $validationStatus.SecurityLevelValid -and 
                                           $validationStatus.WarningsDisabled -and 
                                           $validationStatus.VBOMAccessEnabled -and
                                           $validationStatus.ConfigurationErrors.Count -eq 0
        
        return $validationStatus
    } catch {
        Write-EventLogEntry "Error during macro settings validation: $(<#
.SCRIPT NAME
		FollowUp_M365_Excel_EnableAllMacros.ps1
...
function Test-RemediationEffectiveness {
    try {
        ...
        # Validate macro settings are properly configured
        $settingsValidation = Test-MacroSettingsValidation
        
        if ($settingsValidation.SettingsCorrect) {
            $effectiveness.IssuesResolved += "Excel macro security settings properly configured"
            $effectiveness.IssuesResolved += "VBA Security Level set to enable all macros"
            $effectiveness.IssuesResolved += "VBA warnings disabled for smooth operation"
            
            if ($settingsValidation.VBOMAccessEnabled) {
                $effectiveness.IssuesResolved += "VBA Object Model access enabled"
            }
            
            if ($settingsValidation.TrustedLocationsCount -gt 0) {
                $effectiveness.IssuesResolved += "Trusted locations configured: $($settingsValidation.TrustedLocationsCount) locations"
            }
        } else {
            foreach ($error in $settingsValidation.ConfigurationErrors) {
                $effectiveness.RemainingIssues += $error
            }
        }
        
        # Overall success determination
        $effectiveness.OverallSuccess = ($effectiveness.RemainingIssues.Count -eq 0) -and 
                                       ($settingsValidation.SettingsCorrect)
        
        # Add recommendations based on findings
        if ($effectiveness.OverallSuccess) {
            $effectiveness.RecommendedActions += "Excel macro enablement successful - monitor for macro execution issues"
        } else {
            if ($effectiveness.RemainingIssues.Count -gt 2) {
                $effectiveness.RecommendedActions += "Multiple configuration issues detected - review Excel installation"
            }
            $effectiveness.RecommendedActions += "Schedule follow-up validation in 24 hours"
        }
...
function Send-HealthClinicUpdate {
    ...
    try {
        $healthUpdate = @{
            Automation = "M365_Excel_EnableAllMacros"
            Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            RemediationTime = $RemediationTime.ToString("yyyy-MM-dd HH:mm:ss")
            HealthScore = $ValidationStatus.HealthScore
            ...
            ComplianceStatus = $Effectiveness.ComplianceStatus
            OfficeVersionsConfigured = $ValidationStatus.OfficeVersionsChecked -join ", "
            TrustedLocationsCount = $ValidationStatus.TrustedLocationsCount
            SecurityLevelValid = $ValidationStatus.SecurityLevelValid
            VBOMAccessEnabled = $ValidationStatus.VBOMAccessEnabled
        ...
        Write-EventLogEntry "HEALTH_CLINIC_UPDATE: $healthJson" -EventId 6010
        ...
        if (Test-Path $healthTrackingPath) {
            $trackingFile = Join-Path $healthTrackingPath "Excel_Macro_Health_$(Get-Date -Format 'yyyyMMdd').json"
...
function Start-ExcelMacroFollowUp {
    $startTime = Get-Date
    
    try {
        Write-EventLogEntry "Starting Excel macro enablement follow-up..." -EventId 6001
        
        # Wait for settings to take effect if delay specified
        if ($ValidationDelayMinutes -gt 0) {
            Write-Output "Waiting $ValidationDelayMinutes minutes for Excel settings to stabilize..."
            Start-Sleep -Seconds ($ValidationDelayMinutes * 60)
        }
        
        # Step 1: Validate macro settings
        Write-Output "Step 1: Validating Excel macro settings..."
        $validationStatus = Test-MacroSettingsValidation
        
        # Step 2: Test remediation effectiveness
        ...
        $healthClinicUpdate = Send-HealthClinicUpdate -ValidationStatus $validationStatus -Effectiveness $effectiveness -RemediationTime $startTime.AddMinutes(-$ValidationDelayMinutes)
        ...
        # Log final results
        $logMessage = "Excel macro follow-up completed. Health Score: $($validationStatus.HealthScore)%, Compliance: $($effectiveness.ComplianceStatus), Success: $($effectiveness.OverallSuccess)"
        Write-EventLogEntry $logMessage -EventId 6002
        ...
        Write-EventLogEntry "Excel macro follow-up failed: $(<#
.SCRIPT NAME
		FollowUp_M365_Excel_EnableAllMacros.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" -Type "Error" -EventId 6003
...
$followUp = Start-ExcelMacroFollowUp

# Output results for SysTrack
Write-Output "EXCEL_MACRO_FOLLOWUP_STATUS: $($followUp.Success)"
Write-Output "HEALTH_SCORE: $($followUp.HealthScore)%"
Write-Output "VALIDATION_DURATION: $($followUp.Duration.TotalSeconds) seconds"

if ($followUp.Success) {
    Write-Output "OFFICE_VERSIONS_CHECKED: $($followUp.ValidationStatus.OfficeVersionsChecked -join ', ')"
    Write-Output "TRUSTED_LOCATIONS_COUNT: $($followUp.ValidationStatus.TrustedLocationsCount)"
    Write-Output "SECURITY_LEVEL_VALID: $($followUp.ValidationStatus.SecurityLevelValid)"
    Write-Output "VBOM_ACCESS_ENABLED: $($followUp.ValidationStatus.VBOMAccessEnabled)"
    ...
    Write-Output "Excel macro enablement follow-up completed successfully"
    exit 0
} else {
    Write-Output "FOLLOWUP_ERROR: $($followUp.Error)"
    Write-Output "Excel macro enablement follow-up failed"
    exit 2
}
