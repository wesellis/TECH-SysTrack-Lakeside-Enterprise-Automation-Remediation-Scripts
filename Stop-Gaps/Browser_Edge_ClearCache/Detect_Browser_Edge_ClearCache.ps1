...
# Initialize logging
$LogName = "Application"
$Source = "SysTrack_Edge_Cache_Detection"
...
function Measure-CacheSize {
    ...
    try {
        if (-not (Test-Path $CachePath)) {
            return @{ SizeMB = 0; FileCount = 0; HasCorruption = $false }
        }
...
function Test-PerformanceImpact {
    param([int]$TotalCacheSizeMB)
    
    if ($TotalCacheSizeMB -gt 1000) {
        return "High"
    }
    elseif ($TotalCacheSizeMB -gt 500) {
        return "Medium"
    }
    elseif ($TotalCacheSizeMB -gt 250) {
        return "Low"
    }
    else {
        return "None"
...
try {
    Write-LogEntry "Starting Edge cache detection with MaxCacheSizeMB: $MaxCacheSizeMB"
    ...
    foreach ($Profile in $Profiles) {
        ...
        $DetectionResult.TotalCacheSizeMB += $CacheMetrics.SizeMB
        
        if ($CacheMetrics.SizeMB -gt $DetectionResult.MaxCacheSizeFound) {
            $DetectionResult.MaxCacheSizeFound = $CacheMetrics.SizeMB
        }
        
        # Track corruption
        if ($CacheMetrics.HasCorruption) {
            $DetectionResult.CorruptedFiles += "$($Profile.UserName)\$($Profile.ProfileName): $($CacheMetrics.CorruptedFileCount) corrupted files"
        }
        
        Write-LogEntry "Profile $($Profile.ProfileName): Cache $($CacheMetrics.SizeMB)MB ($($CacheMetrics.FileCount) files)"
        
        if ($CacheMetrics.HasCorruption) {
            Write-LogEntry "Corruption detected in profile $($Profile.ProfileName): $($CacheMetrics.CorruptedFileCount) files" -EntryType Warning
        }
    }
    
    # Determine performance impact
    $DetectionResult.PerformanceImpact = Test-PerformanceImpact -TotalCacheSizeMB $DetectionResult.TotalCacheSizeMB
    ...
    $DetectionResult.ActionRequired = (
        $DetectionResult.TotalCacheSizeMB -gt $MaxCacheSizeMB -or
        $DetectionResult.CorruptedFiles.Count -gt 0
    )
    
    if ($DetectionResult.ActionRequired) {
        $DetectionResult.RecommendedAction = "Clear Edge cache"
        
        $Reasons = @()
        if ($DetectionResult.TotalCacheSizeMB -gt $MaxCacheSizeMB) {
            $Reasons += "cache size exceeds limit ($($DetectionResult.TotalCacheSizeMB)MB > $MaxCacheSizeMB MB)"
        }
        if ($DetectionResult.CorruptedFiles.Count -gt 0) {
            $Reasons += "corrupted cache files detected ($($DetectionResult.CorruptedFiles.Count) issues)"
        }
        
        $Message = "Edge cache cleanup required: " + ($Reasons -join ", ") + ". Performance impact: $($DetectionResult.PerformanceImpact)"
        ...
        $Message = "Edge cache within acceptable limits. Total cache: $($DetectionResult.TotalCacheSizeMB)MB, Performance impact: $($DetectionResult.PerformanceImpact)"
