...
try {
    ...
    [ValidateSet("Quick", "Standard", "Detailed")]
    ...
    [Parameter(Mandatory = $false)]
    [bool]$GenerateHealthReport = $true,
...
# Script metadata
$scriptVersion = "1.0.0"
$scriptName = "FollowUp_SCCM_Client_FileCollectionCycle"
$logSource = "SysTrack_SCCM_FileCollection_FollowUp"
...
function Write-LogEntry {
    ...
    try {
        ...
        Write-EventLog -LogName "Application" -Source $logSource -EntryType Information -EventId 3000 -Message $logMessage
    }
    catch {
...
function Test-FileCollectionCycleHealth {
    [CmdletBinding()]
    param()
    
    Write-LogEntry -Message "Validating file collection cycle health" -Level "Information"
...
ExitScript -msg "Script Completed" -code $returnCode.InventoryActionID -eq $script:sccmFileCollectionCycleGUID }
        ...
        if ($inventoryAgent) {
            $healthResults.CycleEnabled = $true
            
            # Check recent execution
            $lastExecution = [System.Management.ManagementDateTimeConverter]::ToDateTime($inventoryAgent.LastMajorReportTime)
            if ($lastExecution -gt (Get-Date).AddHours(-72)) {
                $healthResults.RecentExecution = $true
                Write-LogEntry -Message "File collection cycle executed recently: $lastExecution" -Level "Debug"
            }
        }
        
        # Check file collection policies
        $policies = Get-WmiObject -Namespace "ROOT\CCM\Policy\Machine\ActualConfig" -Class "CCM_FileCollectionAgent" -ErrorAction SilentlyContinue
        if ($policies -and $policies.Count -gt 0) {
            $healthResults.PolicyCompliance = $true
            Write-LogEntry -Message "File collection policies found: $($policies.Count)" -Level "Debug"
        }
...
ExitScript -msg "Script Completed" -code $returnCode -eq $true }).Count
        $totalChecks = $healthResults.Keys.Count - 1  # Exclude OverallHealthy
        
        if ($healthyChecks -ge ($totalChecks * 0.75)) {
            $healthResults.OverallHealthy = $true
            Write-LogEntry -Message "File collection cycle health: HEALTHY ($healthyChecks/$totalChecks checks passed)" -Level "Information"
        }
        else {
            Write-LogEntry -Message "File collection cycle health: UNHEALTHY ($healthyChecks/$totalChecks checks passed)" -Level "Warning"
        }
        
        $global:followUpResults.ValidationResults.HealthCheck = $healthResults
        return $healthResults
    }
    catch {
        Write-LogEntry -Message "Failed to validate file collection cycle health: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_FileCollectionCycle.ps1
...
function Test-RemediationSuccess {
    ...
    try {
        # Check Event Log for recent remediation activity
        $remediationEvents = Get-WinEvent -FilterHashtable @{
            LogName = "Application"
            ProviderName = "SysTrack_SCCM_FileCollection_Remediation"
            StartTime = (Get-Date).AddHours(-24)
        } -ErrorAction SilentlyContinue | Sort-Object TimeCreated -Descending
        
        $remediationSuccess = $false
        if ($remediationEvents) {
            $latestEvent = $remediationEvents[0]
            if ($latestEvent.LevelDisplayName -eq "Information" -and $latestEvent.Message -like "*SUCCESS*") {
                $remediationSuccess = $true
                Write-LogEntry -Message "Recent successful remediation detected at $($latestEvent.TimeCreated)" -Level "Information"
            }
        }
        
        $global:followUpResults.RemediationSuccess = $remediationSuccess
...
function New-HealthReport {
    ...
    if (-not $GenerateHealthReport) {
        return
    }
    
    Write-LogEntry -Message "Generating health clinic integration report" -Level "Information"
    
    try {
        $healthReport = @{
            Timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
            ComputerName = $env:COMPUTERNAME
            Component = "SCCM_FileCollection"
            HealthStatus = $global:followUpResults.OverallHealth
            ValidationResults = $global:followUpResults.ValidationResults
            Metrics = $global:followUpResults.HealthMetrics
            Recommendations = $global:followUpResults.Recommendations
            RemediationSuccess = $global:followUpResults.RemediationSuccess
        }
        
        # Save health report for health clinic consumption
        $reportPath = "${env:TEMP}\SCCM_FileCollection_HealthReport_$(Get-Date -Format 'yyyyMMdd_HHmmss').json"
        $healthReport | ConvertTo-Json -Depth 10 | Out-File -FilePath $reportPath -Encoding UTF8
        
        Write-LogEntry -Message "Health report generated: $reportPath" -Level "Information"
        
        # Add to global results
        $global:followUpResults.HealthMetrics.ReportGenerated = @{
        ...
        Write-LogEntry -Message "Failed to generate health report: $(<#
.SCRIPT NAME
		FollowUp_SCCM_Client_FileCollectionCycle.ps1
...
# Main execution
function Start-FollowUpValidation {
    Write-LogEntry -Message "Starting SCCM file collection cycle follow-up validation" -Level "Information"
    Write-LogEntry -Message "Validation depth: $ValidationDepth" -Level "Information"
    
    try {
        # Validate remediation execution
        $remediationSuccess = Test-RemediationSuccess
        
        # Validate current health status
        $healthResults = Test-FileCollectionCycleHealth
        
        # Determine overall status
        if ($remediationSuccess -and $healthResults.OverallHealthy) {
            $global:followUpResults.OverallHealth = "Healthy"
            $global:followUpResults.ExitCode = 0
            Write-LogEntry -Message "SUCCESS: File collection cycle is healthy after remediation" -Level "Information"
        }
        elseif ($remediationSuccess -or $healthResults.OverallHealthy) {
            $global:followUpResults.OverallHealth = "Warning"
            $global:followUpResults.ExitCode = 1
            Write-LogEntry -Message "PARTIAL SUCCESS: File collection cycle shows mixed health status" -Level "Warning"
        }
        elseif ($remediationSuccess) {
            $global:followUpResults.OverallHealth = "Monitoring"
            $global:followUpResults.ExitCode = 2
            Write-LogEntry -Message "MONITORING: Remediation successful but health validation inconclusive" -Level "Warning"
        }
        else {
            $global:followUpResults.OverallHealth = "Critical"
            $global:followUpResults.ExitCode = 3
            Write-LogEntry -Message "CRITICAL: File collection cycle remediation failed or health issues persist" -Level "Error"
        }
...
try {
    ...
    # Continue without event logging if it fails
}

# Execute validation
Start-FollowUpValidation
...
$outputResults = @{
    ...
    OverallHealth = $global:followUpResults.OverallHealth
    RemediationSuccess = $global:followUpResults.RemediationSuccess
    ExitCode = $global:followUpResults.ExitCode
    Summary = @{
        HealthStatus = $global:followUpResults.OverallHealth
        RemediationVerified = $global:followUpResults.RemediationSuccess
...
Write-Host "SCCM_FILE_COLLECTION_FOLLOWUP_RESULT:$jsonOutput"

# Write final status to Event Log
$finalMessage = "SCCM File Collection Follow-up completed - Health: $($global:followUpResults.OverallHealth), Remediation Success: $($global:followUpResults.RemediationSuccess)"
