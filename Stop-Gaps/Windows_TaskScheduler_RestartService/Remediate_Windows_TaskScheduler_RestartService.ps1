...
.DESCRIPTION 
		Windows Task Scheduler service management. Remediation and repair operations.
...
try {
    ...
    [string]$LogSource = "SysTrack_TaskScheduler_Remediation",
...
function Test-PreConditions {
    try {
        Write-LogEntry "Checking pre-conditions for Task Scheduler service restart" "Information"
        ...
        $preConditions.Checks.IsAdministrator = $isAdmin
        if (-not $isAdmin) {
            $preConditions.Issues += "Script must be run with Administrator privileges"
            $preConditions.Passed = $false
        }
        
        # Check if Task Scheduler service exists
        $taskSchedulerService = Get-Service -Name "Schedule" -ErrorAction SilentlyContinue
        $preConditions.Checks.ServiceExists = ($taskSchedulerService -ne $null)
        if (-not $taskSchedulerService) {
            $preConditions.Issues += "Task Scheduler service not found on system"
            $preConditions.Passed = $false
        }
        
        # Check system resources
        try {
            $freeMemory = Get-WmiObject -Class Win32_OperatingSystem | ForEach-Object {
                [math]::Round(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.FreePhysicalMemory / 1KB, 2)
            }
            $preConditions.Checks.AvailableMemoryMB = $freeMemory
            
            if ($freeMemory -lt 500) {  # Less than 500MB free
                $preConditions.Issues += "Low system memory may affect service restart ($freeMemory MB available)"
                $preConditions.Passed = $false
            }
        }
        catch {
            Write-LogEntry "Failed to check system memory: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
        ...
        # Check if there are any critical running tasks
        try {
            ...
            $runningTasks = @()
            function Get-RunningTasksFromFolder {
                param($folder)
                ...
                try {
                    ...
                    foreach ($subFolder in $subFolders) {
                        Get-RunningTasksFromFolder $subFolder
                    }
                }
                catch {
                    Write-LogEntry "Failed to check tasks in folder: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
            ...
            Get-RunningTasksFromFolder $rootFolder
            $preConditions.Checks.RunningTasks = $runningTasks.Count
            
            if ($runningTasks.Count -gt 0) {
                $preConditions.Issues += "$($runningTasks.Count) tasks are currently running - restart may interrupt them"
            ...
            Write-LogEntry "Failed to check running tasks: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
            $preConditions.Checks.RunningTasks = "Unknown"
        }
        
        # Check dependency services
        $dependencies = @("RpcSs", "EventLog", "Winmgmt")
        $preConditions.Checks.Dependencies = @{}
        
        foreach ($dep in $dependencies) {
            $depService = Get-Service -Name $dep -ErrorAction SilentlyContinue
            if ($depService) {
                $preConditions.Checks.Dependencies[$dep] = $depService.Status
                if ($depService.Status -ne "Running") {
                    $preConditions.Issues += "Dependency service '$dep' is not running"
                    $preConditions.Passed = $false
                }
            }
            else {
                $preConditions.Checks.Dependencies[$dep] = "NotFound"
                $preConditions.Issues += "Dependency service '$dep' not found"
                $preConditions.Passed = $false
            }
        }
        
        Write-LogEntry "Pre-condition check completed. Passed: $($preConditions.Passed)" "Information"
        return $preConditions
    }
    catch {
        Write-LogEntry "Pre-condition check failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        return @{
            Passed = $false
            Issues = @("Pre-condition check failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
function Backup-TaskConfiguration {
    try {
        Write-LogEntry "Creating Task Scheduler configuration backup" "Information"
        ...
        $backupDir = "$env:TEMP\TaskScheduler_Backup_$(Get-Date -Format 'yyyyMMdd_HHmmss')"
        New-Item -Path $backupDir -ItemType Directory -Force | Out-Null
        $backupResult.BackupPath = $backupDir
        
        # Export task definitions using schtasks
        try {
            Write-LogEntry "Exporting task definitions" "Information"
            
            # Get all task names
            $taskList = schtasks /query /fo csv | ConvertFrom-Csv
            $taskCount = 0
            
            foreach ($task in $taskList) {
                if ($task.TaskName -and $task.TaskName -ne "TaskName" -and $task.TaskName.Trim() -ne "") {
                    try {
                        $taskName = $task.TaskName.Trim()
                        $safeName = $taskName -replace '[\\/:*?"<>|]', '_'
                        $exportPath = Join-Path $backupDir "$safeName.xml"
                        
                        $exportResult = schtasks /query /tn $taskName /xml 2>$null
                        if ($LASTEXITCODE -eq 0 -and $exportResult) {
                            $exportResult | Out-File -FilePath $exportPath -Encoding UTF8
                            $taskCount++
                        }
                    }
                    catch {
                        $backupResult.Issues += "Failed to export task '$taskName': $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
            ...
            $backupResult.TaskCount = $taskCount
            Write-LogEntry "Exported $taskCount task definitions to backup" "Information"
        }
        catch {
            $backupResult.Issues += "Failed to export task definitions: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        # Copy task store directory
        try {
            $sourceStore = "$env:SystemRoot\System32\Tasks"
            $backupStore = Join-Path $backupDir "TaskStore"
            
            if (Test-Path $sourceStore) {
                Copy-Item -Path $sourceStore -Destination $backupStore -Recurse -Force
                Write-LogEntry "Task store directory backed up" "Information"
            }
        }
        catch {
            $backupResult.Issues += "Failed to backup task store directory: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        $summary = @{
            ...
            TaskCount = $backupResult.TaskCount
            BackupPath = $backupResult.BackupPath
            Issues = $backupResult.Issues
        } | ConvertTo-Json -Depth 3
        
        $summary | Out-File -FilePath (Join-Path $backupDir "backup_summary.json") -Encoding UTF8
        
        if ($backupResult.TaskCount -gt 0 -and $backupResult.Issues.Count -eq 0) {
            $backupResult.Success = $true
            Write-LogEntry "Task configuration backup completed successfully" "Information"
        }
        else {
            Write-LogEntry "Task configuration backup completed with issues" "Warning"
        }
        
        return $backupResult
    }
    catch {
        Write-LogEntry "Task configuration backup failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
function Stop-TaskSchedulerService {
    param(
    ...
    try {
        Write-LogEntry "Stopping Task Scheduler service" "Information"
        ...
        if (-not $service) {
            $stopResult.Issues += "Task Scheduler service not found"
            return $stopResult
        }
        
        if ($service.Status -eq "Stopped") {
            Write-LogEntry "Task Scheduler service is already stopped" "Information"
            $stopResult.Success = $true
            $stopResult.FinalStatus = "Stopped"
            return $stopResult
        }
        ...
        for ($attempt = 1; $attempt -le $RetryCount; $attempt++) {
            try {
                Write-LogEntry "Stop attempt $attempt of $RetryCount" "Information"
                ...
                $stopwatch.Stop()
                $stopResult.TimeToStop = [math]::Round($stopwatch.Elapsed.TotalSeconds, 2)
                $stopResult.Success = $true
                $stopResult.FinalStatus = "Stopped"
                
                Write-LogEntry "Task Scheduler service stopped successfully in $($stopResult.TimeToStop) seconds" "Information"
                break
            }
            catch [System.ServiceProcess.TimeoutException] {
                Write-LogEntry "Service stop timeout on attempt $attempt" "Warning"
                if ($attempt -eq $RetryCount) {
                    $stopResult.Issues += "Service failed to stop within timeout period"
                }
                Start-Sleep -Seconds 5
            }
            catch {
                Write-LogEntry "Service stop failed on attempt $attempt: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
                if ($attempt -eq $RetryCount) {
                    $stopResult.Issues += "Service stop failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        # Final status check
        $service.Refresh()
        $stopResult.FinalStatus = $service.Status
        
        if ($service.Status -ne "Stopped") {
            $stopResult.Issues += "Service is in unexpected state: $($service.Status)"
        ...
        Write-LogEntry "Task Scheduler service stop operation failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
function Start-TaskSchedulerService {
    param(
        [int]$TimeoutSeconds = 60,
        [int]$RetryCount = 3
    )
    ...
    try {
        Write-LogEntry "Starting Task Scheduler service" "Information"
        ...
        $service = Get-Service -Name "Schedule" -ErrorAction SilentlyContinue
        if (-not $service) {
            $startResult.Issues += "Task Scheduler service not found"
            return $startResult
        }
        
        if ($service.Status -eq "Running") {
            Write-LogEntry "Task Scheduler service is already running" "Information"
            $startResult.Success = $true
            $startResult.FinalStatus = "Running"
            return $startResult
        }
        ...
        for ($attempt = 1; $attempt -le $RetryCount; $attempt++) {
            try {
                Write-LogEntry "Start attempt $attempt of $RetryCount" "Information"
                ...
                $stopwatch.Stop()
                $startResult.TimeToStart = [math]::Round($stopwatch.Elapsed.TotalSeconds, 2)
                $startResult.Success = $true
                $startResult.FinalStatus = "Running"
                
                Write-LogEntry "Task Scheduler service started successfully in $($startResult.TimeToStart) seconds" "Information"
                break
            }
            catch [System.ServiceProcess.TimeoutException] {
                Write-LogEntry "Service start timeout on attempt $attempt" "Warning"
                if ($attempt -eq $RetryCount) {
                    $startResult.Issues += "Service failed to start within timeout period"
                }
                Start-Sleep -Seconds 5
            }
            catch {
                Write-LogEntry "Service start failed on attempt $attempt: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Warning"
                if ($attempt -eq $RetryCount) {
                    $startResult.Issues += "Service start failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        # Final status check
        $service.Refresh()
        $startResult.FinalStatus = $service.Status
        
        if ($service.Status -ne "Running") {
            $startResult.Issues += "Service is in unexpected state: $($service.Status)"
        ...
        Write-LogEntry "Task Scheduler service start operation failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
function Test-ServiceFunctionality {
    try {
        Write-LogEntry "Testing Task Scheduler service functionality" "Information"
        ...
        # Test COM object connectivity
        try {
            $taskScheduler = New-Object -ComObject "Schedule.Service"
            $taskScheduler.Connect()
            $funcTest.Tests.COMConnectivity = "Success"
            
            # Test basic enumeration
            $rootFolder = $taskScheduler.GetFolder("\")
            $tasks = $rootFolder.GetTasks(0)
            $funcTest.Tests.TaskEnumeration = "Success ($($tasks.Count) tasks)"
            
            [System.Runtime.Interopservices.Marshal]::ReleaseComObject($taskScheduler) | Out-Null
        }
        catch {
            $funcTest.Tests.COMConnectivity = "Failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
            $funcTest.Issues += "COM connectivity test failed"
        }
        
        # Test schtasks command line interface
        try {
            $schtasksResult = schtasks /query /fo list 2>$null
            if ($LASTEXITCODE -eq 0) {
                $funcTest.Tests.CommandLineInterface = "Success"
            }
            else {
                $funcTest.Tests.CommandLineInterface = "Failed (Exit code: $LASTEXITCODE)"
                $funcTest.Issues += "Command line interface test failed"
            }
        }
        catch {
            $funcTest.Tests.CommandLineInterface = "Failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        try {
            ...
            $funcTest.Tests.EventLogAccess = "Success"
        }
        catch {
            $funcTest.Tests.EventLogAccess = "Warning: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.ProcessId -ErrorAction SilentlyContinue
            ...
            if ($serviceProcess) {
                $funcTest.Tests.ServiceProcess = "Success (PID: $($serviceProcess.Id))"
            }
            else {
                $funcTest.Tests.ServiceProcess = "Failed - No process found"
                $funcTest.Issues += "Service process test failed"
            }
        }
        catch {
            $funcTest.Tests.ServiceProcess = "Failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)"
        ...
        $passedTests = $funcTest.Tests.Values | Where-Object { <#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
#NOTE: You MUST assign a value to $returnCode prior to the next statement based on script resutls.
ExitScript -msg "Script Completed" -code $returnCode -like "Success*" }
        if ($passedTests.Count -ge 3) {  # At least 3 tests must pass
            $funcTest.Success = $true
            Write-LogEntry "Task Scheduler functionality tests passed ($($passedTests.Count) of $($funcTest.Tests.Count))" "Information"
        }
        else {
            Write-LogEntry "Task Scheduler functionality tests failed ($($passedTests.Count) of $($funcTest.Tests.Count) passed)" "Warning"
        }
        
        return $funcTest
    }
    catch {
        Write-LogEntry "Task Scheduler functionality testing failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
ExitScript -msg "Script Completed" -code $returnCode.Exception.Message)" "Error"
        return @{
            Success = $false
            Issues = @("Functionality testing failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
...
try {
    Write-LogEntry "Starting Task Scheduler service remediation" "Information"
    ...
    Write-LogEntry "Performing pre-condition checks" "Information"
    $preConditions = Test-PreConditions
    $remediationResults.Details.PreConditions = $preConditions
    $remediationResults.Actions += "Pre-condition validation"
    
    if (-not $preConditions.Passed) {
        $errorMsg = "Pre-conditions not met: $($preConditions.Issues -join '; ')"
        Write-LogEntry $errorMsg "Error"
        $remediationResults.Issues += $errorMsg
        
        $errorOutput = @{
            RemediationResult = $false
            Success = $false
            Issues = $remediationResults.Issues
            Summary = $errorMsg
    ...
    # Step 2: Backup task configuration (if requested)
    if ($BackupTasks) {
        Write-LogEntry "Creating task configuration backup" "Information"
        $backupResult = Backup-TaskConfiguration
        $remediationResults.Details.Backup = $backupResult
        $remediationResults.Actions += "Task configuration backup"
        
        if (-not $backupResult.Success) {
            Write-LogEntry "Task backup failed but proceeding with restart" "Warning"
            $remediationResults.Issues += "Task backup failed: $($backupResult.Issues -join '; ')"
        }
    }
    
    # Step 3: Capture initial state
    $initialService = Get-Service -Name "Schedule"
    $remediationResults.Details.InitialStatus = $initialService.Status
    
    $restartStopwatch = [System.Diagnostics.Stopwatch]::StartNew()
    
    # Step 4: Stop Task Scheduler service
    Write-LogEntry "Stopping Task Scheduler service" "Information"
    $stopResult = Stop-TaskSchedulerService -TimeoutSeconds $ServiceTimeout -RetryCount $MaxRetries
    $remediationResults.Details.StopOperation = $stopResult
    $remediationResults.Actions += "Service stop"
    
    if (-not $stopResult.Success) {
        $errorMsg = "Failed to stop Task Scheduler service: $($stopResult.Issues -join '; ')"
        Write-LogEntry $errorMsg "Error"
        $remediationResults.Issues += $errorMsg
    ...
    # Step 5: Brief pause to ensure clean state
    Write-LogEntry "Pausing for service state stabilization" "Information"
    Start-Sleep -Seconds 3
    
    # Step 6: Start Task Scheduler service
    Write-LogEntry "Starting Task Scheduler service" "Information"
    $startResult = Start-TaskSchedulerService -TimeoutSeconds $ServiceTimeout -RetryCount $MaxRetries
    $remediationResults.Details.StartOperation = $startResult
    $remediationResults.Actions += "Service start"
    
    if (-not $startResult.Success) {
        $errorMsg = "Failed to start Task Scheduler service: $($startResult.Issues -join '; ')"
        Write-LogEntry $errorMsg "Error"
        $remediationResults.Issues += $errorMsg
    ...
    $remediationResults.RestartDuration = [math]::Round($restartStopwatch.Elapsed.TotalSeconds, 2)
    
    # Step 7: Post-restart validation (if requested)
    if ($ValidateAfterRestart) {
        Write-LogEntry "Performing post-restart validation" "Information"
        Start-Sleep -Seconds 5  # Allow service to fully initialize
        
        $funcTest = Test-ServiceFunctionality
        $remediationResults.Details.FunctionalityTest = $funcTest
        $remediationResults.Actions += "Post-restart validation"
        
        if (-not $funcTest.Success) {
            Write-LogEntry "Post-restart validation failed" "Warning"
            $remediationResults.Issues += "Post-restart validation failed: $($funcTest.Issues -join '; ')"
        }
    }
    
    # Step 8: Final status verification
    $finalService = Get-Service -Name "Schedule"
    $remediationResults.Details.FinalStatus = $finalService.Status
    
    if ($finalService.Status -eq "Running") {
        $remediationResults.Success = $true
        Write-LogEntry "Task Scheduler service restart completed successfully in $($remediationResults.RestartDuration) seconds" "Information"
    }
    else {
        $remediationResults.Issues += "Service is not running after restart (Status: $($finalService.Status))"
        Write-LogEntry "Task Scheduler service restart failed - service not running" "Error"
    }
    
    # Generate summary
    $summary = @"
Task Scheduler Service Restart Summary:
- Initial Status: $($remediationResults.Details.InitialStatus)
- Final Status: $($remediationResults.Details.FinalStatus)
- Restart Duration: $($remediationResults.RestartDuration) seconds
- Actions Performed: $($remediationResults.Actions -join ', ')
- Success: $($remediationResults.Success)
- Issues: $($remediationResults.Issues.Count)
"@
    ...
    $output = @{
        RemediationResult = $remediationResults.Success
        Success = $remediationResults.Success
        RestartDuration = $remediationResults.RestartDuration
        IssueCount = $remediationResults.Issues.Count
        Summary = $summary
        Details = $remediationResults
    } | ConvertTo-Json -Depth 10 -Compress
    ...
    if ($remediationResults.Success) {
        Write-LogEntry "Task Scheduler service remediation completed successfully" "Information"
        exit 0  # Success
    }
    elseif ($remediationResults.Issues.Count -gt 0 -and $finalService.Status -eq "Running") {
        Write-LogEntry "Task Scheduler service remediation completed with warnings" "Warning"
        exit 2  # Partial success
    }
    else {
        Write-LogEntry "Task Scheduler service remediation failed" "Error"
        exit 1  # Failure
    }
}
catch {
    $errorMessage = "Task Scheduler service remediation failed: $(<#
.SCRIPT NAME
		Remediate_Windows_TaskScheduler_RestartService.ps1
